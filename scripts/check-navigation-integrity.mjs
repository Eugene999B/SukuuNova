...
function normalize(href){return href.split(/[?#]/)[0].replace(/\/+/g,"/").replace(/\/$/,"")||"/";}
function routeCandidates(href){const clean=normalize(href);const segments=clean.split("/").filter(Boolean);const candidates=[`src/app${clean}/page.tsx`];for(let i=0;i<segments.length;i+=1){for(const token of ["[id]","[slug]"]){const copy=[...segments];copy[i]=token;candidates.push(`src/app/${copy.join("/")}/page.tsx`);}}return [...new Set(candidates)];}
function hasApprovedCatchAll(href){const clean=normalize(href);const first=clean.split("/").filter(Boolean)[0];return first?approvedCatchAllPrefixes.has(`/${first}`)&&exists(`src/app/${first}/[...module]/page.tsx`):false;}
...
for(const{href,source}of unique){if(/^(https?:|mailto:|tel:|javascript:)/.test(href)||href.startsWith("/api/"))continue;if(href.includes("${")||href.includes("{"))continue;if(href.startsWith("#"))continue;const clean=normalize(href);if(approvedLegacyAliases.has(clean)||hasApprovedCatchAll(href))continue;if(!routeCandidates(href).some(exists))failures.push(`${source}: internal link has no concrete Next page: ${href}`);}
