...
function routeCandidates(href) {
  ...
  for (let i = 0; i < segments.length; i += 1) {
    for (const token of ["[id]", "[slug]"]) {
      const copy = [...segments]; copy[i] = token; candidates.push(`src/app/${copy.join("/")}/page.tsx`);
    }
  }
...
for (const rootDir of scanRoots) {
  ...
  for (const file of walk(rootDir)) {
    ...
    for (const match of text.matchAll(/\bhref\s*=\s*["']([^"']+)["']/g)) links.push({ href: match[1], source: relative });
    for (const match of text.matchAll(/\bhref:\s*["']([^"']+)["']/g)) links.push({ href: match[1], source: relative });
  }
}

const unique = [...new Map(links.map((item) => [`${item.source}:${item.href}`, item])).values()];
for (const { href, source } of unique) {
  if (/^(https?:|mailto:|tel:|javascript:)/.test(href) || href.startsWith("/api/")) continue;
  if (href.includes("${") || href.includes("{")) continue;
  const clean = href.split("?")[0];
  if (approvedLegacyAliases.has(clean) || hasApprovedCatchAll(href)) continue;
  if (!routeCandidates(href).some(exists)) failures.push(`${source}: internal link has no concrete Next page: ${href}`);
}
