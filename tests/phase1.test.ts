...
describe("Phase 1 MVP security and workflow gates", () => {
  ...
  beforeAll(async () => {
    ...
    await withTenant(fixture.schoolId, async (tx) => {
      await tx.schoolSettings.update({ where: { schoolId: fixture.schoolId }, data: { expectedResumptionTime: "08:00", attendanceGraceMinutes: 10, timezone: "Africa/Accra" } });
      const year = await tx.academicYear.create({ data: { schoolId: fixture.schoolId, name: "Phase 1 Test Year", startDate: new Date("2026-01-01T00:00:00.000Z"), endDate: new Date("2026-12-31T00:00:00.000Z") } });
      academicYearId = year.id;
      const term = await tx.term.create({ data: { schoolId: fixture.schoolId, academicYearId: year.id, name: "Term 1", startDate: new Date("2026-01-01T00:00:00.000Z"), endDate: new Date("2026-04-30T00:00:00.000Z") });
      termId = term.id;
      ...
      const schoolClass = await tx.class.create({ data: { schoolId: fixture.schoolId, name: "Primary 6 " + fixture.schoolId, classTeacherId: teacherId } });
  ...
  it("marks post-grace arrival late and suppresses attendance on a holiday", async () => {
    await withTenant(fixture.schoolId, async (tx) => {
      const event = await recordAttendance(tx, { schoolId: fixture.schoolId, actorId: fixture.ownerId, target: { studentId }, type: "in", method: "manual", timestamp: new Date("2026-02-02T08:11:00.000Z") });
  ...
  it("sends SMS synchronously without requiring a worker", async () => {
    ...
    await withTenant(fixture.schoolId, async (tx) => {
      const messages = await enqueueSms(tx, { schoolId: fixture.schoolId, recipientType: "guardian", recipientId: parentId, recipientPhone: "+233200000001", body: "Phase 1 synchronous SMS proof" }, { sms: sender });
