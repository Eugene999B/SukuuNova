...
describe("Phase 1 MVP security and workflow gates", () => {
  ...
  it("marks post-grace arrival late and suppresses attendance on a holiday", async () => {
    await withTenant(fixture.schoolId, async (tx) => {
      const event = await recordAttendance(tx, {
        ...
        type: "in",
        method: "manual",
        timestamp: new Date("2026-02-02T08:11:00.000Z")
      });
      ...
      await expect(
        recordAttendance(tx, {
          ...
          type: "in",
          method: "manual",
          timestamp: new Date("2026-02-03T08:30:00.000Z")
        })
      ).rejects.toMatchObject<Partial<AppError>>({ code: "CALENDAR_BLOCKS_ATTENDANCE" });
      await expect(
  ...
  it("returns 403 when a teacher writes outside an assigned subject", async () => {
    await withTenant(fixture.schoolId, async (tx) => {
      ...
      ).rejects.toMatchObject<Partial<ForbiddenError>>({ status: 403, code: "FORBIDDEN" });
    });
  });
