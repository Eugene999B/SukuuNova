...
import { attendanceSummary, recordAttendance } from "../src/lib/attendance-service";
import { enterScore } from "../src/lib/gradebook-service";
import { approveReportCard, submitReportCard } from "../src/lib/report-card-service";
...
describe("Phase 1 MVP security and workflow gates", () => {
  ...
  beforeAll(async () => {
    ...
    await withTenant(fixture.schoolId, async (tx) => {
      ...
      teacherId = (await tx.user.create({ data: { schoolId: fixture.schoolId, name: "Assigned Teacher", email: `teacher-${fixture.schoolId}@test.invalid`, passwordHash: "test-only" } })).id;
      ...
      classId = (await tx.class.create({ data: { schoolId: fixture.schoolId, name: `Primary 6 ${fixture.schoolId}`, classTeacherId: teacherId } })).id;
      otherClassId = (await tx.class.create({ data: { schoolId: fixture.schoolId, name: `JHS 1 ${fixture.schoolId}` } })).id;
      mathId = (await tx.subject.create({ data: { schoolId: fixture.schoolId, name: `Mathematics ${fixture.schoolId}` } })).id;
  ...
  it("marks post-grace arrival late and suppresses attendance on a holiday", async () => {
    ...
    try {
      await withTenant(fixture.schoolId, async (tx) => {
        const event = await recordAttendance(tx, { schoolId: fixture.schoolId, actorId: fixture.ownerId, target: { studentId }, type: "in", method: "manual", timestamp: new Date("2000-01-01T08:11:00.000Z") });
        ...
        await expect(recordAttendance(tx, { schoolId: fixture.schoolId, actorId: fixture.ownerId, target: { studentId }, type: "in", method: "manual", timestamp: new Date("2026-02-03T08:30:00.000Z") })).rejects.toMatchObject({ code: "CALENDAR_BLOCKS_ATTENDANCE" });
        await expect(attendanceSummary(tx, { actorId: fixture.ownerId, day: holiday, classId, schoolId: fixture.schoolId })).resolves.toEqual({ calendarBlocked: true, present: 0, late: 0, absent: 0 });
      });
    } finally {
      vi.useRealTimers();
    }
  ...
  it("queues SMS for worker delivery", async () => {
    ...
    await withTenant(fixture.schoolId, async (tx) => { const sent = await tx.message.findUnique({ where: { id: messageId } }); expect(sent?.status).toBe("sent"); });
  });

  it("keeps new SIS records isolated by tenant RLS", async () => {
    const other = await createTenantFixture();
    await withTenant(fixture.schoolId, async (tx) => { expect(await tx.student.findUnique({ where: { id: otherStudentId } })).not.toBeNull(); expect(await tx.student.findFirst({ where: { admissionNo: "missing-from-this-tenant" } })).toBeNull(); });
    await withTenant(other.schoolId, async (tx) => { expect(await tx.student.findUnique({ where: { id: studentId } })).toBeNull(); });
  });
});