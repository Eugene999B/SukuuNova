...
describe("Phase 1 MVP security and workflow gates", () => {
    ...
    await withTenant(fixture.schoolId, async (tx) => {
      ...
      mathId = (
        await tx.subject.create({
          data: {
            schoolId: fixture.schoolId,
            name: `Mathematics ${fixture.schoolId}`,
          },
        })
      ).id;
  ...
  it("keeps new SIS records isolated by tenant RLS", async () => {
    ...
    await withTenant(fixture.schoolId, async (tx) => {
      expect(
        await tx.student.findUnique({ where: { id: otherStudentId } })
      ).not.toBeNull();
      expect(
        await tx.student.findFirst({
          where: { admissionNo: "missing-from-this-tenant" },
        })
      ).toBeNull();
    });
