...
import { createCustomRole, customRoleBuilderData, updateCustomRole } from "../src/lib/role-builder-service";
import { confirmSubstitute, createTimetableSlot, suggestSubstitutes } from "../src/lib/timetable-service";
...
describe("Phase 2 differentiator safety gates", () => {
  ...
  beforeAll(async () => {
    ...
    await withTenant(fixture.schoolId, async (tx) => {
      await tx.schoolSettings.update({ where: { schoolId: fixture.schoolId }, data: { expectedResumptionTime: "08:00", timezone: "Africa/Accra", substituteLateMinutes: 20 } });
      for (const permission of ["attendance:record", "attendance:pickup_approve", "payroll:view_own"] as const) {
        await tx.userPermissionOverride.create({ data: { schoolId: fixture.schoolId, userId: fixture.memberId, permissionId: fixture.permissionIds.get(permission)!, granted: true } });
      }
      staffId = (await tx.user.create({ data: { schoolId: fixture.schoolId, name: "Phase 2 Staff", email: "phase2-staff-" + fixture.schoolId + "@test.invalid", passwordHash: "test-only" } })).id;
      otherStaffId = (await tx.user.create({ data: { schoolId: fixture.schoolId, name: "Other Phase 2 Staff", email: "other-phase2-staff-" + fixture.schoolId + "@test.invalid", passwordHash: "test-only" } })).id;
      for (const userId of [staffId, otherStaffId]) await tx.userPermissionOverride.create({ data: { schoolId: fixture.schoolId, userId, permissionId: fixture.permissionIds.get("payroll:view_own")!, granted: true } });
      classId = (await tx.class.create({ data: { schoolId: fixture.schoolId, name: "Phase 2 Class " + fixture.schoolId } })).id;
      otherClassId = (await tx.class.create({ data: { schoolId: fixture.schoolId, name: "Phase 2 Other Class " + fixture.schoolId } })).id;
      subjectId = (await tx.subject.create({ data: { schoolId: fixture.schoolId, name: "Phase 2 Subject " + fixture.schoolId } })).id;
      studentId = (await tx.student.create({ data: { schoolId: fixture.schoolId, admissionNo: "P2-" + fixture.schoolId, name: "Phase 2 Student", classId } })).id;
      guardianId = (await tx.guardian.create({ data: { schoolId: fixture.schoolId, name: "Consenting Guardian", phone: "+233240" + fixture.schoolId.slice(-6) } })).id;
      unlinkedGuardianId = (await tx.guardian.create({ data: { schoolId: fixture.schoolId, name: "Unlinked Guardian", phone: "+233250" + fixture.schoolId.slice(-6) } })).id;
      await tx.studentGuardian.create({ data: { schoolId: fixture.schoolId, studentId, guardianId, relationship: "Parent", isPrimary: true } });
    });
  });

  it("requires linked guardian consent and encrypts the provider face reference", async () => {
    const provider: FaceProvider = { async indexFace() { return { faceId: "provider-face-id-must-not-be-plain" }; }, async searchFace() { return {}; } };
    const image = Buffer.alloc(256, 7).toString("base64");
    await withTenant(fixture.schoolId, async (tx) => {
      await expect(enrollFace(tx, { schoolId: fixture.schoolId, actorId: fixture.ownerId, target: { studentId, consentByGuardianId: unlinkedGuardianId }, image }, provider)).rejects.toMatchObject({ code: "FACE_CONSENT_REQUIRED" });
      const result = await enrollFace(tx, { schoolId: fixture.schoolId, actorId: fixture.ownerId, target: { studentId, consentByGuardianId: guardianId }, image }, provider);
  ...
  it("scopes staff payslip lists and guessed PDF IDs to the signed-in staff member", async () => {
    await withTenant(fixture.schoolId, async (tx) => {
      ...
      await expect(getVisiblePayslipPdf(tx, { actorId: staffId, payslipId: someoneElses.id })).rejects.toMatchObject({ status: 403 });
    });
  });
  ...
  it("suggests substitutes without writing an assignment until explicit confirmation", async () => {
    ...
    await withTenant(fixture.schoolId, async (tx) => {
      ...
      const result = await suggestSubstitutes(tx, { actorId: fixture.ownerId, absentTeacherId: staffId, day, period: 1, asOf: new Date("2026-08-17T10:00:00.000Z") });
