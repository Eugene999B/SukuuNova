...
describe("SukuuNova Phase 4 invariants",()=>{
  it("blocks a school without the requested feature flag",async()=>{const tx={school:{findUnique:async()=>({subscriptionPlan:{name:"Starter",featureFlags:[]}})}} as any;await expect(requireSchoolFeatureInTransaction(tx,"school-a","payroll")).rejects.toMatchObject({code:"FEATURE_NOT_INCLUDED"});expect(featureFlagsContain(["payroll"],"payroll")).toBe(true);});
