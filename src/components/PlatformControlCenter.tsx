...
export default function PlatformControlCenter({ overview, health, audit }: Props) {
  ...
  return <AppShell universe="platform" title="Control Center" subtitle="Operate the SukuuNova network from one accountable view: schools, risk, finance, access and support.">
    ...
    <div className="app-grid kpis"><Stat icon={School} label="Schools" value={String(overview.totals.schools)} meta={`${activeRate}% active across the network`} /><Stat icon={GraduationCap} label="Students" value={Number(overview.totals.students).toLocaleString()} meta={`${Number(overview.totals.classes).toLocaleString()} classes configured`} /><Stat icon={CreditCard} label="Collected" value={`₵${totalCollected.toLocaleString()}`} meta={`${overview.totals.unpaidInvoices} unpaid platform invoices`} /><Stat icon={Gauge} label="Attention index" value={`${averageAttention}/100`} meta={averageAttention >= 60 ? "Immediate review recommended" : averageAttention >= 25 ? "Some schools need review" : "Network currently stable"} /></div>
    <div style={{display:"grid",gridTemplateColumns:"minmax(0,1.55fr) minmax(320px,.85fr)",gap:18,marginTop:18}}>
      <section className="app-card app-panel" style={{padding:20}}><div className="app-card-head"><div><h2>School attention queue</h2><p>Explainable operational scoring. Open the school record to investigate and act.</p></div><Link href="/platform/schools" className="app-pill">View all</Link></div><div style={{display:"grid",gap:4}}>{attention.map(({school,score})=>{const state=attentionLabel(score);return <div key={school.id} className="app-list-row" style={{alignItems:"center"}}><span className="app-list-icon"><BadgeAlert size={16}/></span><div style={{minWidth:0}}><b>{school.name}</b><span>{school.uniqueCode} · {school.studentCount} students · {school.unpaidInvoices}/{school.invoices} invoices unpaid</span></div><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",justifyContent:"flex-end"}}><span style={{fontSize:10,fontWeight:850,color:state.tone,background:state.bg,borderRadius:999,padding:"5px 8px"}}>{state.label} · {score}</span><Link className="app-action" href={`/platform/schools/${school.id}`}><strong>Inspect</strong></Link></div></div>})}</div>{attention.length===0&&<div style={{padding:"22px 0",color:"var(--sn-muted)"}}>No schools are currently visible to this administrator.</div>}</section>
      <section className="app-card app-panel" style={{padding:20}}><div className="app-card-head"><div><h2>Platform posture</h2><p>Core control-plane signals.</p></div></div><div className="app-list-row"><div><b><ShieldCheck size={15} style={{verticalAlign:"-3px",marginRight:7}}/>Access</b><span>Role-based platform permissions enabled</span></div><span className="app-pill">Protected</span></div><div className="app-list-row"><div><b><Activity size={15} style={{verticalAlign:"-3px",marginRight:7}}/>Database</b><span>{health.database} · {health.latencyMs}ms</span></div><span className="app-pill">Live</span></div><div className="app-list-row"><div><b><Users size={15} style={{verticalAlign:"-3px",marginRight:7}}/>Operators</b><span>Use Workers & Permissions for least-privilege access</span></div><Link href="/platform/admins" className="app-pill">Manage</Link></div><div className="app-list-row"><div><b>School coverage</b><span>{overview.totals.activeSchools} active · {overview.totals.suspendedSchools} suspended</span></div><Link href="/platform/schools" className="app-pill">Review</Link></div></section>
    </div>
