...
export function HomeHelpBar() {
  ...
  async function send() {
    ...
    try {
      ...
      if (response.ok) { setSent(true); setReference(typeof body.reference === "string" ? body.reference : ""); }
  ...
  return (
    <section className={`home-help ${open ? "is-open" : ""}`}>
      <div className="home-help-summary">
        <div className="home-help-intro"><span className="home-help-icon"><LifeBuoy size={18} /></span><div><span className="home-help-kicker">Human support</span><strong>Need help deciding where to start?</strong><p>Tell us what you need. Leave an email or phone number so our team can reply.</p></div></div>
      ...
      {open ? <div className="home-help-panel">
        ...
        {sent ? <div className="home-help-success"><strong>Your message is in the support inbox.</strong><p>Thanks for getting in touch. Our team has your message and contact details.</p>{reference && <p style={{overflowWrap:"anywhere"}}>Your reference: <strong>{reference}</strong></p>}<button type="button" onClick={another}>Send another message</button></div> : <div className="home-help-form">
          <label><span>Your name</span><input required maxLength={120} placeholder="e.g. Ama Mensah" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label>
          ...
          <label className="home-help-message"><span>Message</span><textarea required maxLength={5000} placeholder="Describe the question or request…" value={form.message} onChange={(event) => setForm({ ...form, message: event.target.value })} /></label>
