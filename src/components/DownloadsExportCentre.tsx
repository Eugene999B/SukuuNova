  ...
  async function exportDataset(dataset: Dataset, format: OutputKey) {
    ...
    try {
      ...
      if (format === "csv") {
        ...
        const html = `<!doctype html><html><head><meta charset="utf-8"><title>${dataset.title}</title><style>@page{size:A4;margin:15mm}body{font-family:Arial,sans-serif;color:var(--sn-ink);margin:0}header{display:flex;align-items:center;gap:14px;border-bottom:3px solid var(--sn-line);padding-bottom:14px}header img{max-height:58px;max-width:90px}h1{font-size:20px;margin:0 0 4px}h2{font-size:14px;margin:18px 0 10px}p{font-size:10px;color:var(--sn-ink);margin:2px 0}.meta{display:flex;gap:18px;margin:12px 0;font-size:10px}.meta span{font-weight:700}table{width:100%;border-collapse:collapse;font-size:8px}th{background:var(--sn-surface);color:var(--sn-ink);text-align:left;padding:7px}td{border-bottom:1px solid var(--sn-line);padding:6px}tr:nth-child(even){background:var(--sn-surface)}footer{margin-top:18px;border-top:1px solid var(--sn-line);padding-top:8px;font-size:8px;color:var(--sn-ink)}@media print{.no-print{display:none}}</style></head><body><header>${logoUrl ? `<img src="${logoUrl}" alt="School logo">` : ""}<div><h1>${schoolName}</h1><p>${dataset.title}</p></div></header><div class="meta"><span>School code: ${safeCode}</span><span>Generated: ${new Date().toLocaleString("en-GH")}</span><span>Records: ${rows.length - 1}</span></div><h2>Official school export</h2><table><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table><footer>Generated by SukuuNova · ${schoolName} · Keep this document with the school's official records.</footer><script>window.onload=()=>window.print()</script></body></html>`;
