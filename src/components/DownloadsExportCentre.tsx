...
export default function DownloadsExportCentre({ schoolName, logoUrl, schoolCode, terms = [], classes = [] }: Props) {
  ...
  return <div className="space-y-5">
    <section className="rounded-3xl border border-slate-200 bg-slate-950 p-6 text-white shadow-xl">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div><span className="text-[9px] font-black uppercase tracking-[.16em] text-emerald-300">Official downloads & exports</span><h2 className="mt-1 text-2xl font-black">One place for files that actually download or print.</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-300">Generate data exports, class report packs and official printable documents from the same tenant-scoped source.</p></div>
        </div>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div><span className="text-[9px] font-black uppercase tracking-[.13em] text-slate-500">Official document packs</span><h2 className="mt-1 text-base font-black text-slate-900">Choose the class once, then print the document.</h2><p className="mt-1 text-xs text-slate-500">Report cards use the school's selected report theme, logo, learner photographs and frozen signatures.</p></div>
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <label className="grid gap-1 text-[10px] font-black text-slate-700">Term<select value={termId} onChange={(event) => setTermId(event.target.value)} className="min-h-11 border border-slate-300 bg-white px-3 text-xs font-semibold"><option value="">Choose term</option>{terms.map((term) => <option key={term.id} value={term.id}>{term.name}</option>)}</select></label>
        <label className="grid gap-1 text-[10px] font-black text-slate-700">Class<select value={classId} onChange={(event) => setClassId(event.target.value)} className="min-h-11 border border-slate-300 bg-white px-3 text-xs font-semibold"><option value="">Choose class</option>{classes.map((item) => <option key={item.id} value={item.id}>{item.level ? `${item.level} · ` : ""}{item.name}</option>)}</select></label>
      </div>
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        <a href={reportPackHref || reportWorkspaceHref} className={`rounded-2xl border p-4 no-underline ${reportPackHref ? "border-slate-900 bg-slate-900 text-white" : "pointer-events-none border-slate-200 bg-slate-100 text-slate-400"}`}><strong className="block text-sm font-black">Print class report cards</strong><span className="mt-1 block text-[10px] leading-4 opacity-80">{selectedClass && selectedTerm ? `${selectedClass.name} · ${selectedTerm.name}` : "Choose a term and class"}</span></a>
