...
const outputOptions = [
  ...
  { key: "excel", label: "Spreadsheet CSV", detail: "CSV opens directly in Excel" },
  { key: "json", label: "JSON", detail: "Structured system data" },
  { key: "print", label: "Print / Save PDF", detail: "Browser print dialog with PDF option" },
] as const;
...
export default function DownloadsExportCentre({ schoolName, logoUrl, schoolCode }: { schoolName: string; logoUrl?: string | null; schoolCode: string }) {
  ...
  async function exportDataset(dataset: Dataset, format: OutputKey) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try { message = (JSON.parse(source) as { message?: string; error?: string }).message ?? (JSON.parse(source) as { error?: string }).error ?? message; } catch { /* CSV/error body is not JSON. */ }
      ...
      if (format === "csv" || format === "excel") {
        ...
        const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="color-scheme" content="light dark"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${htmlEscape(dataset.title)}</title><style>@page{size:A4;margin:15mm}body{font-family:Arial,sans-serif;color:CanvasText;background:Canvas;margin:0}header{display:flex;align-items:center;gap:14px;border-bottom:3px solid currentColor;padding-bottom:14px}header img{max-height:58px;max-width:90px}h1{font-size:20px;margin:0 0 4px}h2{font-size:14px;margin:18px 0 10px}p{font-size:10px;margin:2px 0}.meta{display:flex;gap:18px;margin:12px 0;font-size:10px}table{width:100%;border-collapse:collapse;font-size:8px}th{background:color-mix(in srgb, CanvasText 10%, Canvas);text-align:left;padding:7px;border-bottom:1px solid currentColor}td{border-bottom:1px solid currentColor;padding:6px}footer{margin-top:18px;border-top:1px solid currentColor;padding-top:8px;font-size:8px}@media print{.no-print{display:none}}</style></head><body><header>${logoUrl ? `<img src="${htmlEscape(logoUrl)}" alt="School logo">` : ""}<div><h1>${htmlEscape(schoolName)}</h1><p>${htmlEscape(dataset.title)}</p></div></header><div class="meta"><span><strong>School code:</strong> ${htmlEscape(safeCode)}</span><span><strong>Generated:</strong> ${htmlEscape(new Date().toLocaleString("en-GH"))}</span><span><strong>Records:</strong> ${String(Math.max(0, rows.length - 1))}</span></div><h2>Official school export</h2><table><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table><footer>Generated by SukuuNova · ${htmlEscape(schoolName)} · Keep this document with the school's official records.</footer><script>window.onload=()=>window.print()<\/script></body></html>`;
      ...
      setHistory((current) => [{ id, title: dataset.title, format: format === "excel" ? "CSV / Excel" : format === "print" ? "Print / PDF" : format.toUpperCase(), at: new Date().toLocaleString("en-GH") }, ...current].slice(0, 12));
      setNotice(`${dataset.title} · ${format === "print" ? "print/PDF" : format === "excel" ? "CSV for Excel" : format.toUpperCase()} prepared successfully.`);
    } catch (error) {
  ...
  return <div className="space-y-5">
    ...
    {notice && <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-[11px] font-semibold text-emerald-900" role="status" aria-live="polite">{notice}</div>}
    <div className="grid gap-5 lg:grid-cols-2"><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_30px_rgba(15,23,42,.05)]"><h2 className="text-sm font-black text-slate-900">Bulk document packs</h2><p className="mt-1 text-[11px] leading-5 text-slate-500">Scheduled report packs, class registers, fee statements, payroll packs and archive bundles can be generated here as the corresponding operational modules expose them.</p></section><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_30px_rgba(15,23,42,.05)]"><div className="flex items-center justify-between"><div><h2 className="text-sm font-black text-slate-900">This session's export history</h2><p className="mt-1 text-[11px] text-slate-500">Recent files prepared from this browser session.</p></div><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[9px] font-black text-slate-600">{history.length}</span></div>{history.length ? <div className="mt-4 divide-y divide-slate-200">{history.map((item) => <div key={`${item.id}-${item.at}`} className="flex items-center justify-between gap-4 py-3"><div><strong className="block text-[10px] font-black text-slate-900">{item.title}</strong><span className="text-[9px] text-slate-500">{item.format} · {item.at}</span></div><span className="rounded-full bg-emerald-50 px-2 py-1 text-[8px] font-black uppercase text-emerald-700">Prepared</span></div>)}</div> : <div className="mt-4 rounded-xl border border-dashed border-slate-300 py-8 text-center text-[10px] text-slate-400">No exports prepared in this session.</div>}</section></div>
  </div>;
}
