...
export default function TeacherTeachingWorkspaceV2({ assignments, terms, timetable, initialTab = "gradebook" }: Props) {
  ...
  async function createAssessment() {
    ...
    try { await api("/api/mvp/gradebook", { method: "POST", body: JSON.stringify({ action: "assessment", termId, classId: selectedAssignment.classId, subjectId: selectedAssignment.subjectId, name: assessment.name, type: assessment.type, weight: Number(assessment.weight), maxScore: Number(assessment.maxScore) }) }); setAssessment((v) => ({ ...v, name: "" })); setAssessmentOpen(false); setMessage("Assessment created."); await loadGradebook(); }
  ...
  return <section className="teacher-real-workspace">
    ...
    {tab === "gradebook" && <div className="teacher-real-grid"><article className="teacher-real-card"><div className="teacher-real-card-head"><div><span>MARK ENTRY</span><h3>{selectedAssignment ? classLabel(selectedAssignment) : "No teaching assignment"}</h3></div><div style={{display:"flex",gap:8}}><button type="button" onClick={() => setAssessmentOpen(true)}>+ New assessment</button><button type="button" onClick={() => void loadGradebook()} disabled={loading}>{loading ? "Loading…" : "Refresh"}</button></div></div>{!gradebook.assessments.length ? <div className="teacher-real-empty"><strong>No assessments yet.</strong></div> : <div className="teacher-score-table-wrap"><table><thead><tr><th>Learner</th>{gradebook.assessments.map((a) => <th key={a.id}>{a.name}<small>{a.type} · /{a.maxScore}</small></th>)}</tr></thead><tbody>{gradebook.students.map((student) => <tr key={student.id}><td><strong>{student.name}</strong><small>{student.admissionNo}</small></td>{gradebook.assessments.map((a) => <td key={a.id}><input defaultValue={scoreMap.get(`${student.id}:${a.id}`) ?? ""} type="number" min="0" max={Number(a.maxScore)} step="0.01" aria-label={`${a.name} mark for ${student.name}`} onBlur={(e) => void saveScore(student.id, a.id, e.currentTarget.value, Number(a.maxScore))} /></td>)}</tr>)}</tbody></table></div>}</article></div>}
    <Dialog open={tab === "gradebook" && assessmentOpen} onClose={() => setAssessmentOpen(false)} title="New assessment" description={selectedAssignment ? `For ${classLabel(selectedAssignment)}` : undefined} size="sm">
      <div className="teacher-real-form"><label>Name<input value={assessment.name} onChange={(e) => setAssessment({ ...assessment, name: e.target.value })} placeholder="e.g. Midterm Test" autoFocus /></label><label>Type<input value={assessment.type} onChange={(e) => setAssessment({ ...assessment, type: e.target.value })} /></label><div className="teacher-real-form-row"><label>Weight %<input value={assessment.weight} onChange={(e) => setAssessment({ ...assessment, weight: e.target.value })} type="number" min="0.01" max="100" step="0.01" /></label><label>Max score<input value={assessment.maxScore} onChange={(e) => setAssessment({ ...assessment, maxScore: e.target.value })} type="number" min="1" step="0.01" /></label></div><div style={{display:"flex",justifyContent:"flex-end",gap:8}}><button type="button" onClick={() => setAssessmentOpen(false)}>Cancel</button><button type="button" onClick={() => void createAssessment()}>Create assessment</button></div></div>
    </Dialog>
    {tab === "homework" && <div className="teacher-real-grid"><article className="teacher-real-card"><div className="teacher-real-card-head"><div><span>MY HOMEWORK</span><h3>Recent work</h3></div><div style={{display:"flex",gap:8}}><button type="button" onClick={() => setHomeworkOpen(true)}>+ New homework</button><button type="button" onClick={() => void loadHomework()}>Refresh</button></div></div>{homework.length ? <div className="teacher-homework-list">{homework.map((item) => <div key={item.id}><div><strong>{item.title}</strong><small>{item.className} · {item.subjectName}{item.termName ? ` · ${item.termName}` : ""}</small></div><span>{item.assignmentStatus}</span></div>)}</div> : <div className="teacher-real-empty"><strong>No homework yet.</strong></div>}</article></div>}
    <Dialog open={tab === "homework" && homeworkOpen} onClose={() => setHomeworkOpen(false)} title="New homework" description={selectedAssignment ? `For ${classLabel(selectedAssignment)}` : undefined} size="md">
      <div className="teacher-real-form"><label>Title<input value={homeworkForm.title} onChange={(e) => setHomeworkForm({ ...homeworkForm, title: e.target.value })} autoFocus /></label><label>Instructions<textarea value={homeworkForm.instructions} onChange={(e) => setHomeworkForm({ ...homeworkForm, instructions: e.target.value })} rows={6} /></label><div className="teacher-real-form-row"><label>Due date<input type="date" value={homeworkForm.dueDate} onChange={(e) => setHomeworkForm({ ...homeworkForm, dueDate: e.target.value })} /></label><label>Points<input type="number" min="0" step="0.01" value={homeworkForm.points} onChange={(e) => setHomeworkForm({ ...homeworkForm, points: e.target.value })} /></label></div><div style={{display:"flex",justifyContent:"flex-end",gap:8}}><button type="button" onClick={() => setHomeworkOpen(false)}>Cancel</button><button type="button" onClick={() => void createHomework()}>Save homework</button></div></div>
    </Dialog>
