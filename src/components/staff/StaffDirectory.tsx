...
export function StaffDirectory({ people }: { people: StaffRow[] }) {
  ...
  const rows = useMemo(() => {
    ...
    return people.filter((person) => {
      ...
      const haystack = `${person.name} ${person.email ?? ""} ${person.phone ?? ""} ${person.staffNumber ?? ""} ${person.gender} ${person.department ?? ""} ${person.jobTitle ?? ""} ${person.roles.join(" ")}`.toLowerCase();
  ...
  return <div className="staff-simple-directory">
    <div className="staff-simple-toolbar">
      <label><span>Search staff</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Name, staff no., contact, department or role" /></label>
    ...
    {rows.length ? <div className="staff-simple-list">
      {rows.map((person) => {
        ...
        const identity = [person.jobTitle ?? person.roles[0] ?? "Role not assigned", person.staffNumber, person.gender].filter(Boolean).join(" · ");
        return <div className="staff-simple-row" key={person.id}>
          ...
          <span className="staff-simple-person"><strong><Link href={`/school/staff/${encodeURIComponent(person.id)}`}>{person.name}</Link></strong><small>{identity}</small><small>{person.email ?? person.phone ?? "No sign-in contact"}</small></span>
          <span className="staff-simple-scope">{person.department ? `${person.department}${teaching[0] ? ` · ${teaching[0]}` : ""}` : teaching[0] ?? "No department / teaching assignment"}{teaching.length > 1 ? ` +${teaching.length - 1}` : ""}</span>
          <span className={`staff-simple-status is-${person.status}`}>{person.status === "pending" ? "Needs login" : person.status}</span>
          <Link className="staff-profile-link" href={`/school/staff/${encodeURIComponent(person.id)}/personnel`}>Personnel <ArrowRight size={14}/></Link>
        </div>;
      })}
