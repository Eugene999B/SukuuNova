...
type LiveGame = "math" | "keyboard-ninja" | "force-motion-lab" | "word" | "comprehension-quest";
...
const gameLabels: Record<LiveGame, string> = { math: "Nova Runner", "keyboard-ninja": "TurboType", "force-motion-lab": "AstroLab Defender", word: "Word Kingdom", "comprehension-quest": "Reading Quest" };
...
export default function LearningArcadeV3() {
  ...
  const finishRound = (answers: string[], typingTelemetry?: TurboTypeTelemetry) => run(async () => {
    ...
    setMessage(`${completedGame === "keyboard-ninja" ? "Race" : completedGame === "force-motion-lab" ? "Defence mission" : completedGame === "word" ? "Quest" : completedGame === "comprehension-quest" ? "Expedition" : "Mission"} complete — ${completed.xp} XP earned.`);
  ...
  const highestGameLevel = Math.max(runnerProgress?.level ?? 1, typingProgress?.level ?? 1, astroProgress?.level ?? 1, wordProgress?.level ?? 1, readingProgress?.level ?? 1);
  ...
  if (result) {
    ...
    const unit = typingResult ? "typing checkpoints" : astroResult ? "science anomalies" : wordResult ? "kingdom runes" : readingResult ? "evidence chapters" : "Knowledge Gates";
    const kicker = typingResult ? "TURBOTYPE · RACE COMPLETE" : astroResult ? "ASTROLAB DEFENDER · LAB SECURED" : wordResult ? "WORD KINGDOM · CROWN SECURED" : readingResult ? "READING QUEST · MYSTERY MAPPED" : "NOVA RUNNER · MISSION COMPLETE";
    const replayLabel = typingResult ? "race" : astroResult ? "defence mission" : wordResult ? "quest" : readingResult ? "expedition" : "mission";
    return <div className="nova-finish">
      <section className="nova-finish-card">
        <div className="nova-finish-top"><div className="nova-finish-mark"><Trophy size={34}/></div><span className="nova-arcade-kicker">{kicker}</span><h1>{result.stars === 3 ? (typingResult ? "Perfect precision!" : astroResult ? "Lab fully stabilised!" : wordResult ? "Crown restored!" : readingResult ? "Mystery solved!" : "Legendary run!") : result.stars === 2 ? "Strong mission!" : "World cleared!"}</h1><p>{result.correct}/{result.roundLength} {unit} cleared correctly. The Adaptive Director will use this performance for the next game.</p><div className="nova-rewards"><div><strong>{result.stars}/3</strong><span>Stars</span></div><div><strong>+{result.xp}</strong><span>XP</span></div><div><strong>{result.score ?? 0}</strong><span>Score</span></div></div></div>
