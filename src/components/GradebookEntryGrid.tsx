...
export default function GradebookEntryGrid({ assessments, rows, rules }: { assessments: Assessment[]; rows: Row[]; rules: PreviewRules }) {
  ...
  return <div>
    {(error || saved) && <div role="status" style={{ marginBottom: 12, padding: "10px 12px", borderRadius: 10, background: error ? "var(--color-bg)" : "var(--color-bg)", color: error ? "var(--color-danger)" : "var(--color-text-subtle)" }}>{error || "Saved."}</div>}
    <div style={{ overflowX: "auto" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 980 }}>
        <thead><tr><th style={{ textAlign: "left", padding: 10, position: "sticky", left: 0, background: "var(--color-text-primary)" }}>Student</th>{assessments.map((assessment) => <th key={assessment.id} style={{ textAlign: "left", padding: 10 }}>{assessment.name}<small style={{ display: "block", opacity: .65 }}>{assessment.type} · max {assessment.maxScore}</small></th>)}<th style={{ padding: 10 }}>Weighted</th></tr></thead>
        <tbody>{rows.map((row) => <tr key={row.student.id}>
          <td style={{ padding: 10, borderTop: "1px solid var(--color-bg)", position: "sticky", left: 0, background: "var(--color-text-primary)" }}><b>{row.student.name}</b><small style={{ display: "block", opacity: .65 }}>{row.student.admissionNo}</small></td>
          {assessments.map((assessment) => { const key = `${row.student.id}:${assessment.id}`; const existing = values[key] ?? ""; const status = statuses[key] ?? "present"; return <td key={assessment.id} style={{ padding: 8, borderTop: "1px solid var(--color-bg)" }}><div style={{ display: "flex", gap: 6, alignItems: "center" }}><input aria-label={`${assessment.name} mark for ${row.student.name}`} inputMode="decimal" value={existing} onChange={(event) => { const next = event.target.value; setValues((current) => ({ ...current, [key]: next })); setTotals((current) => ({ ...current, [row.student.id]: calculateOptimisticTotal(row.student.id, next, assessment.id) })); setSaved(""); }} onBlur={() => void save(row.student.id, assessment.id, assessment.maxScore, key)} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} style={{ width: 62 }} placeholder="—"/><span style={{ opacity: .5 }}>/{assessment.maxScore}</span><select aria-label={`Attendance status for ${assessment.name}, ${row.student.name}`} value={status} onChange={(event) => { const next = event.target.value as "present" | "absent" | "excused"; setStatuses((current) => ({ ...current, [key]: next })); setTotals((current) => ({ ...current, [row.student.id]: calculateOptimisticTotal(row.student.id, existing, assessment.id, next) })); setSaved(""); }} style={{ maxWidth: 92 }}><option value="present">Present</option><option value="absent">Absent</option><option value="excused">Excused</option></select></div><div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 4 }}>{saving === key && <small role="status" style={{ color: "var(--color-danger)" }}>Saving…</small>}{saved === key && <small style={{ color: "var(--sn-ink)" }}>Saved</small>}{existing !== "" && <button type="button" onClick={() => void clear(row.student.id, assessment.id, key)} style={{ background: "none", border: "none", color: "var(--color-danger)", cursor: "pointer", fontSize: 11 }}>Clear</button>}</div></td>; })}
          <td style={{ padding: 10, borderTop: "1px solid var(--color-bg)" }}>{totals[row.student.id] == null ? <span style={{ color: "var(--color-danger)" }}>Incomplete</span> : `${totals[row.student.id]!.toFixed(2)}%`}</td>
        </tr>)}</tbody>
      </table>
    </div>
    <p style={{ margin: "12px 0 0", color: "var(--color-text-muted)", fontSize: 12 }}>Tip: enter a mark and move to the next cell. SukuuNova updates the weighted result immediately, then confirms the server save.</p>
  </div>;
}
