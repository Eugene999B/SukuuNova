...
export default function GradebookEntryGrid({ assessments, rows, rules }: Props) {
  ...
  return <div className="gradebook-entry">
    ...
    <div className="gradebook-table-wrap">
      <table className="gradebook-table" aria-label="Gradebook mark entry"><thead><tr><th>Student</th>{assessments.map((assessment) => <th key={assessment.id}><div className="gradebook-assessment-head"><strong>{assessment.name}</strong><small>{assessment.type} · max {assessment.maxScore} · {assessment.weight}%</small></div></th>)}<th>Weighted result</th></tr></thead>
        <tbody>{rows.map((row) => <tr key={row.student.id}>
          ...
          {assessments.map((assessment) => { const key = `${row.student.id}:${assessment.id}`; const existing = values[key] ?? ""; const status = statuses[key] ?? "present"; return <td key={assessment.id}><div className="gradebook-input-row"><input className="gradebook-input" aria-label={`${assessment.name} mark for ${row.student.name}`} inputMode="decimal" value={existing} onChange={(event) => { const next = event.target.value; setValues((current) => ({ ...current, [key]: next })); setTotals((current) => ({ ...current, [row.student.id]: calculateOptimisticTotal(row.student.id, next, assessment.id) })); setSaved(""); }} onBlur={() => void save(row.student.id, assessment.id, assessment.maxScore, key)} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} placeholder="—"/><span className="gradebook-score-max">/{assessment.maxScore}</span><select className="gradebook-status-select" aria-label={`Assessment status for ${assessment.name}, ${row.student.name}`} value={status} onChange={(event) => { const next = event.target.value as ScoreStatus; setStatuses((current) => ({ ...current, [key]: next })); setTotals((current) => ({ ...current, [row.student.id]: calculateOptimisticTotal(row.student.id, existing, assessment.id, next) })); setSaved(""); }}><option value="present">Present</option><option value="absent">Absent</option><option value="excused">Excused</option></select></div><div className="gradebook-save-state">{saving === key && <span className="saving" role="status">Saving…</span>}{saved === key && <span className="ok">Saved</span>}{existing !== "" && <button type="button" onClick={() => void clear(row.student.id, assessment.id, key)}>Clear</button>}</div></td>; })}
    ...
    <p className="gradebook-tip">Enter a mark, then move to the next cell. SukuuNova previews the weighted result immediately and confirms the server save.</p>
  </div>;
}
