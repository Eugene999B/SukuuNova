...
const THEMES: Array<{ test: RegExp; value: Theme }> = [
  { test: /numeracy|mathematics/i, value: { key: "math", label: "Nova Number City", objects: ["7", "+", "×", "½", "%", "₵", "π", "=", "△"], Icon: Rocket } },
  { test: /literacy|english|language/i, value: { key: "words", label: "Word Forest", objects: ["A", "B", "?", "!", "Aa", "read", "word", "story", "ABC"], Icon: BookOpenText } },
  { test: /science|chemistry|physics/i, value: { key: "science", label: "Discovery Lab", objects: ["H₂O", "Fe", "⚡", "☁", "DNA", "O₂", "★", "F", "m/s"], Icon: Atom } },
  { test: /social|geography|history|civic/i, value: { key: "world", label: "Explorer World", objects: ["GH", "AF", "⚑", "N", "S", "◎", "⌛", "⚖", "◆"], Icon: MapPinned } },
  { test: /ict|computing/i, value: { key: "code", label: "Digital Grid", objects: ["01", "⌨", "</>", "{ }", "CPU", "RAM", "#", "_", "↵"], Icon: Keyboard } },
  { test: /logic|reasoning|memory/i, value: { key: "logic", label: "Puzzle Orbit", objects: ["◇", "#", "↗", "?", "○", "▦", "1→2", "…", "✓"], Icon: BrainCircuit } },
  { test: /life|health|safety|business|financial/i, value: { key: "life", label: "Life Skills Town", objects: ["₵", "♥", "⚠", "↗", "✓", "BUS", "SAVE", "PLAN", "GO"], Icon: HeartPulse } },
];

const AGE_COPY: Record<AgeBand, { label: string; pace: string; scale: number; density: number }> = {
  age_4_5: { label: "Little explorer mode", pace: "gentle", scale: 1.18, density: 5 },
  age_6_8: { label: "Explorer mode", pace: "steady", scale: 1.1, density: 6 },
  age_9_11: { label: "Adventure mode", pace: "active", scale: 1, density: 7 },
  age_12_14: { label: "Challenge mode", pace: "focused", scale: .96, density: 8 },
  age_15_18: { label: "Mastery mode", pace: "precision", scale: .92, density: 9 },
};

function themeFor(category: string, subject: string) {
  const haystack = `${category} ${subject}`;
  return THEMES.find((theme) => theme.test.test(haystack))?.value ?? { key: "nova", label: "Nova Universe", objects: ["✦", "★", "✓", "?", "+", "→", "∞", "N", "S"], Icon: Gamepad2 };
}
...
export default function ArcadeMotionWorld({ gameKey, gameName, category, subject, ageBand, questionIndex, questionCount, answeredCount, hasAnswer }: Props) {
  const theme = themeFor(category, subject);
  const age = AGE_COPY[ageBand ?? "age_6_8"];
  ...
  const objects = theme.objects.slice(0, age.density);
  const Icon = theme.Icon;

  return <section
    className={`arcade-motion-world is-${theme.key} age-${ageBand ?? "age_6_8"} ${hasAnswer ? "has-answer" : ""}`}
    style={{ "--motion-scale": age.scale, "--runner-x": `${progress}%` } as CSSProperties}
  ...
  >
    <div className="arcade-motion-sky" aria-hidden="true">
      ...
      >{object}</span>)}
      <div className="arcade-motion-beacon"><Sparkles size={16}/><span>Next</span></div>
    </div>

    <div className="arcade-motion-meta">
      <div className="arcade-motion-world-name"><span><Icon size={18}/></span><div><strong>{theme.label}</strong><small>{age.label} · {age.pace} motion</small></div></div>
    ...
    <div className="arcade-motion-foot">
      <span>World {questionIndex + 1}/{Math.max(1, questionCount)}</span>
      <span>{hasAnswer ? "Input locked into the mission path" : "Solve the task to move the world forward"}</span>
    </div>

    <style>{`
      .arcade-motion-world{position:relative;overflow:hidden;min-height:244px;border:1px solid var(--sn-line);border-radius:22px;background:linear-gradient(155deg,var(--sn-guardian-tint),var(--sn-surface) 52%,var(--sn-surface-2));box-shadow:var(--sn-shadow-sm);isolation:isolate}
      ...
      .arcade-motion-checkpoint{position:absolute;top:23px;display:grid;place-items:center;width:23px;height:23px;border-radius:50%;background:var(--sn-surface);border:1px solid var(--sn-line);font-size:9px;color:var(--sn-muted)}.arcade-motion-checkpoint.start{left:2px}.arcade-motion-checkpoint.finish{right:2px;color:var(--sn-guardian-accent)}
      .arcade-motion-foot{position:absolute;z-index:4;left:18px;right:18px;bottom:12px;display:flex;justify-content:space-between;gap:12px;color:var(--sn-muted);font-size:8.5px;font-weight:750}
      .is-science .arcade-motion-object{border-radius:50%}.is-code .arcade-motion-object{border-radius:8px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}.is-world .arcade-motion-sky:after{content:"";position:absolute;left:15%;right:15%;bottom:74px;height:42px;border:1px dashed var(--sn-line);border-radius:50%;transform:skewX(-18deg)}.is-life .arcade-motion-runner>span{border-radius:50%}
      @keyframes arcade-world-spin{to{transform:rotate(360deg)}}@keyframes arcade-object-flight{0%{transform:translateX(0) translateY(0) rotate(-5deg) scale(var(--motion-scale));opacity:0}8%{opacity:.9}48%{transform:translateX(calc(50vw + 120px)) translateY(-12px) rotate(5deg) scale(var(--motion-scale));opacity:.95}92%{opacity:.9}100%{transform:translateX(calc(100vw + 180px)) translateY(8px) rotate(-4deg) scale(var(--motion-scale));opacity:0}}@keyframes arcade-runner-float{0%,100%{transform:translateY(0) rotate(-4deg)}50%{transform:translateY(-5px) rotate(2deg)}}@keyframes arcade-shadow{0%,100%{transform:scaleX(.9);opacity:.55}50%{transform:scaleX(.68);opacity:.28}}@keyframes arcade-runner-boost{0%{transform:translateX(-8px) scale(.9) rotate(-8deg)}55%{transform:translateX(8px) scale(1.12) rotate(5deg)}100%{transform:translateX(0) scale(1)}}@keyframes arcade-beacon{0%,100%{transform:translateY(0);opacity:.75}50%{transform:translateY(-5px);opacity:1}}
      @media(max-width:650px){.arcade-motion-world{min-height:226px}.arcade-motion-meta{padding:14px}.arcade-motion-readout{display:none}.arcade-motion-foot{left:14px;right:14px;display:block}.arcade-motion-foot span:last-child{display:none}.arcade-motion-track{left:14px;right:14px}.arcade-motion-object{transform:scale(calc(var(--motion-scale)*.9))}}
