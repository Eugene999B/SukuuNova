...
export default function LearningArcadeV2() {
  ...
  const missions = useMemo(() => [...eligible].sort((a, b) => {
    const ap = data?.progress.find((progress) => progress.game === a.gameKey);
    const bp = data?.progress.find((progress) => progress.game === b.gameKey);
    return (ap?.rounds ?? 0) - (bp?.rounds ?? 0);
  }).slice(0, 3), [eligible, data?.progress]);
  ...
  if (round) return <div className="nova3-stage">
    <header className="nova3-stage-top">
      <button type="button" onClick={back} disabled={busy}><ArrowLeft size={15}/>{complete ? "Back to Playroom" : "Save & leave"}</button>
      <div><span>{roundGame?.name ?? round.game}</span><strong>{data?.selected?.name ?? "Learner"} · Level {round.difficulty} · {round.ageBand ? ageLabels[round.ageBand] : "Adaptive"}</strong></div>
      <div className="nova3-stage-progress"><span>{complete ? round.questions.length : answered}/{round.questions.length}</span><i><b style={{ width: `${round.questions.length ? Math.round((complete ? round.questions.length : answered) / round.questions.length * 100) : 0}%` }}/></i></div>
    </header>
    {error ? <div className="nova3-alert bad" role="alert">{error}</div> : null}
    {complete ? <main className="nova3-finish">
      <div className="nova3-confetti" aria-hidden="true"><Star/><Sparkles/><Trophy/><Star/><Sparkles/><Award/></div>
      <span className="nova3-kicker">MISSION COMPLETE</span>
      <h1 ref={heading} tabIndex={-1}>{round.stars === 3 ? "Brilliant run!" : round.stars === 2 ? "Strong work!" : "Mission cleared!"}</h1>
      <p>{round.correct}/{round.roundLength} correct. Arcade practice builds mastery but never changes official school grades.</p>
      <div className="nova3-reward-row"><div><Trophy size={22}/><strong>{round.stars}/3</strong><span>Stars</span></div><div><Zap size={22}/><strong>+{round.xp}</strong><span>XP</span></div><div><Coins size={22}/><strong>+{Math.floor(round.xp / 10) + round.stars * 2}</strong><span>Nova Coins</span></div></div>
      <button className="nova3-primary" type="button" onClick={back}>Return to Playroom <ArrowRight size={15}/></button>
      ...
      <div className="nova3-task-head"><span className="nova3-kicker">TASK {index + 1} OF {round.questions.length} · {engineLabel(round.engine)}</span><h1 ref={heading} tabIndex={-1}>{current.prompt}</h1><p>{roundGame?.description}</p></div>
      <div className="nova3-control">{renderQuestion(current)}</div>
  ...
  return <div className="nova3-playroom">
    ...
    {loading && !data ? <div className="nova3-loading"><Gamepad2 size={30}/><strong>Building your learning universe…</strong></div> : !data?.selected ? <div className="nova3-loading"><Gamepad2 size={30}/><strong>No linked learner is available for play yet.</strong></div> : <>
      ...
      <section className="nova3-mission-zone"><div className="nova3-section-head"><div><span className="nova3-kicker">TODAY’S MISSION DECK</span><h2>Three worlds chosen for this learner</h2><p>Recommendations favour suitable games with less completed practice so learning stays broad instead of repeating one activity forever.</p></div><span className="nova3-age-chip">{ageBand ? ageLabels[ageBand] : "Adaptive"}</span></div><div className="nova3-missions">{missions.map((game, missionIndex) => <MissionCard key={game.gameKey} game={game} progress={data.progress.find((item) => item.game === game.gameKey)} index={missionIndex} busy={busy} onPlay={() => void open({ action: "start", studentId: data.selected!.id, game: game.gameKey, ageBand, easier, roundLength: game.defaultRoundLength })}/>)}</div></section>
...
function GameLogo({ game }: { game: Game }) {
  return <div className="nova3-logo" title={`${game.name} game logo`}><span>{game.symbol}</span><b>{gameInitials(game.name)}</b><i>{game.engine.split("_")[0].slice(0, 3).toUpperCase()}</i></div>;
}

function Reward({ icon, name, threshold, xp }: { icon: React.ReactNode; name: string; threshold: number; xp: number }) {
  const unlocked = xp >= threshold;
  return <div className={unlocked ? "unlocked" : "locked"}><i>{icon}</i><strong>{name}</strong><span>{unlocked ? "Unlocked" : `${Math.max(0, threshold - xp)} XP to unlock`}</span></div>;
}
