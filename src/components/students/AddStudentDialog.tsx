...
export function AddStudentDialog({ classes, action, triggerLabel = "+ Add student" }: Props) {
  ...
  return (
    <>
      <button type="button" className="button primary" onClick={() => { setStep(0); setOpen(true); }}>{triggerLabel}</button>
      {open ? (
        <div className="student-dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
          <section className="student-dialog" role="dialog" aria-modal="true" aria-labelledby="add-student-title">
            <header className="student-dialog-header">
              ...
              <button type="button" className="dialog-close" onClick={() => setOpen(false)} aria-label="Close">×</button>
            </header>
            ...
            <form action={action} className="student-dialog-form">
              ...
              <footer className="student-dialog-footer">
                ...
                <div className="dialog-footer-actions">
                  <button type="button" className="button secondary" onClick={() => step === 0 ? setOpen(false) : setStep((value) => value - 1)}>{step === 0 ? "Cancel" : "Back"}</button>
