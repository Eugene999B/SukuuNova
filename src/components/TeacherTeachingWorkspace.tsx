...
type Assessment = { id: string; name: string; type: string; maxScore: number; weight: number; class: { name: string }; subject: { name: string }; term: { name: string } };
type Score = { studentId: string; assessmentId: string; value: number };
type Homework = { id: string; classId: string; className: string; subjectId: string; subjectName: string; termName: string | null; title: string; instructions: string; dueDate: string; points: number | null; assignmentStatus: string; reviewStatus: string };
...
export default function TeacherTeachingWorkspace({ assignments, terms, timetable }: { assignments: Assignment[]; terms: Term[]; timetable: TimetableSlot[] }) {
  const [tab, setTab] = useState<"gradebook" | "homework" | "timetable">("gradebook");
  ...
  async function loadHomework() {
    try {
      const data = await api("/api/school/homework");
      setHomework(data.rows || []);
  ...
  async function createAssessment() {
    ...
    try {
      await api("/api/mvp/gradebook", { method: "POST", body: JSON.stringify({ action: "assessment", termId, classId: selectedAssignment.classId, subjectId: selectedAssignment.subjectId, name: assessment.name, type: assessment.type, weight: Number(assessment.weight), maxScore: Number(assessment.maxScore) }) });
      setAssessment((v) => ({ ...v, name: "" })); setMessage("Assessment created."); await loadGradebook();
  ...
  return <section className="teacher-real-workspace">
    <div className="teacher-real-tabs" role="tablist" aria-label="Teaching workspace">
      <button type="button" className={tab === "gradebook" ? "active" : ""} onClick={() => setTab("gradebook")}>My Gradebook</button>
      <button type="button" className={tab === "homework" ? "active" : ""} onClick={() => setTab("homework")}>My Homework</button>
      <button type="button" className={tab === "timetable" ? "active" : ""} onClick={() => setTab("timetable")}>My Timetable</button>
    </div>

    {(error || message) && <div className={`teacher-real-message ${error ? "error" : "success"}`}>{error || message}</div>}

    {tab !== "timetable" && <div className="teacher-real-filters">
      <label>Teaching assignment<select value={assignmentIndex} onChange={(e) => setAssignmentIndex(Number(e.target.value))}>{assignments.length ? assignments.map((a, i) => <option key={`${a.classId}-${a.subjectId}`} value={i}>{classLabel(a)}</option>) : <option value={0}>No assignments</option>}</select></label>
      <label>Academic term<select value={termId} onChange={(e) => setTermId(e.target.value)}>{terms.map((t) => <option key={t.id} value={t.id}>{t.name}{t.academicYear ? ` · ${t.academicYear.name}` : ""}</option>)}</select></label>
    </div>}

    {tab === "gradebook" && <div className="teacher-real-grid">
      <article className="teacher-real-card">
        <div className="teacher-real-card-head"><div><span>MARK ENTRY</span><h3>{selectedAssignment ? classLabel(selectedAssignment) : "No teaching assignment"}</h3></div><button type="button" onClick={() => void loadGradebook()} disabled={loading}>{loading ? "Loading…" : "Refresh"}</button></div>
        {!gradebook.assessments.length ? <div className="teacher-real-empty"><strong>No assessments yet.</strong></div> : <div className="teacher-score-table-wrap"><table><thead><tr><th>Learner</th>{gradebook.assessments.map((a) => <th key={a.id}>{a.name}<small>{a.type} · /{a.maxScore}</small></th>)}</tr></thead><tbody>{gradebook.students.map((student) => <tr key={student.id}><td><strong>{student.name}</strong><small>{student.admissionNo}</small></td>{gradebook.assessments.map((a) => <td key={a.id}><input defaultValue={scoreMap.get(`${student.id}:${a.id}`) ?? ""} type="number" min="0" max={a.maxScore} step="0.01" aria-label={`${a.name} mark for ${student.name}`} onBlur={(e) => void saveScore(student.id, a.id, e.currentTarget.value, a.maxScore)} /></td>)}</tr>)}</tbody></table></div>}
      </article>
    ...
    {tab === "homework" && <div className="teacher-real-grid">
      ...
      <article className="teacher-real-card"><div className="teacher-real-card-head"><div><span>MY HOMEWORK</span><h3>Recent work</h3><p>Your own homework items, newest changes first.</p></div><button type="button" onClick={() => void loadHomework()}>Refresh</button></div>{homework.length ? <div className="teacher-homework-list">{homework.map((item) => <div key={item.id}><div><strong>{item.title}</strong><small>{item.className} · {item.subjectName}{item.termName ? ` · ${item.termName}` : ""}</small></div><span>{item.assignmentStatus}</span></div>)}</div> : <div className="teacher-real-empty"><strong>No homework yet.</strong></div>}</article>
    </div>}

    {tab === "timetable" && <article className="teacher-real-card"><span>WEEKLY SCHEDULE</span><h3>My timetable</h3><div className="teacher-timetable-grid">{days.map((day, index) => <div key={day}><strong>{day}</strong>{timetable.filter((slot) => slot.dayOfWeek === index + 1).map((slot) => <div key={slot.id}><b>P{slot.period}</b><span>{slot.subject.name}</span><small>{slot.class.level ? `${slot.class.level} · ` : ""}{slot.class.name}</small></div>)}{!timetable.some((slot) => slot.dayOfWeek === index + 1) ? <em>No class</em> : null}</div>)}</div></article>}
  </section>;
}
