...
function latestApprovedPickup(child: ChildTransport, direction: PickupDirection | null) {
  if (!direction) return child.pickups.find((pickup) => pickup.status === "approved") ?? null;
  return child.pickups.find((pickup) => pickup.status === "approved" && pickup.direction === direction) ?? null;
}
...
function TransportRouteMap({ shape, bus, pickup }: { shape: RoutePoint[]; bus: LatestLocation | null; pickup: Pickup | null }) {
  ...
  return <div className="guardian-transport-map-shell">
    <svg viewBox={`0 0 ${geometry.width} ${geometry.height}`} role="img" aria-label="Live school transport route showing the bus and approved pickup point">
    ...
    <div className="guardian-transport-map-legend"><span><i className="is-bus"/>Live bus</span><span><i className="is-pickup"/>Approved pickup</span><span><i className="is-route"/>School route shape</span></div>
  </div>;
}
...
export function GuardianTransportWorkspace({ initialData }: { initialData: GuardianTransportData }) {
  ...
  if (!child) return <div className="guardian-transport-empty"><BusFront size={34}/><h2>No transport-linked learner yet</h2><p>When the school assigns transport to a linked child, the route, pickup approval and live-trip tools will appear here.</p></div>;
  ...
  const etaText = progress?.etaMinutes != null
    ? `${Math.max(0, Math.round(progress.etaMinutes))} min${progress.etaConfidenceMinutes != null ? ` ± ${Math.max(1, Math.round(progress.etaConfidenceMinutes))}` : ""}`
    : "Calculating";
  const pending = child.pickups.filter((point) => point.status === "pending");
  ...
  const useCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setMessage("Current location captured. Submit it for school approval when ready.");
      },
      () => setMessage("Your location could not be captured. Check browser location permission or enter coordinates manually."),
  ...
  const requestPickup = async () => {
    ...
    if (!Number.isFinite(lat) || !Number.isFinite(lng)) { setMessage("Enter a valid latitude and longitude or use Current location."); return; }
    ...
    try {
      const response = await fetch("/api/guardian/transport", {
        ...
        body: JSON.stringify({
          ...
          effectiveTo: temporary && effectiveTo ? new Date(effectiveTo).toISOString() : undefined,
        }),
      });
      const body = await response.json();
      if (!response.ok) throw new Error(body?.error || body?.message || "Pickup request could not be submitted.");
      setLatitude(""); setLongitude(""); setLabel(""); setTemporary(false); setEffectiveTo("");
      setMessage("Pickup request submitted. The school must approve it before it becomes active.");
      await load(true);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Pickup request could not be submitted.");
    } finally { setSaving(false); }
  };

  return <div className="guardian-transport-workspace">
    <section className="guardian-transport-hero">
      <div><span>Family transport intelligence</span><h2>Know where the school bus is—and when it is reaching your child.</h2><p>Live GPS is filtered through NovaCore before it reaches this page. Family pickup changes are requests only and require school approval.</p></div>
    ...
    <section className="guardian-transport-main-grid">
      <div className="guardian-transport-panel guardian-transport-live-panel">
        ...
        <div className="guardian-transport-coordinate-strip">
          ...
          <div><span>Approved pickup</span><strong>{pickup ? `${Number(pickup.latitude).toFixed(5)}, ${Number(pickup.longitude).toFixed(5)}` : "Not approved"}</strong></div>
      ...
      <aside className="guardian-transport-panel guardian-transport-pickup-panel">
        <div className="guardian-transport-panel-head"><div><span>Pickup point</span><h3>Request a location</h3><p>A new pin never changes the school bus route. It remains pending until school approval.</p></div></div>
        <div className="guardian-transport-current-pickup"><MapPin size={20}/><div><span>Current approved point</span><strong>{pickup?.label || (pickup ? `${pickup.latitude}, ${pickup.longitude}` : "No approved point")}</strong>{pickup ? <small>{stateLabel(pickup.direction)}{pickup.isTemporary ? " · temporary" : ""}</small> : null}</div></div>
        <div className="guardian-transport-form">
          ...
          <label><span>Location label</span><input value={label} onChange={(event) => setLabel(event.target.value)} placeholder="e.g. Main gate / House junction" maxLength={160}/></label>
          ...
          <label className="guardian-transport-check"><input type="checkbox" checked={temporary} onChange={(event) => setTemporary(event.target.checked)}/><span>This is a temporary pickup point</span></label>
          ...
          <button type="button" className="button primary" onClick={() => void requestPickup()} disabled={saving || !child.assignment}><MapPin size={15}/>{saving ? "Submitting…" : "Submit for school approval"}</button>
          {message ? <p className="guardian-transport-message" role="status">{message}</p> : null}
        </div>
        {pending.length ? <div className="guardian-transport-pending"><strong>{pending.length} request{pending.length === 1 ? "" : "s"} awaiting school review</strong>{pending.slice(0, 3).map((point) => <span key={point.id}>{stateLabel(point.direction)} · {point.label || `${point.latitude}, ${point.longitude}`} · {dateTime(point.requestedAt)}</span>)}</div> : null}
      </aside>
    </section>
