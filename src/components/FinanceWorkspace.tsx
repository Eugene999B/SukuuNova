...
const ActionArrow = () => <ArrowRight aria-hidden="true" size={14} strokeWidth={2.2} />;
...
export default function FinanceWorkspace({ mode = "overview", schoolName = "School" }: { mode?: "overview"|"fees"|"invoices"|"payments"|"arrears"|"reports"|"payroll"; schoolName?: string }) {
  ...
  const recordPayment = (e:FormEvent) => {
    e.preventDefault();
    if (!invoiceId || !paymentAmount) return setNotice("Choose an invoice and enter the payment amount.");
    const ref = paymentReference.trim();
    if (!ref) return setNotice("A receipt reference or transaction ID is required to prevent duplicate payments.");
    post("/api/mvp/finance", { action:"payment", invoiceId, amount:Number(paymentAmount), method:paymentMethod, reference:ref });
  };
  ...
  return <div className="finance-app">
    <section className="finance-hero"><div><span className="finance-kicker">{schoolName.toUpperCase()} · FINANCE CONTROL CENTRE</span><h1>{title}</h1><p>{subtitle}</p></div><div className="finance-hero-actions"><Link href="/school/fees">Overview</Link><Link href="/school/fees/payments">Record payment</Link><Link href="/school/fees/arrears">Arrears</Link><Link href="/school/fees/payroll">Payroll</Link></div></section>
    ...
    {(mode === "overview" || mode === "fees") && <section className="finance-grid"><div className="finance-panel"><div className="finance-head"><div><span className="finance-kicker">SETUP</span><h2>Fee structure</h2><p>Create tuition, feeding, transport, examination, activity or any other billable item.</p></div></div><form className="finance-form" onSubmit={createFee}><select value={termId} onChange={e=>setTermId(e.target.value)} aria-label="Academic term"><option value="">Academic term</option>{data.terms.map(t=><option key={t.id} value={t.id}>{t.academicYear?.name ? `${t.academicYear.name} · ` : ""}{t.name}</option>)}</select><select value={classId} onChange={e=>setClassId(e.target.value)} aria-label="Class scope"><option value="">All classes</option>{data.classes.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><input value={feeName} onChange={e=>setFeeName(e.target.value)} placeholder="Fee item · e.g. Tuition" aria-label="Fee item name"/><input type="number" min="0.01" step="0.01" value={feeAmount} onChange={e=>setFeeAmount(e.target.value)} placeholder="Amount (GHS)" aria-label="Fee amount in cedis"/><button disabled={busy}>Create fee item</button></form><div className="finance-chips">{["Tuition","Feeding","Transport","Examination","Activities","Development Levy"].map(x=><span key={x}>{x}</span>)}</div></div><div className="finance-panel finance-accent"><span className="finance-kicker">COLLECTION MODEL</span><h2>Bill <ActionArrow/> collect <ActionArrow/> reconcile</h2><p>Fee items drive invoices. Payments reduce the same invoice balance. Reversals stay visible and never erase the audit trail.</p><div className="finance-flow"><b>1</b><span>Set term/class fees</span><b>2</b><span>Generate student bill</span><b>3</b><span>Record payment + receipt</span><b>4</b><span>Review balance</span></div></div></section>}
