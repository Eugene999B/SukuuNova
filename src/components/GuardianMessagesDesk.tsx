...
type Message = { id:string; title:string; body:string; senderName:string; senderId?:string|null; recipientName?:string|null; direction?:"incoming"|"outgoing"; createdAt:string; readAt?:string|null; attachments?:Array<{name:string;dataUrl:string}> };
type Person = { id:string; name:string; email?:string|null; phone?:string|null; roles?:string[] };
type InboxPayload = { messages?:Message[]; unreadCount?:number; recipients?:Person[]; message?:string; messageId?:string };
...
const counterpart = (message:Message) => message.direction === "outgoing" ? message.recipientName || "School staff" : message.senderName;
...
export default function GuardianMessagesDesk() {
  ...
  const visibleMessages = useMemo(() => {
    ...
    return query ? messages.filter((message) => `${message.title} ${message.senderName} ${message.recipientName || ""} ${message.body}`.toLowerCase().includes(query)) : messages;
  }, [messages, search]);
  ...
  async function read(message: Message) {
    ...
    if (message.direction === "outgoing" || message.readAt) return;
  ...
  async function send(event: React.FormEvent) {
    ...
    try {
      ...
      await load();
      if (payload.messageId) setSelectedMessage(payload.messageId);
    } catch (sendError) {
  ...
  return <div className="gmd-page">
    <section className="gmd-hero">
      <div><span>FAMILY COMMUNICATION HUB</span><h1>One inbox. The right person. No chasing phone numbers.</h1><p>Read school updates, see your sent history and message active school staff through one auditable SukuuNova conversation space.</p></div>
    ...
    <section className="gmd-shell">
      <aside className="gmd-inbox">
        <header><div><span>CONVERSATIONS</span><h2>Inbox & sent</h2></div><button type="button" onClick={() => void load()} disabled={loading} aria-label="Refresh messages"><RefreshCw size={16}/></button></header>
        ...
        <div className="gmd-message-list">{loading && !messages.length ? <div className="gmd-empty">Loading messages…</div> : visibleMessages.length ? visibleMessages.map((message) => {
          const outgoing=message.direction==="outgoing";
          const person=counterpart(message);
          return <button type="button" key={message.id} onClick={() => void read(message)} className={`${selectedMessage === message.id ? "active" : ""} ${outgoing?"sent":""}`.trim()}>
            <span className={`gmd-avatar ${!outgoing&&!message.readAt ? "unread" : ""}`}>{person.slice(0,2).toUpperCase()}</span>
            <span className="gmd-preview"><strong>{outgoing?`To ${person}`:person}</strong><b>{message.title}</b><small>{messageText(message).slice(0,76)}{messageText(message).length > 76 ? "…" : ""}</small></span>
            {outgoing?<span className="gmd-direction">SENT</span>:!message.readAt ? <i aria-label="Unread message" /> : null}
          </button>;
        }) : <div className="gmd-empty"><MessageCircle size={26}/><strong>No messages found</strong><span>School notices, sent questions and replies will appear here.</span></div>}</div>
      </aside>

      <main className="gmd-reader">{active ? <>
        <header><div><span>{active.direction==="outgoing"?"SENT MESSAGE":"MESSAGE"}</span><h2>{active.title}</h2><p>{active.direction==="outgoing"?<>To <strong>{active.recipientName||"School staff"}</strong></>:<>From <strong>{active.senderName}</strong></>} · {new Date(active.createdAt).toLocaleString("en-GH")}</p></div><span className="gmd-status">{active.direction==="outgoing"?"Sent":active.readAt ? "Read" : "New"}</span></header>
...
.gmd-search{margin:14px 0;display:flex;align-items:center;gap:8px;padding:0 11px;border:1px solid var(--sn-line);border-radius:12px;background:var(--sn-surface-2);color:var(--sn-muted)}.gmd-search input{width:100%;height:40px;border:0;outline:0;background:transparent;color:var(--sn-ink)}.gmd-message-list{display:grid;gap:6px;max-height:520px;overflow:auto}.gmd-message-list>button{width:100%;display:grid;grid-template-columns:40px 1fr auto;gap:10px;align-items:center;text-align:left;padding:11px;border:1px solid transparent;border-radius:14px;background:transparent;color:var(--sn-ink);cursor:pointer}.gmd-message-list>button:hover,.gmd-message-list>button.active{background:var(--sn-surface-2);border-color:var(--sn-line)}.gmd-message-list>button.sent{border-color:color-mix(in srgb,var(--sn-line) 70%,transparent)}.gmd-message-list>button>i{width:7px;height:7px;border-radius:99px;background:var(--sn-guardian-accent)}.gmd-avatar{width:40px;height:40px;display:grid;place-items:center;border-radius:12px;background:var(--sn-surface-2);border:1px solid var(--sn-line);font-size:10px;font-weight:900}.gmd-avatar.unread{background:var(--sn-guardian-tint);color:var(--sn-guardian-accent)}.gmd-preview{min-width:0;display:grid;gap:2px}.gmd-preview strong,.gmd-preview b,.gmd-preview small{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.gmd-preview strong{font-size:11px}.gmd-preview b{font-size:10.5px}.gmd-preview small{color:var(--sn-muted);font-size:9.5px;font-weight:500}.gmd-direction{padding:4px 6px;border:1px solid var(--sn-line);border-radius:999px;color:var(--sn-muted);font-size:7px;font-weight:900;letter-spacing:.06em}
