...
type ArcadeQuestionKind = "choice"|"match"|"sort"|"classify";
...
type ArcadeQuestion = { id:string;kind:ArcadeQuestionKind;prompt:string;options:string[];answer?:string;explanation?:string };
type Round = { id: string; studentId: string; game: string; difficulty: number; status: string; answers: string[]; correct: number | null; xp: number; stars: number; ageBand:AgeBand|null;standardBand:string|null;engine:string;roundLength:number;score:number|null;challengeMode:boolean;questions: ArcadeQuestion[] };
...
function sortAnswer(value:string|undefined,fallback:string[]){
  if(!value)return[...fallback];
  try{const parsed=JSON.parse(value);if(Array.isArray(parsed)&&parsed.length===fallback.length&&parsed.every(item=>typeof item==="string")){const expected=[...fallback].sort(),actual=[...parsed].sort();if(actual.every((item,index)=>item===expected[index]))return parsed as string[];}}catch{}
  return[...fallback];
}
function displayAnswer(value:string|undefined,kind:ArcadeQuestionKind){
  if(!value)return"—";
  if(kind!=="sort")return value;
...
export default function LearningArcade() {
  ...
  const choose=(value:string)=>{setAnswers(current=>current.map((answer,i)=>i===index?value:answer));setDirty(true);setMessage("");};
  const ordered=question?.kind==="sort"?sortAnswer(answers[index],question.options):[];
  const moveSort=(position:number,direction:-1|1)=>{if(!question||question.kind!=="sort")return;const target=position+direction;if(target<0||target>=ordered.length)return;const next=[...ordered];[next[position],next[target]]=[next[target],next[position]];choose(JSON.stringify(next));};
  ...
  return <div className="arcade">
    ...
    {round?<section className="arcade-panel">
      ...
      {complete?<><h2 ref={heading} tabIndex={-1}>Round complete.</h2><p className="arcade-result">{round.correct}/{round.roundLength} correct · {round.xp} XP · {round.stars}/3 stars</p><p>{round.score?`Leaderboard score ${round.score.toLocaleString()} · `:""}Practice results stay separate from official school marks.</p><div className="arcade-feedback">{round.questions.map((item,i)=><article key={item.id}><span className="arcade-kicker">{item.kind.toUpperCase()}</span><h3>{item.prompt}</h3><p>Your answer: <strong>{displayAnswer(round.answers[i],item.kind)}</strong>{round.answers[i]===item.answer?" ✓":" · Correct answer: "+displayAnswer(item.answer,item.kind)}</p><p>{item.explanation}</p></article>)}</div></>:question?<><p className="arcade-kicker">TASK {index+1} OF {round.questions.length} · {answers.filter(Boolean).length} ANSWERED · {engineLabel(round.engine)}</p><h2 ref={heading} tabIndex={-1}>{question.prompt}</h2>
        {question.kind==="sort"?<div className="arcade-sort" role="group" aria-label="Arrange the items in the correct order"><p className="arcade-muted">Move each tile until the sequence is correct, then confirm the order.</p><ol className="arcade-sort-list">{ordered.map((item,position)=><li key={item}><strong>{position+1}</strong><span>{item}</span><div><button disabled={busy||position===0} aria-label={`Move ${item} up`} onClick={()=>moveSort(position,-1)}>↑</button><button disabled={busy||position===ordered.length-1} aria-label={`Move ${item} down`} onClick={()=>moveSort(position,1)}>↓</button></div></li>)}</ol><button className={answers[index]?"arcade-confirmed":"arcade-primary"} disabled={busy} onClick={()=>choose(JSON.stringify(ordered))}>{answers[index]?"Order confirmed ✓":"Confirm this order"}</button></div>
        :question.kind==="match"?<div className="arcade-interaction-grid" role="group" aria-label="Choose the matching answer"><p className="arcade-muted arcade-full">Find the best match.</p>{question.options.map(option=><button key={option} className={answers[index]===option?"chosen":""} disabled={busy} aria-pressed={answers[index]===option} onClick={()=>choose(option)}>{option}</button>)}</div>
        :question.kind==="classify"?<div className="arcade-interaction-grid arcade-buckets" role="group" aria-label="Choose the correct category"><p className="arcade-muted arcade-full">Place this item in the correct group.</p>{question.options.map(option=><button key={option} className={answers[index]===option?"chosen":""} disabled={busy} aria-pressed={answers[index]===option} onClick={()=>choose(option)}>{option}</button>)}</div>
        :<fieldset className="arcade-options" disabled={busy}><legend>Choose one answer</legend>{question.options.map(option=><label key={option} className={answers[index]===option?"chosen":""}><input type="radio" name={"question-"+question.id} checked={answers[index]===option} onChange={()=>choose(option)}/>{option}</label>)}</fieldset>}
        <div className="arcade-row"><button disabled={busy||index===0} onClick={()=>setIndex(current=>current-1)}>Previous</button><button disabled={busy||!dirty} onClick={()=>void save(false)}>Save progress</button>{index<round.questions.length-1?<button className="arcade-primary" disabled={busy} onClick={()=>setIndex(current=>current+1)}>Next task →</button>:<button className="arcade-primary" disabled={busy||answers.some(answer=>!answer)} onClick={()=>void save(true)}>{busy?"Saving…":"Finish round"}</button>}</div><p className="arcade-muted">{round.challengeMode?"Challenge mode is enabled for this round.":"No timer. You can save and pause at any point."}</p></>:null}
    </section>:<>
    ...
    <style>{`
      ...
      .arcade button{cursor:pointer}.arcade button:disabled{opacity:.55;cursor:not-allowed}.arcade .arcade-primary{background:var(--sn-guardian-accent);color:var(--color-surface);font-weight:700}.arcade .arcade-confirmed{border-color:var(--sn-guardian-accent);background:var(--sn-guardian-tint);font-weight:700}
      ...
      .arcade-interaction-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:20px 0}.arcade-interaction-grid .arcade-full{grid-column:1/-1;margin:0}.arcade-interaction-grid button{min-height:64px;font-size:17px;text-align:left;border-width:2px}.arcade-interaction-grid button.chosen{border-color:var(--sn-guardian-accent);background:var(--sn-guardian-tint);font-weight:700}.arcade-buckets button{text-align:center}
      .arcade-sort{margin:20px 0;display:grid;gap:12px}.arcade-sort-list{list-style:none;padding:0;margin:0;display:grid;gap:8px}.arcade-sort-list li{display:grid;grid-template-columns:42px minmax(0,1fr) auto;gap:12px;align-items:center;padding:10px 12px;border:1px solid var(--sn-line);border-radius:12px;background:var(--sn-surface-2)}.arcade-sort-list li>strong{display:grid;place-items:center;width:32px;height:32px;border-radius:999px;background:var(--sn-guardian-tint);color:var(--sn-guardian-accent)}.arcade-sort-list li>div{display:flex;gap:6px}.arcade-sort-list button{min-width:44px;padding:8px 10px}
      ...
      @media(max-width:650px){.arcade-games,.arcade-feedback,.arcade-controls,.arcade-interaction-grid{grid-template-columns:1fr}.arcade-interaction-grid .arcade-full{grid-column:auto}.arcade-hero{display:block}.arcade-options{grid-template-columns:1fr}.arcade-panel,.arcade-hero{padding:18px}.arcade-history li{align-items:flex-start;flex-direction:column}.arcade-ranking li{grid-template-columns:40px 1fr}.arcade-ranking li>span:nth-of-type(2),.arcade-ranking small{grid-column:2}.arcade-card-actions button{flex:1}.arcade-sort-list li{grid-template-columns:36px minmax(0,1fr)}.arcade-sort-list li>div{grid-column:2}}
    `}</style>
  </div>;
}
