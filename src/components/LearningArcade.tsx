...
type ArcadeQuestionKind = "choice" | "match" | "sort" | "classify" | "path" | "build" | "typed";
...
function displayAnswer(value: string | undefined, kind: ArcadeQuestionKind) {
  if (!value) return "—";
  if (kind !== "sort" && kind !== "build") return value;
  try {
...
export default function LearningArcade() {
  ...
  const [leaderScope, setLeaderScope] = useState("standard");
  const [leaderPeriod, setLeaderPeriod] = useState("weekly");
  ...
  useEffect(() => {
    ...
    void api("/api/guardian/arcade?" + query.toString())
      .then((next: Leaderboard) => { if (!cancelled) setLeaderboard(next); })
      .catch((leaderError) => { if (!cancelled) setError(leaderError instanceof Error ? leaderError.message : "Could not load leaderboard."); })
  ...
  const run = async (action: () => Promise<void>) => {
    ...
    try {
      ...
      setError(actionError instanceof Error ? actionError.message : "Could not save. Please try again.");
    } finally {
  ...
  const open = async (body: unknown) => run(async () => {
    const next: Round = await api("/api/guardian/arcade", body);
    setRound(next);
  ...
  const save = async (finish: boolean, close = false) => run(async () => {
    ...
    setRound(close ? null : next);
    setDirty(false);
    setMessage(finish ? "Round complete. Take your time to explore the answers." : "Progress saved. You can come back whenever you like.");
    if (finish || close) await refresh(round.studentId);
  });
  const back = () => {
    if (round?.status === "in_progress") void save(false, true);
    else { setRound(null); setMessage(""); }
  };
  ...
  const renderQuestion = (item: ArcadeQuestion) => {
    if (item.kind === "sort" || item.kind === "build") {
      return <ArcadePhysicsOrder items={item.options} value={answers[index] ?? ""} onChange={choose} disabled={busy} mode={item.kind} />;
    }
    if (item.kind === "typed") {
      return <ArcadePhysicsTyped value={answers[index] ?? ""} onChange={choose} disabled={busy} label={round?.engine === "typing" ? "Type exactly what the task asks for" : "Type your answer"} />;
    }
    if (item.kind === "path") {
      return <ArcadePhysicsChoice options={item.options} value={answers[index] ?? ""} onChange={choose} disabled={busy} variant="path" hint="Choose the route or door that solves the challenge." />;
    }
    if (item.kind === "match") {
      return <ArcadePhysicsChoice options={item.options} value={answers[index] ?? ""} onChange={choose} disabled={busy} variant="match" hint="Find the best match." />;
    }
    if (item.kind === "classify") {
      return <ArcadePhysicsChoice options={item.options} value={answers[index] ?? ""} onChange={choose} disabled={busy} variant="classify" hint="Place this item in the correct group." />;
    }
  ...
  return <div className="arcade">
    <header className="arcade-hero">
      <div>
        <span className="arcade-kicker">SUKUUNOVA LEARNING UNIVERSE</span>
        <h1>Choose your level. Find a challenge. Keep growing.</h1>
        <p>A 64-game educational catalogue organised by age, school standard and subject—with progress, rewards and fair rankings.</p>
      </div>
      <Link href="/guardian" onClick={(event) => { if (dirty && !window.confirm("Leave without saving your latest answers?")) event.preventDefault(); }}>Family dashboard →</Link>
    </header>

    {error ? <p role="alert" className="arcade-alert">{error}</p> : null}
    {message ? <p role="status" className="arcade-message">{message}</p> : null}

    {round ? <section className="arcade-panel arcade-round-panel">
      <div className="arcade-row">
        <button disabled={busy} onClick={back}>← {complete ? "Back to games" : "Save & return to games"}</button>
        <span>{selectedName} · {catalogName(round.game)} · Difficulty {round.difficulty} · {round.roundLength} tasks</span>
      </div>

      {complete ? <>
        <div className="arcade-complete-burst" aria-hidden="true"><span>★</span><span>★</span><span>★</span></div>
        <h2 ref={heading} tabIndex={-1}>Round complete.</h2>
        <p className="arcade-result">{round.correct}/{round.roundLength} correct · {round.xp} XP · {round.stars}/3 stars</p>
        <p>{round.score ? `Leaderboard score ${round.score.toLocaleString()} · ` : ""}Practice results stay separate from official school marks.</p>
        <div className="arcade-feedback">{round.questions.map((item, answerIndex) => <article key={item.id} data-correct={item.correct === true ? "true" : "false"}>
          <span className="arcade-kicker">{item.kind.toUpperCase()}</span>
          <h3>{item.prompt}</h3>
          <p>Your answer: <strong>{displayAnswer(round.answers[answerIndex], item.kind)}</strong>{item.correct ? " ✓" : " · Correct answer: " + displayAnswer(item.answer, item.kind)}</p>
          <p>{item.explanation}</p>
        </article>)}</div>
      </> : question ? <>
        <p className="arcade-kicker">TASK {index + 1} OF {round.questions.length} · {answers.filter((answer) => answer.trim()).length} ANSWERED · {engineLabel(round.engine)}</p>
        <h2 ref={heading} tabIndex={-1}>{question.prompt}</h2>
        {renderQuestion(question)}
        <div className="arcade-row arcade-round-actions">
          <button disabled={busy || index === 0} onClick={() => setIndex((current) => current - 1)}>Previous</button>
          <button disabled={busy || !dirty} onClick={() => void save(false)}>Save progress</button>
          {index < round.questions.length - 1
            ? <button className="arcade-primary" disabled={busy} onClick={() => setIndex((current) => current + 1)}>Next task →</button>
            : <button className="arcade-primary" disabled={busy || unfinished} onClick={() => void save(true)}>{busy ? "Saving…" : "Finish round"}</button>}
        </div>
        <p className="arcade-muted">{round.challengeMode ? "Challenge mode is enabled for this round." : "No timer. You can save and pause at any point."}</p>
      </> : null}
    </section> : <>
      <section className="arcade-panel arcade-row">
        <label>Choose a child
          <select disabled={busy || loading} value={data?.selected?.id ?? ""} onChange={(event) => {
            setData((current) => current ? { ...current, selected: null, progress: [], recent: [] } : current);
            setMessage("");
            void refresh(event.target.value);
          }}>
            {data?.children.map((child) => <option key={child.id} value={child.id}>{child.name} · {child.class?.name ?? "Class not set"}</option>)}
          </select>
        </label>
        <div><strong>{loading ? "…" : data?.streak ?? 0}</strong> learning-day streak<p className="arcade-muted">Breaks are welcome. Come back when you are ready.</p></div>
      </section>
      ...
      {loading ? <p role="status">Loading your learning universe…</p> : !data?.selected ? <section className="arcade-panel"><h2>No linked learners yet</h2><p>Ask your school to link an active learner to this guardian account.</p></section> : <>
        <section className="arcade-panel arcade-controls">
          <div><span className="arcade-kicker">LEARNER PROFILE</span><h2>{data.selected.name}</h2><p>{data.selected.class?.name ?? "Class not set"} · {standardLabels[data.standardBand ?? ""] ?? data.standardBand ?? "Standard not set"}</p></div>
          <label>Choose age practice band
            <select value={ageBand} onChange={(event) => { setAgeBand(event.target.value as AgeBand); setLeaderGame(null); setLeaderboard(null); }}>
              {data.allowedAgeBands.map((age) => <option key={age} value={age}>{ageLabels[age]}{age === data.recommendedAgeBand ? " · recommended" : " · revision"}</option>)}
            </select>
          </label>
          ...
          <label className="arcade-check"><input type="checkbox" checked={easier} onChange={(event) => setEasier(event.target.checked)} /> One step easier when adaptive difficulty allows</label>
        </section>
        ...
        <div className="arcade-games">{visibleGames.map((game) => {
          const progress = data.progress.find((item) => item.game === game.gameKey);
          const canPlay = game.live && game.enabled && game.eligible;
          return <article className={`arcade-panel arcade-game-card ${canPlay ? "live" : "planned"}`} key={game.gameKey}>
            <div className="arcade-card-top"><span aria-hidden="true" className="arcade-symbol">{game.symbol}</span><span className="arcade-status">{canPlay ? "LIVE" : game.live && !game.enabled ? "SCHOOL DISABLED" : "COMING SOON"}</span></div>
            <h3>{game.name}</h3><p>{game.description}</p>
            <div className="arcade-tags"><span>{game.category}</span><span>{engineLabel(game.engine)}</span><span>{ageBand ? ageLabels[ageBand] : "Age band"}</span></div>
            {game.live ? <p className="arcade-muted">Level {progress?.level ?? 1} · {progress?.xp ?? 0} XP · {progress?.rounds ?? 0} rounds · {progress?.accuracy === null || progress?.accuracy === undefined ? "No accuracy yet" : progress.accuracy + "% accuracy"}</p> : <p className="arcade-muted">Learning pack planned on the shared {engineLabel(game.engine)} engine.</p>}
            <details><summary>Game settings</summary><div className="arcade-settings"><span>Difficulty {game.difficultyMin}–{game.difficultyMax}</span><span>Default {game.defaultRoundLength} tasks</span><span>{game.timerPolicy === "none" ? "No timer" : game.timedChallengesEnabled ? "Timed challenge enabled" : "Timer optional by school"}</span><span>{game.standardBands.map((item) => standardLabels[item] ?? item).join(", ")}</span><span>{game.curriculumTags.join(" · ")}</span></div></details>
            <div className="arcade-card-actions">{canPlay ? <button className="arcade-primary" disabled={busy} onClick={() => void open({ action: "start", studentId: data.selected!.id, game: game.gameKey, ageBand, easier, roundLength: game.defaultRoundLength })}>Play / resume →</button> : <button disabled>{game.live ? "Unavailable" : "Content pack coming"}</button>}{game.live ? <button disabled={busy} onClick={() => { setLeaderGame(game.gameKey); setLeaderScope("standard"); setLeaderPeriod("weekly"); }}>Leaderboard</button> : null}</div>
          </article>;
        })}</div>

        {leaderGame && leaderDefinition ? <section className="arcade-panel arcade-leaderboard">
          <div className="arcade-row"><div><span className="arcade-kicker">GAME RANKING</span><h2>{leaderDefinition.name} leaderboard</h2><p>Privacy-safe ranking. Accuracy and suitable difficulty lead; XP breaks ties.</p></div><button onClick={() => { setLeaderGame(null); setLeaderboard(null); }}>Close</button></div>
          <div className="arcade-row arcade-ranking-filters"><label>Ranking group<select value={leaderScope} onChange={(event) => setLeaderScope(event.target.value)}><option value="class">Class</option><option value="standard">Standard</option><option value="age">Age category</option><option value="school">Whole school</option></select></label><label>Period<select value={leaderPeriod} onChange={(event) => setLeaderPeriod(event.target.value)}><option value="weekly">This week</option><option value="monthly">This month</option><option value="all">All time</option></select></label></div>
          {leaderLoading ? <p role="status">Calculating ranking…</p> : leaderboard?.rows.length ? <ol className="arcade-ranking">{leaderboard.rows.map((row) => <li key={row.studentId} className={row.studentId === data.selected?.id ? "me" : ""}><strong>#{row.rank}</strong><span>{row.displayName}{row.studentId === data.selected?.id ? " · You" : ""}</span><span>{row.bestScore.toLocaleString()} pts</span><small>{row.totalXp} XP · {row.rounds} rounds</small></li>)}</ol> : <p>No completed rounds in this ranking yet. The first learner to finish can set the pace.</p>}
        </section> : null}
    ...
    <style>{`
      ...
      .arcade-kicker{font-size:11px;font-weight:800;letter-spacing:.08em;color:var(--sn-guardian-accent)}.arcade a{color:var(--sn-guardian-accent)}.arcade-muted{color:var(--sn-muted);font-size:13px}
      ...
      .arcade-feedback article{padding:16px;background:var(--sn-surface-2);border:2px solid var(--sn-line);border-radius:12px;overflow-wrap:anywhere}.arcade-feedback article[data-correct="true"]{border-color:var(--color-success)}.arcade-feedback article[data-correct="false"]{border-color:var(--color-danger)}
      .arcade-game-card{display:flex;flex-direction:column;transform:translate3d(0,0,0);transition:transform 220ms cubic-bezier(.2,.8,.2,1.2),box-shadow 220ms ease,border-color 180ms ease}.arcade-game-card.live:hover{transform:translate3d(0,-5px,0);box-shadow:0 8px 0 var(--sn-line);border-color:var(--sn-guardian-accent)}.arcade-game-card.planned{background:var(--sn-surface-2)}
      ...
      .arcade-symbol{display:grid;place-items:center;width:56px;height:56px;font-size:24px;border-radius:16px;background:var(--sn-guardian-tint);color:var(--sn-guardian-accent);overflow:hidden;transition:transform 220ms cubic-bezier(.2,.8,.2,1.4)}.arcade-game-card.live:hover .arcade-symbol{transform:rotate(-5deg) scale(1.08)}
      .arcade button,.arcade select{font:inherit;min-height:44px;padding:10px 16px;border-radius:10px;border:1px solid var(--sn-line);background:var(--sn-surface-2);color:var(--sn-ink)}.arcade button{cursor:pointer;transition:transform 120ms ease}.arcade button:active:not(:disabled){transform:translateY(1px) scale(.985)}.arcade button:disabled{opacity:.55;cursor:not-allowed}.arcade .arcade-primary{background:var(--sn-guardian-accent);color:var(--color-surface);font-weight:700}.arcade .arcade-confirmed{border-color:var(--sn-guardian-accent);background:var(--sn-guardian-tint);font-weight:700}
      ...
      .arcade-round-panel{position:relative;overflow:hidden}.arcade-round-actions{margin-top:20px}.arcade-complete-burst{display:flex;gap:10px;margin:22px 0 4px;font-size:30px;color:var(--sn-guardian-accent)}.arcade-complete-burst span{display:inline-block;animation:arcade-star-pop 520ms cubic-bezier(.2,1.5,.3,1) both}.arcade-complete-burst span:nth-child(2){animation-delay:90ms}.arcade-complete-burst span:nth-child(3){animation-delay:180ms}@keyframes arcade-star-pop{0%{transform:translateY(18px) scale(.2) rotate(-18deg);opacity:0}65%{transform:translateY(-5px) scale(1.18) rotate(8deg);opacity:1}100%{transform:translateY(0) scale(1) rotate(0);opacity:1}}
