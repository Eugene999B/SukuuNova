    ...
    <main className="ops-studio">
      ...
      <section className="ops-main-grid">
        <div className="ops-workspace">
          {module === "library" && <div className="library-grid">{records.map(row => { const hasCover = hasText(row.coverUrl); const hasFile = hasText(row.fileUrl); return <article className="book-card" key={getText(row.id)}><div className="book-cover">{hasCover ? <img src={getText(row.coverUrl)} alt="" /> : <div><small>{getText(row.materialType, "book").toUpperCase()}</small><strong>{titleFor(row).slice(0, 2).toUpperCase()}</strong></div>}</div><div className="book-body"><span className="book-category">{getText(row.category)}</span><h3>{titleFor(row)}</h3><p>{getText(row.author, "School resource")}</p><div className="book-meta"><span>{getNumber(row.availableCopies)} available</span><span>{hasFile ? "Digital" : `${getNumber(row.copies)} copies`}</span></div>{hasFile && <div className="book-actions"><a className="ops-link" href={getText(row.fileUrl, "#")} target="_blank" rel="noreferrer">Read</a><a className="ops-link secondary" href={getText(row.fileUrl, "#")} download>Download</a></div>}</div></article>; })}</div>}
          ...
          {module === "recruitment" && <div className="ops-list">{records.map(row => { const hasPublicToken = hasText(row.publicToken); return <article className="ops-list-row" key={getText(row.id)}><div><span>{getText(row.department, "School")}</span><h3>{getText(row.title)}</h3><p>{getText(row.employmentType)} · {getText(row.closingDate, "Open")}</p></div><div><strong>{getText(row.status, "open")}</strong><small>{hasPublicToken ? "Public application link ready" : "Draft"}</small></div></article>; })}</div>}
