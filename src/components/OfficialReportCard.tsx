      ...
      <article className={`paper theme-${theme.key} header-${theme.headerMode} density-${theme.density} font-${theme.fontMode}`} style={vars}>
        ...
        <section className="performance-section">
          ...
          <table className="marks">
            ...
            <tbody>{data.results.map((result) => <tr key={result.subjectId}><td>{result.subject}</td><td>{formatNumber(result.ca)}</td><td>{formatNumber(result.exam)}</td><td><strong>{formatNumber(result.total)}</strong></td><td><strong className="grade-pill">{result.grade ?? "—"}</strong></td>{data.reportSettings.showSubjectPosition ? <td>{ordinal(result.position)}{result.position != null && positionDenominator ? ` / ${positionDenominator}` : ""}</td> : null}</tr>)}</tbody>
          </table>
        </section>
