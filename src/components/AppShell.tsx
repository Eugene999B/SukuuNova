...
const schoolGroups = [
  ...
  { label: "People", items: [["♟", "Students", "/school/students"], ["♧", "Parents & Guardians", "/school/guardians"], ["♙", "Staff & Teachers", "/school/staff"]] },
  ...
  { label: "Attendance", items: [["◉", "Student Attendance", "/school/attendance"], ["◌", "Staff Attendance", "/school/staff-attendance"], ["!", "Late / Absence", "/school/attendance/exceptions"], ["⌁", "Parent Alerts", "/school/communications/alerts"]] },
      ...
      <aside className="app-sidebar">
        ...
        <nav className="app-nav" aria-label="Primary navigation">
          {groups.map(group => <div className="app-nav-group" key={group.label}>
            ...
            {group.items.map(([icon, label, href]) => <Link key={label} href={href} className={`app-nav-item ${active === label ? "is-active" : ""}`} title={label}>
        ...
        <div className="app-sidebar-bottom">
          ...
          <div className="app-user-mini"><span className="app-user-avatar">{avatar}</span><span><b>{userName}</b><small>{role}</small></span><span className="app-dots">•••</span></div>
        </div>
      </aside>
      <main className="app-main">
        <header className="app-topbar">
          <div><div className="app-breadcrumb">SukuuNova <span>›</span> {universe === "school" ? schoolName : "Platform Control"}</div><h1>{title}</h1><p>{subtitle}</p></div>
          <div className="app-top-actions">
            ...
            <Link className="app-icon-button" href={universe === "school" ? "/school/communications/alerts" : "/platform/notifications"} aria-label="Notifications">◌<i /></Link>
            <button className="app-profile-button" type="button"><span>{avatar}</span>⌄</button>
          </div>
        </header>
