...
const platformGroups: Group[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: Workflow, label: "Worker School Scope", href: "/platform/admins/access", permission: "admins.view" },
...
function normalize(groups: Group[]): NavGroup[] {
  return groups.map(({ label, items }) => ({ label, items }));
}

function commandItems(groups: Group[]): CommandItem[] {
...
export function AppShell({
  ...
  const groups =
    universe === "platform" && platformAccess
      ? baseGroups
          .map((group) => ({
            ...group,
            items: group.items.filter(
              (item) => !item.permission || platformAccess[item.permission],
            ),
          }))
  ...
  const preferenceScope = `${universe}:${schoolCode || "platform"}:${userName || "user"}`
    .replace(/\s+/g, "_")
    .toLowerCase();
  const isTeacher = universe === "teacher";
  const isGuardian = universe === "guardian";
  const visiblePlatformItems =
    universe === "platform" ? groups.flatMap((group) => group.items) : [];
  const firstPlatformHref = visiblePlatformItems[0]?.href || "/account/security";
  const utilityHref =
    universe === "platform"
      ? platformAccess?.["support.view"]
        ? "/platform/inbox"
        : firstPlatformHref
      : isGuardian
        ? "/guardian/messages"
        : isTeacher
          ? "/teacher/module?view=My%20Messages"
  ...
  useEffect(() => {
    try {
      setCompact(
        localStorage.getItem(`sukuunova-sidebar-compact:${preferenceScope}`) ===
          "true",
      );
    } catch {
      setCompact(false);
    ...
    setCompact((value) => {
      ...
      try {
        localStorage.setItem(
          `sukuunova-sidebar-compact:${preferenceScope}`,
          String(next),
        );
      } catch {
  ...
  return (
    <div
      className={`app-shell app-shell-${universe} ${compact ? "is-compact" : ""}`}
    >
      <aside className="app-sidebar">
        ...
        <div className="app-school-chip">
          ...
          <span>
            <b>
              {universe === "platform" ? "SukuuNova Network" : schoolName}
            </b>
            <small>
              {universe === "platform"
                ? "Operator workspace"
                : `${schoolCode}${schoolCode ? " · " : ""}School account`}
            </small>
          </span>
        </div>
        ...
        <div className="app-sidebar-bottom">
          ...
          <div className="app-account-actions">
            <Link href="/account/security" className="app-account-link">
        ...
        <header className="app-topbar">
          <div className="app-topbar-title">
            <div className="app-breadcrumb">
              SukuuNova <span>›</span>{" "}
              {universe === "platform" ? "Platform Control" : schoolName}
            </div>
            <h1>{title}</h1>
            <p>{subtitle}</p>
          </div>
