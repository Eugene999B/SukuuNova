...
export default function TeacherWeeklyWorkSetup({ classId, subjectId, termId, weekNumber, nextWorkNumber, termStart, termEnd }: Props) {
  ...
  async function createWork() {
    ...
    try {
      const response = await fetch("/api/school/teacher-academic-workspace", {
        ...
        body: JSON.stringify({ action: "createMarkSheet", termId, classId, subjectId, weekNumber, workDate, maxScore }),
      });
  ...
  return <div className="weekly-work-setup">
    <div className="weekly-work-setup-title">
      <div className="weekly-work-number">{nextWorkNumber}</div>
      <div><span>NEW WORK</span><h3>Work {nextWorkNumber}</h3><p>Set the date and total marks first. The student mark sheet opens immediately after this.</p></div>
    </div>
    <div className="weekly-work-setup-fields">
      <label><span><CalendarDays size={14} aria-hidden="true" /> Date</span><input type="date" min={termStart} max={termEnd} value={workDate} disabled={busy} onChange={(event) => setWorkDate(event.target.value)} /></label>
      ...
      <button type="button" disabled={busy || !workDate || !Number.isFinite(maxScore) || maxScore <= 0} onClick={() => void createWork()}>{busy ? <><Loader2 size={16} className="spin" /> Creating…</> : <><Plus size={16} /> Create Work {nextWorkNumber} & enter marks</>}</button>
    </div>
