...
import { ArrowRight, BadgeAlert, Building2, Plus, Search } from "lucide-react";
...
export default function PlatformSchoolsConsole({ overview }: { overview: Overview }) {
  ...
  return <div className="platform-page-stack platform-schools-simple">
    <section className="platform-page-header">
      <div><span className="platform-eyebrow">Network directory</span><h2>Schools</h2><p>{allSchools.length} accounts · {overview.totals.activeSchools} active · {needsAttention} need attention · {critical} critical</p></div>
      <div className="platform-header-actions">{canOnboard ? <Link href="/platform/schools/new" className="app-action"><Plus size={14}/><strong>Add school</strong></Link> : null}</div>
    </section>

    <section className="app-card app-panel platform-filter-bar"><div className="platform-filter-search"><Search size={16}/><input aria-label="Search schools" placeholder="Search school name or code" value={query} onChange={(event) => setQuery(event.target.value)}/>{query && <button type="button" onClick={() => setQuery("")} aria-label="Clear search">Clear</button>}</div><div className="platform-filter-chips" aria-label="School filters">{([['all','All'],['attention','Needs attention'],['active','Active'],['suspended','Suspended']] as const).map(([value,label]) => <button type="button" key={value} className={`platform-filter-chip ${filter === value ? 'is-active' : ''}`} onClick={() => setFilter(value)}>{label}<span>{value==='all'?allSchools.length:value==='attention'?needsAttention:value==='active'?overview.totals.activeSchools:overview.totals.suspendedSchools}</span></button>)}</div><div className="platform-filter-tools"><label><span>Sort</span><select aria-label="Sort schools" value={sort} onChange={(event) => setSort(event.target.value as Sort)}><option value="attention">Attention</option><option value="name">Name</option><option value="students">Students</option><option value="collected">Collections</option></select></label>{(query || filter !== 'all' || sort !== 'attention') && <button type="button" className="app-pill" onClick={resetFilters}>Reset</button>}</div></section>

    <section className="app-card app-panel platform-school-table" aria-label="School accounts"><div className="platform-table-toolbar"><div><b>{schools.length.toLocaleString()}</b><span>matching schools</span></div></div><div className="platform-school-grid platform-school-grid-head"><span>School</span><span>State</span><span>Plan</span><span>People</span><span>Finance</span><span>Open</span></div>{schools.map((school) => { const score = attentionScore(school); const tone = attentionTone(score); return <div key={school.id} className={`platform-school-grid platform-school-row platform-tone-${tone}`}><div className="platform-school-primary"><span className="platform-school-avatar"><Building2 size={16}/></span><div><b>{school.name}</b><small>{school.uniqueCode}</small></div></div><div><span className={`platform-status platform-status-${tone}`}>{score >= 60 ? "Critical" : score >= 25 ? "Watch" : school.status}</span></div><div><span className="platform-table-value">{school.subscriptionPlan?.name || "No plan"}</span></div><div><span className="platform-table-value">{school.studentCount.toLocaleString()} students</span><small className="platform-table-muted">{school.userCount.toLocaleString()} users · {school.classCount.toLocaleString()} classes</small></div><div><span className={`platform-table-value ${school.unpaidInvoices ? 'is-warning' : ''}`}>{school.unpaidInvoices}/{school.invoices} unpaid</span><small className="platform-table-muted">₵{Number(school.collected || 0).toLocaleString()} collected</small></div><div><Link href={`/platform/schools/${school.id}`} className="app-action"><strong>Open</strong><ArrowRight size={13}/></Link></div></div>; })}{schools.length === 0 && <div className="platform-empty"><BadgeAlert size={22}/><b>No schools match this view.</b><button type="button" className="app-action" onClick={resetFilters}><strong>Reset</strong></button></div>}</section>
  </div>;
}