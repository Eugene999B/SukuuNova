...
function sameCell(a: Cell | undefined, b: Cell) {
...
export default function TeacherWeeklyMarkbook({ weekNumber, classId, subjectId, termId, termStart, termEnd, nextWorkNumber, works, rows, locked = false }: Props) {
  const router = useRouter();
  const pathname = usePathname();
  const initialSnapshots = useMemo(() => {
    const result: Record<string, Snapshot> = {};
    for (const work of works) for (const row of rows) result[cellKey(work.id, row.student.id)] = row.scores[work.id] ?? null;
    return result;
  }, [rows, works]);
  const initialCells = useMemo(() => Object.fromEntries(Object.entries(initialSnapshots).map(([key, snapshot]) => [key, fromSnapshot(snapshot)])) as Record<string, Cell>, [initialSnapshots]);
  ...
  function readPending(workId: string): Record<string, Cell> {
    ...
    try {
      const raw = JSON.parse(window.localStorage.getItem(storageKey(workId)) || "{}") as Record<string, unknown>;
      return Object.fromEntries(Object.entries(raw).filter(([, value]) => storedCell(value))) as Record<string, Cell>;
    } catch {
      return {};
    }
  }
  ...
  function writePending(workId: string, studentId: string, cell: Cell | null) {
    ...
    try {
      ...
      // Server autosave remains available if browser storage is blocked.
    }
  }
  ...
  function setOffline(key: string, active: boolean) {
    setOffline((current) => {
      const next = new Set(current);
      if (active) next.add(key); else next.delete(key);
      return next;
  ...
  function buildChange(work: Work, row: Row, cell: Cell, snapshot: Snapshot): GradebookChange | null {
    ...
    if (!Number.isFinite(value) || value < 0 || value > work.maxScore || (cell.status !== "present" && value !== 0)) {
      throw new Error(`${row.student.name}: enter a mark from 0 to ${work.maxScore}. You can also type A for absent or E for excused.`);
    }
    if (cell.status === "present" && !/^(?:\d+(?:\.\d*)?|\.\d+)$/.test(cell.value.trim())) {
  ...
  function scheduleSave(workId: string, studentId: string, delay = 500) {
  ...
  function updateCell(workId: string, studentId: string, nextCell: Cell, delay = 500) {
    if (locked) return;
    const key = cellKey(workId, studentId);
  ...
  async function saveCell(workId: string, studentId: string) {
    ...
    if (inFlight.current.has(key)) {
      scheduleSave(workId, studentId, 300);
      return;
    }
    ...
    try {
      ...
      if (!result || result.studentId !== studentId || result.assessmentId !== workId) throw new Error("SukuuNova did not receive a complete save confirmation. The mark remains queued safely on this device.");
      ...
      if (sameCell(cellsRef.current[key], submitted)) {
        ...
        scheduleSave(workId, studentId, 180);
      }
      setError("");
      setOffline(key, false);
    } catch (cause) {
  ...
  function applyBulkMark() {
    if (locked || !activeWork || !selected.size) return;
    ...
    cellsRef.current = next;
    setCells(next);
    targetRows.forEach((row, index) => scheduleSave(activeWork.id, row.student.id, 70 + index * 25));
  ...
  async function createWork() {
    ...
    try {
      ...
      if (!data.result?.assessmentId) throw new Error("Work was created but the worksheet could not be refreshed safely.");
  ...
  useEffect(() => {
    ...
    for (const work of works) for (const row of rows) {
      const key = cellKey(work.id, row.student.id);
      if (!sameCell(next[key], fromSnapshot(snapshotsRef.current[key] ?? null))) scheduleSave(work.id, row.student.id, 500);
    }
    // Recovery is intentionally scoped to the currently opened weekly markbook.
  ...
  const saveLabel = offline.size ? `${offline.size} waiting for connection` : saving.size || dirtyKeys.length ? "Saving automatically…" : "All changes saved";

  return (
    <div className="markbook-shell">
      <div className="markbook-toolbar">
        <div className="markbook-toolbar-left">
          <label className="markbook-search"><Search size={16} aria-hidden="true" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search learner" aria-label="Search learner" /></label>
          <div className={`markbook-save-state ${offline.size ? "is-offline" : saving.size || dirtyKeys.length ? "is-saving" : ""}`} role="status">
            {offline.size ? <CloudOff size={15} aria-hidden="true" /> : saving.size || dirtyKeys.length ? <Loader2 size={15} className="spin" aria-hidden="true" /> : <CheckCircle2 size={15} aria-hidden="true" />}
            {saveLabel}
          </div>
        </div>
      ...
      {showNewWork && !locked ? <div className="markbook-new-work">
        <div className="markbook-new-work-heading"><div><span>NEW COLUMN</span><strong>Set up Work {nextWorkNumber}</strong><small>Choose its date and total marks. It will appear as a new column in this Week {weekNumber} markbook.</small></div><button type="button" aria-label="Close new work setup" onClick={() => setShowNewWork(false)}><X size={17} /></button></div>
      ...
      {!works.length ? <div className="markbook-empty">
        ...
        <span>Create Work 1 above. Your learners are already ready in this markbook.</span>
        ...
        <table className="markbook-grid" aria-label={`Week ${weekNumber} marks`}>
          <thead>
            <tr>
              ...
              {works.map((work) => <th key={work.id} className={activeWork?.id === work.id ? "is-active-work" : ""} onClick={() => setActiveWorkId(work.id)}>
                <div className="markbook-work-head"><div><strong>Work {work.workNumber}</strong><span>{prettyDate(work.workDate)}</span></div><small>Out of <b>{work.maxScore}</b></small></div>
              </th>)}
            </tr>
          </thead>
            ...
            {works.map((work) => {
              ...
              return <td key={work.id} className={`${activeWork?.id === work.id ? "is-active-work" : ""} ${isDirty ? "is-dirty" : ""}`}>
                <div className="markbook-cell">
                  <input ref={(element) => { inputs.current[inputKey] = element; }} value={inputValue(cell)} disabled={locked} inputMode="decimal" placeholder="—" aria-label={`${row.student.name}, Work ${work.workNumber}, out of ${work.maxScore}`}
      ...
      {works.length ? <div className="markbook-hints"><span><b>Enter</b> moves down the column.</span><span>Type <b>A</b> for absent or <b>E</b> for excused.</span><span>Select learners to give the same mark to many students at once.</span></div> : null}
    </div>
  );
}
