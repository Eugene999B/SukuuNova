...
export default function FinanceEvidencePrint() {
  ...
  function download() {
    ...
    if (format === "csv") {
      ...
      downloadBlob(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(docTitle)}</title><style>body{font:14px Arial;color:#172033;padding:36px}header{display:flex;gap:14px;align-items:center;border-bottom:3px solid #176b61;padding-bottom:16px}header img{width:52px;height:52px;object-fit:contain}h1{margin:0 0 6px}table{border-collapse:collapse;width:100%;margin-top:24px}th,td{border-bottom:1px solid #ddd;padding:10px;text-align:left}th{background:#f3f6f5}</style></head><body><header>${logoUrl ? `<img src="${esc(logoUrl)}" alt="">` : ""}<div><h1>${esc(schoolName)}</h1><div>${esc(docTitle)}</div></div></header><table>${table}</table></body></html>`, "text/html;charset=utf-8", `${kind}-evidence.html`);
    }
  }
