...
export default function FinanceEvidencePrint() {
  ...
  const selectedPayment = (finance?.payments ?? []).find(p => p.id === paymentId) ?? scopedPayments[0];
  ...
  const invoiceRows = useMemo(() => (finance?.invoices ?? []).filter(i => (!studentId || i.student?.admissionNo === selectedStudent?.admissionNo) && (!termId || i.term?.id === termId || i.term?.name === finance?.terms.find(t => t.id === termId)?.name)), [finance, selectedStudent, studentId, termId]);
  ...
  const rows = useMemo((): string[][] => {
    ...
    if (kind === "invoice" || kind === "arrears") return [["Student", "Admission", "Term", "Invoice", "Amount", "Status"], ...invoiceRows.map(i => [i.student?.name ?? "—", i.student?.admissionNo ?? "—", i.term?.name ?? "—", i.id, money(i.totalAmount), i.status])];
  ...
  }, [kind, selectedPayment, selectedInvoice, scopedPayments, invoiceMap, salaryRows, period, invoiceRows, finance]);

  function download() {
    if (format === "print") { document.documentElement.dataset.printPaper = paper; window.print(); return; }
    if (format === "csv") {
      const csv = rows.map(r => r.map(v => `\"${v.replace(/\"/g, '\"\"')}\"`).join(",")).join("\n");
      downloadBlob(csv, "text/csv;charset=utf-8", `${kind}-evidence.csv`);
    } else if (format === "rtf") {
      ...
      const table = rows.map((r, i) => `<tr>${r.map(c => `<${i === 0 && kind !== "receipt" ? "th" : "td"}>${esc(c)}</${i === 0 && kind !== "receipt" ? "th" : "td"}>`).join("")}</tr>`).join("");
      downloadBlob(`<!doctype html><html><head><meta charset="utf-8"><meta name="color-scheme" content="light dark"><title>${esc(docTitle)}</title><style>body{font:14px Arial;color:CanvasText;background:Canvas;padding:36px}header{display:flex;gap:14px;align-items:center;border-bottom:3px solid currentColor;padding-bottom:16px}header img{width:52px;height:52px;object-fit:contain}h1{margin:0 0 6px}table{border-collapse:collapse;width:100%;margin-top:24px}th,td{border-bottom:1px solid currentColor;padding:10px;text-align:left}th{background:color-mix(in srgb, CanvasText 10%, Canvas)}</style></head><body><header>${logoUrl ? `<img src="${esc(logoUrl)}" alt="">` : ""}<div><h1>${esc(schoolName)}</h1><div>${esc(docTitle)}</div></div></header><table>${table}</table></body></html>`, "text/html;charset=utf-8", `${kind}-evidence.html`);
    }
  }

  if (!finance) return <main className="evidence-app"><div className="evidence-loading">{error || "Loading finance evidence…"}</div></main>;

  return <main className="evidence-app"><section className="evidence-controls no-print"><div><span className="evidence-kicker">FINANCE EVIDENCE CENTRE</span><h2>Professional financial documents</h2><p>Select the record, scope and output, then create a finished document for the office, counter, filing cabinet or reconciliation desk.</p></div><div className="evidence-control-grid"><label>Document<select value={kind} onChange={e => setKind(e.target.value as Kind)}><option value="receipt">Payment receipt</option><option value="student">Student payment statement</option><option value="invoice">Invoice register</option><option value="salary">Staff salary statement</option><option value="payroll">Payroll register</option><option value="arrears">Arrears statement</option><option value="finance">Finance summary</option></select></label><label>Student<select value={studentId} onChange={e => setStudentId(e.target.value)}><option value="">All students</option>{finance.students.map(s => <option key={s.id} value={s.id}>{s.name} · {s.admissionNo}</option>)}</select></label><label>Academic term<select value={termId} onChange={e => setTermId(e.target.value)}><option value="">All terms</option>{finance.terms.map(t => <option key={t.id} value={t.id}>{t.academicYear?.name ? `${t.academicYear.name} · ` : ""}{t.name}</option>)}</select></label><label>Payment<select value={paymentId} onChange={e => setPaymentId(e.target.value)}><option value="">Latest matching</option>{scopedPayments.map(p => <option key={p.id} value={p.id}>{date(p.createdAt)} · {money(p.amount)}</option>)}</select></label><label>Staff<select value={staffId} onChange={e => setStaffId(e.target.value)}><option value="">All staff</option>{Array.from(new Set((payroll?.payslips ?? []).map(p => p.staffId))).map(id => <option key={id} value={id}>{id}</option>)}</select></label><label>Payroll period<input type="month" value={period} onChange={e => setPeriod(e.target.value)} /></label><label>Paper<select value={paper} onChange={e => setPaper(e.target.value as Paper)}><option value="a4">A4 filing</option><option value="thermal">80mm thermal receipt</option></select></label><label>Output<select value={format} onChange={e => setFormat(e.target.value as Format)}><option value="print">Print / Save PDF</option><option value="rtf">Word</option><option value="csv">CSV / Excel</option><option value="html">HTML</option></select></label></div><div className="evidence-buttons"><button onClick={download}>Create document</button><button type="button" onClick={() => window.history.back()}>Back to Finance</button></div></section><section className={`evidence-paper ${paper === "thermal" ? "thermal" : ""}`}><header className="evidence-paper-head"><div className="evidence-brand">{logoUrl ? <img src={logoUrl} alt="" /> : <img src="/icon.svg" alt="" />}<div><strong>{schoolName}</strong><small>SukuuNova Finance Evidence</small></div></div><div className="evidence-title"><h1>{docTitle}</h1><p>Official financial record · {studentId ? selectedStudent?.name ?? "Selected student" : "School scope"}</p></div><div className="evidence-meta"><span>Generated</span><b>{date(new Date().toISOString())}</b></div></header>{kind === "receipt" && selectedPayment ? <><div className="evidence-receipt-no"><span>Receipt No.</span><strong>{selectedPayment.id}</strong></div><div className="evidence-summary"><div><span>Received from</span><strong>{selectedInvoice?.student?.name ?? "Student"}</strong><small>{selectedInvoice?.student?.admissionNo ?? "—"}</small></div><div><span>Amount received</span><strong className="amount">{money(selectedPayment.amount)}</strong><small>{selectedPayment.method.toUpperCase()}</small></div><div><span>Date</span><strong>{date(selectedPayment.createdAt)}</strong><small>{selectedPayment.reference ?? "No reference"}</small></div></div><table><tbody>{rows.map(([k, v]) => <tr key={k}><th>{k}</th><td>{v}</td></tr>)}</tbody></table><div className="evidence-note">This document reflects the transaction recorded in SukuuNova. Reversals remain part of the audit history.</div></> : <><div className="evidence-summary"><div><span>School</span><strong>{schoolName}</strong><small>{studentId ? selectedStudent?.admissionNo ?? "" : "All students"}</small></div><div><span>Academic scope</span><strong>{termId ? finance.terms.find(t => t.id === termId)?.name ?? "Selected term" : "All terms"}</strong><small>{kind === "salary" || kind === "payroll" ? period || "All payroll periods" : `${rows.length > 1 ? rows.length - 1 : 0} records`}</small></div><div><span>Document</span><strong>{docTitle}</strong><small>Generated evidence</small></div></div><div className="evidence-table-wrap"><table><thead><tr>{rows[0]?.map(c => <th key={c}>{c}</th>)}</tr></thead><tbody>{rows.slice(1).map((r, idx) => <tr key={idx}>{r.map((c, j) => <td key={j}>{c}</td>)}</tr>)}</tbody></table></div></>}<footer className="evidence-footer"><span>{schoolName}</span><span>Generated from SukuuNova Finance</span><span>Retain with supporting records</span></footer></section></main>;
}

function downloadBlob(content: string, type: string, filename: string) { const blob = new Blob([content], { type }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = filename; document.body.appendChild(a); a.click(); a.remove(); setTimeout(() => URL.revokeObjectURL(url), 1000); }
