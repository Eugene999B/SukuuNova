...
export default function FinanceEvidencePrint() {
  ...
  function download() {
    ...
    if (format === "csv") {
      ...
      downloadBlob(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(docTitle)}</title><style>body{font:14px Arial;color: var(--sn-ink);padding:36px}header{display:flex;gap:14px;align-items:center;border-bottom: 3px solid var(--sn-line);padding-bottom:16px}header img{width:52px;height:52px;object-fit:contain}h1{margin:0 0 6px}table{border-collapse: collapse;width:100%;margin-top:24px}th,td{border-bottom: 1px solid var(--sn-line);padding:10px;text-align:left}th{background: var(--sn-surface)}</style></head><body><header>${logoUrl ? `<img src="${esc(logoUrl)}" alt="">` : ""}<div><h1>${esc(schoolName)}</h1><div>${esc(docTitle)}</div></div></header><table>${table}</table></body></html>`, "text/html;charset=utf-8", `${kind}-evidence.html`);
    }
  }
  ...
  return <main className="evidence-app">
    <section className="evidence-controls no-print">
      <div><span className="evidence-kicker">FINANCE EVIDENCE CENTRE</span><h2>Professional financial documents</h2><p>Select the record, scope and output, then create a finished document for the office, counter, filing cabinet or reconciliation desk.</p></div>
      <div className="evidence-control-grid">
        ...
        <label>Paper<select value={paper} onChange={e => setPaper(e.target.value as Paper)}><option value="a4">A4 filing</option><option value="thermal">80mm thermal receipt</option></select></label>
