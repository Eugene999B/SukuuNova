...
export async function signOutVisitor(tx: TenantDb, input: { schoolId: string; actorId: string; visitorId: string }) {
  await requirePermission(tx, input.actorId, "visitors:log");
  const visitor = await tx.visitorLog.findFirst({ where: { id: input.visitorId, schoolId: input.schoolId }, select: { id: true, timeOut: true } });
  ...
  const result = await tx.visitorLog.updateMany({ where: { id: visitor.id, schoolId: input.schoolId, timeOut: null }, data: { timeOut } });
  if (result.count !== 1) throw new AppError("Visitor sign-out was already completed by another operator.", 409, "INVALID_STATE");
  await appendSchoolAudit(tx, { schoolId: input.schoolId, actorId: input.actorId, action: "visitor.signed_out", entityType: "VisitorLog", entityId: visitor.id, before: { timeOut: null }, after: { timeOut } });
  return { id: visitor.id, timeOut };
}
