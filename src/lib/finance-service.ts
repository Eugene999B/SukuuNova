...
export async function generateInvoice(
  tx: TenantDb,
  input: { schoolId: string; actorId: string; studentId: string; termId: string }
) {
  ...
  const items = await tx.feeItem.findMany({
    where: {
      termId: input.termId,
      OR: [{ classId: null }, { classId: student.classId ?? "__none__" }]
    }
  });
  ...
  await appendSchoolAudit(tx, {
    ...
    entityType: "Invoice",
    entityId: invoice.id,
    after: { ...invoice, lines: items.map((item) => item.id) }
  });
  return invoice;
}
...
export async function recordPayment(
  ...
  const guardians = await tx.studentGuardian.findMany({
    where: { studentId: result.invoice.studentId, isPrimary: true },
