...
export async function createTimetableSlot(tx: TenantDb, input: { schoolId: string; actorId: string; classId: string; subjectId: string; teacherId: string; dayOfWeek: number; period: number; }) {
  ...
  if (input.dayOfWeek < 0 || input.dayOfWeek > 6 || !Number.isInteger(input.period) || input.period < 1) throw new AppError("Invalid timetable day or period.", 400, "INVALID_TIMETABLE_SLOT");
  ...
  const slot = await tx.timetableSlot.upsert({ where: { schoolId_classId_dayOfWeek_period: { schoolId: input.schoolId, classId: input.classId, dayOfWeek: input.dayOfWeek, period: input.period } }, update: { subjectId: input.subjectId, teacherId: input.teacherId }, create: { schoolId: input.schoolId, classId: input.classId, subjectId: input.subjectId, teacherId: input.teacherId, dayOfWeek: input.dayOfWeek, period: input.period } });
