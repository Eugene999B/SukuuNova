...
export async function processPayrollRun(
  ...
  const plan = await planPayrollRun(tx, input);
  const rendered = await renderPlannedPayslips(plan);
  return commitPayrollRun(tx, { schoolId: input.schoolId, actorId: input.actorId, payrollRunId: input.payrollRunId, rendered });
}

export type PlannedPayslip = { id: string; staffId: string; staffName: string; gross: string; deductions: Array<{ label: string; type: string; value: number; amount: number }>; net: string };
export type PayrollPlan = { runId: string; period: string; schoolName: string; rows: PlannedPayslip[] };

/** Read + validate phase: fast, no writes, safe to run before rendering. */
export async function planPayrollRun(
  tx: TenantDb,
  input: { schoolId: string; actorId: string; payrollRunId: string }
): Promise<PayrollPlan> {
  await requirePermission(tx, input.actorId, "payroll:manage");
  ...
  for (const structure of structures) {
    ...
    rows.push({ id: createId(), staffId: structure.staffId, staffName: structure.staff.name, gross: grossAmount.toString(), deductions, net: net.toString() });
  }
  if (rows.length === 0) throw new AppError("No active staff with salary structures to process.", 409, "NO_PAYSLIPS");
  return { runId: run.id, period: run.period, schoolName: school.name, rows };
}

/** Pure CPU phase: run OUTSIDE any database transaction. */
export async function renderPlannedPayslips(plan: PayrollPlan): Promise<Array<PlannedPayslip & { pdfData: Uint8Array<ArrayBuffer> }>> {
  const rendered: Array<PlannedPayslip & { pdfData: Uint8Array<ArrayBuffer> }> = [];
  for (const row of plan.rows) {
    ...
    rendered.push({ ...row, pdfData });
  }
  return rendered;
}

/** Write phase: short transaction, re-validates the draft claim under lock. */
export async function commitPayrollRun(
  tx: TenantDb,
  input: { schoolId: string; actorId: string; payrollRunId: string; rendered: Array<PlannedPayslip & { pdfData: Uint8Array<ArrayBuffer> }> }
) {
  await requirePermission(tx, input.actorId, "payroll:manage");
  ...
  for (const row of input.rendered) {
    await tx.payslip.create({
      data: {
        ...
        payrollRunId: run.id,
        staffId: row.staffId,
        gross: new Prisma.Decimal(row.gross),
        deductions: row.deductions,
        net: new Prisma.Decimal(row.net),
        pdfUrl: "/api/phase2/payroll/payslips/" + row.id + "/pdf",
        pdfData: row.pdfData
      }
    });
  }
  ...
  await appendSchoolAudit(tx, {
    ...
    entityType: "PayrollRun",
    entityId: run.id,
    after: { period: run.period, payslips: input.rendered.length }
  });
  return updated;
}
