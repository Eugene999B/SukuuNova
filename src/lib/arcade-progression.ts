...
const UNIVERSE_GAMES = ["math", "keyboard-ninja", "force-motion-lab", "word", "comprehension-quest"] as const;

type Context = { schoolId: string; guardianId: string; userId: string };
type ProgressStats = { rounds: number; xp: number; stars: number; correct: number; questions: number; games: number; perfectRounds: number; bossWins: number };
...
function schoolWeekStart(day: string) { const weekday = new Date(`${day}T12:00:00Z`).getUTCDay(); return previousDate(day, weekday === 0 ? 6 : weekday - 1); }
function mission(key: string, title: string, description: string, current: number, target: number, unit: string): Mission { return { key, title, description, current, target, unit, complete: current >= target }; }
function achievement(key: string, symbol: string, title: string, description: string, current: number, target: number): Achievement { return { key, symbol, title, description, current, target, unlocked: current >= target }; }

export function arcadeUniverseLevel(totalXp: number) {
  const safeXp = Math.max(0, Math.floor(totalXp)), levelSize = 250;
  return { level: 1 + Math.floor(safeXp / levelSize), xpIntoLevel: safeXp % levelSize, xpForNextLevel: levelSize, xpRemaining: levelSize - (safeXp % levelSize) };
}
...
export function buildArcadeProgression(stats: ProgressStats, today: ProgressStats, week: ProgressStats, streak: number) {
  ...
  return {
    profile: { totalXp: stats.xp, totalRounds: stats.rounds, totalStars: stats.stars, accuracy, novaCoins: Math.floor(stats.xp / 10) + stats.stars * 2, streak, ...level },
    dailyMissions: [mission("daily-play", "Launch sequence", "Complete one learning mission today.", today.rounds, 1, "mission"), mission("daily-variety", "Double world", "Play two different learning games today.", today.games, 2, "games"), mission("daily-stars", "Star sweep", "Collect five stars today.", today.stars, 5, "stars")],
    weeklyChallenges: [mission("weekly-rounds", "Arcade regular", "Complete five missions this school week.", week.rounds, 5, "missions"), mission("weekly-worlds", "Three-world tour", "Visit three different learning worlds this week.", week.games, 3, "games"), mission("weekly-xp", "Nova charge", "Earn 300 XP this school week.", week.xp, 300, "XP")],
    achievements,
...
export async function guardianArcadeProgression(tx: TenantDb, context: Context, studentId: string) {
  const child = await tx.student.findFirst({ where: { id: studentId, schoolId: context.schoolId, status: "active", guardians: { some: { guardianId: context.guardianId, schoolId: context.schoolId } } }, select: { id: true, class: { select: { level: true } } } });
  ...
  const summarySql = (extra: Prisma.Sql) => tx.$queryRaw<ProgressStats[]>`
    SELECT COUNT(*)::int AS "rounds", COALESCE(SUM("xp"),0)::int AS "xp", COALESCE(SUM("stars"),0)::int AS "stars", COALESCE(SUM("correct"),0)::int AS "correct", COALESCE(SUM("roundLength"),0)::int AS "questions", COUNT(DISTINCT "game")::int AS "games", COUNT(*) FILTER (WHERE "stars"=3)::int AS "perfectRounds", COUNT(*) FILTER (WHERE "stars"=3 AND "difficulty">=5)::int AS "bossWins"
    FROM "ArcadeRound" WHERE "schoolId"=${context.schoolId} AND "studentId"=${child.id} AND "status"='completed' ${extra}
  `;
  const [allRows, todayRows, weekRows, dateRows, championRows] = await Promise.all([
    summarySql(Prisma.empty), summarySql(Prisma.sql`AND "localDate"=${today}`), summarySql(Prisma.sql`AND "localDate">=${weekStart}`),
    ...
    tx.$queryRaw<Array<{ studentId: string; name: string; totalXp: number; totalStars: number; rounds: number }>>`
      SELECT r."studentId",s."name",COALESCE(SUM(r."xp"),0)::int AS "totalXp",COALESCE(SUM(r."stars"),0)::int AS "totalStars",COUNT(*)::int AS "rounds" FROM "ArcadeRound" r
      JOIN "Student" s ON s."id"=r."studentId" AND s."schoolId"=r."schoolId"
      WHERE r."schoolId"=${context.schoolId} AND r."status"='completed' AND s."status"='active' AND r."standardBand"=${standardBand} AND r."game" IN (${gameList})
      GROUP BY r."studentId",s."name" ORDER BY "totalXp" DESC,"totalStars" DESC,"rounds" DESC LIMIT 10
    `,
  ]);
  ...
  return {
    ...
    champions: championRows.map((row, index) => ({ rank: index + 1, studentId: row.studentId, displayName: leaderboardDisplayName(row.name), totalXp: row.totalXp, totalStars: row.totalStars, rounds: row.rounds, isCurrent: row.studentId === child.id })),
