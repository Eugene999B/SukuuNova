...
const DOCUMENTS: Record<string, DemoDocument> = {
  ...
  "pta-family-guide.pdf": {
    ...
    sections: [
      { heading: "Partnership principles", paragraphs: ["Families and school staff share responsibility for learner safety, attendance, communication and encouragement. Questions should be raised early, respectfully and through the channel most likely to resolve them. Sensitive learner matters should not be debated in public group chats."] },
      { heading: "Keeping family information current", paragraphs: ["Tell the school when a guardian phone number, email address, pickup authorization or emergency contact changes. Accurate records help attendance alerts, fee receipts, transport notices and safeguarding communication reach the correct person."] },
      { heading: "Supporting learning at home", paragraphs: ["Check the learner's timetable and published academic work, provide a regular place for study and ask the learner to explain what they learned rather than completing work for them. Contact the teacher when repeated difficulty, missing work or unusual stress becomes visible."] },
      { heading: "Meetings and school events", paragraphs: ["Use published event information and appointment instructions. Arrive through the approved visitor process and sign out when required. Volunteers should follow the same safeguarding, privacy and conduct expectations as other adults on school premises."] },
      { heading: "Finance questions", paragraphs: ["Use the learner's fee statement and official receipt as the starting point for any accounts query. Share transaction references only with authorized staff. If a payment appears missing, ask the accounts team to reconcile it rather than making a second payment without clarification."] },
      {
...
async function renderDocument(document: DemoDocument) {
  ...
  const ensureSpace = (height: number) => { if (y - height < 64) startPage(); };
  ...
  drawParagraph(document.subtitle, { size: 10, color: rgb(0.38, 0.42, 0.48), gap: 14 });
...
export async function renderDemoLibraryPdf(filename: string) {
