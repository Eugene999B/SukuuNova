...
import { assertRate, calculatePlatformTotal, majorUnits, minorUnits } from "./billing-math";
...
export async function getBillingRules(session: PlatformSession, schoolId: string) {
  ...
  return withTenant(schoolId, async (tx) => {
    ...
    const baseMajor = rules.billingMode === "per_student" ? majorUnits(minorUnits(rules.studentRate) * activeStudents) : Number(rules.flatRate);
    const totals = calculatePlatformTotal({ base: baseMajor, discountPercent: rules.discountPercent, minimumCharge: rules.minimumCharge, maximumCharge: rules.maximumCharge, taxPercent: rules.taxPercent });
    const { base, discountedBase: discounted, subtotal: capped, tax, total } = totals;
...
export async function saveBillingRules(session: PlatformSession, input: {
  ...
  assertRate(input.studentRate, "Student rate");
  assertRate(input.flatRate, "Flat rate");
  assertRate(input.minimumCharge, "Minimum charge");
  if (input.maximumCharge !== null) assertRate(input.maximumCharge, "Maximum charge");
  if (!Number.isInteger(input.billingDay) || input.billingDay < 1 || input.billingDay > 28) throw new AppError("Billing day must be between 1 and 28.", 400, "INVALID_BILLING_DAY");
  if (!Number.isInteger(input.graceDays) || input.graceDays < 0 || input.graceDays > 90) throw new AppError("Grace days must be between 0 and 90.", 400, "INVALID_GRACE_DAYS");
  if (!Number.isInteger(input.trialDays) || input.trialDays < 0 || input.trialDays > 365) throw new AppError("Trial days must be between 0 and 365.", 400, "INVALID_TRIAL_DAYS");
  ...
  await withTenant(input.schoolId, async (tx) => {
    ...
    await appendPlatformAudit({ actorId: session.adminId, action: "school.billing_rules.updated", targetSchoolId: input.schoolId, targetEntity: "PlatformSchoolBillingConfig", meta: input }, tx);
  });
...
export async function generateBillingRulesInvoice(session: PlatformSession, schoolId: string, period: string) {
  ...
  const result = await withTenant(schoolId, async (tx) => {
    ...
    if (existing) {
      const result = { ...existing, amount: Number(existing.amount), existing: true };
      await appendPlatformAudit({ actorId: session.adminId, action: "platform.billing.invoice_already_exists", targetSchoolId: schoolId, targetEntity: `PlatformInvoice:${existing.id}`, meta: result }, tx);
      return result;
    }
    ...
    const baseMajor = row ? (row.billingMode === "per_student" ? majorUnits(minorUnits(row.studentRate) * activeStudents) : Number(row.flatRate)) : Number(school.subscriptionPlan?.price ?? 0);
    ...
    const totals = calculatePlatformTotal({ base: baseMajor, discountPercent: row?.discountPercent ?? 0, minimumCharge: row?.minimumCharge ?? 0, maximumCharge: row?.maximumCharge ?? null, taxPercent: row?.taxPercent ?? 0 });
    const discount = totals.base - totals.discountedBase;
    const discounted = totals.discountedBase;
    const floor = row ? Number(row.minimumCharge) : 0;
    const capped = totals.subtotal;
    const tax = totals.tax;
    const amount = totals.total;
    const calculation = { billingMode: row?.billingMode ?? "plan", currency: row?.currency ?? "GHS", activeStudents, baseAmount: totals.base, discountPercent: row ? Number(row.discountPercent) : 0, discountAmount: discount, minimumCharge: floor, cappedSubtotal: capped, taxPercent: row ? Number(row.taxPercent) : 0, taxAmount: tax, finalAmount: amount, invoicePrefix: row?.invoicePrefix ?? "INV", invoiceDueDays: row?.invoiceDueDays ?? row?.graceDays ?? 0, planName: school.subscriptionPlan?.name ?? null };
    let created;
    try {
      created = (await tx.$queryRawUnsafe<Array<{ id: string }>>(`INSERT INTO "PlatformInvoice" ("id","schoolId","period","amount","status","calculation") VALUES ($1,$2,$3,$4,'unpaid',$5::jsonb) RETURNING "id"`, createId(), schoolId, period, amount, JSON.stringify(calculation)))[0];
    } catch (error) {
      if ((error as { code?: string }).code === "23505") {
        const raced = (await tx.$queryRawUnsafe<Array<{ id: string; amount: string; status: string; calculation: unknown }>>(`SELECT "id","amount"::text,"status","calculation" FROM "PlatformInvoice" WHERE "schoolId"=$1 AND "period"=$2 LIMIT 1`, schoolId, period))[0];
        if (raced) return { ...raced, amount: Number(raced.amount), existing: true };
      }
      throw error;
    }
    const result = { id: created.id, amount, status: "unpaid", existing: false, calculation };
    await appendPlatformAudit({ actorId: session.adminId, action: "platform.billing.invoice_generated", targetSchoolId: schoolId, targetEntity: `PlatformInvoice:${created.id}`, meta: result }, tx);
    return result;
  });
  return result;
}
