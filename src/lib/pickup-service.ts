...
export async function attemptPickup(tx: TenantDb, input: { schoolId: string; actorId: string; studentId: string; guardianId: string }) {
  ...
  await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`pickup:${input.schoolId}:${input.studentId}:${input.guardianId}`}))`;
  ...
  if (!student || !guardian) throw new AppError("Student or collecting guardian not found in this school.", 404, "NOT_FOUND");
  if (approved) {
...
export async function reviewPickupRequest(tx: TenantDb, input: { schoolId: string; actorId: string; requestId: string; decision: "approved" | "rejected" }) {
  ...
  await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`pickup:${input.schoolId}:${request.studentId}:${request.collectedByGuardianId}`}))`;
