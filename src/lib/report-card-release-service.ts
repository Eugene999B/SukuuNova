...
function extractChannelConfig(value: Prisma.JsonValue | null | undefined) {
  ...
  const whatsappTemplateConfig = config.whatsappTemplateConfig && !Array.isArray(config.whatsappTemplateConfig) && typeof config.whatsappTemplateConfig === "object" ? config.whatsappTemplateConfig as Record<string, Prisma.JsonValue> : {};
...
function requireProviderConfiguration(channels: Channel[], whatsappTemplateConfig: Record<string, Prisma.JsonValue>) {
  if (channels.includes("sms") && (!process.env.SMS_PROVIDER_URL || !process.env.SMS_PROVIDER_TOKEN)) throw new AppError("SMS is enabled for report-card delivery but the SMS provider is not configured.", 503, "NOTIFICATION_PROVIDER_UNAVAILABLE");
  if (channels.includes("whatsapp")) {
    const configured = whatsappTemplateConfig.report_card_ready;
    const contentSid = typeof configured === "string" ? configured : configured && !Array.isArray(configured) && typeof configured === "object" && typeof (configured as Record<string, Prisma.JsonValue>).contentSid === "string" ? (configured as Record<string, Prisma.JsonValue>).contentSid as string : undefined;
    if (!process.env.TWILIO_ACCOUNT_SID || !process.env.TWILIO_AUTH_TOKEN || !process.env.TWILIO_WHATSAPP_FROM || !contentSid) throw new AppError("WhatsApp report-card delivery is enabled but the Twilio or report-card template configuration is incomplete.", 503, "NOTIFICATION_PROVIDER_UNAVAILABLE");
  }
}
...
async function queuePublicRelease(tx: TenantDb, input: { schoolId: string; actorId: string; reportCardId: string; origin: string }) {
  ...
  const { channels, whatsappTemplateConfig } = extractChannelConfig(settings?.notificationChannels);
...
export async function approveAndQueuePublicReportCard(tx: TenantDb, input: { schoolId: string; actorId: string; reportCardId: string; origin: string }) {
  ...
  const settings = await tx.schoolSettings.findUnique({ where: { schoolId: input.schoolId }, select: { notificationChannels: true } });
  const { config, channels, whatsappTemplateConfig } = extractChannelConfig(settings?.notificationChannels);
  ...
  if (automation.report_card_ready === true && channels.length) requireProviderConfiguration(channels, whatsappTemplateConfig);
  ...
  if (automation.report_card_ready === true && channels.length) { const queued = await queuePublicRelease(tx, input); notification = { attempted: queued.recipientCount, queued: queued.queued, skipped: false, publicPdf: true }; await appendSchoolAudit(tx, { schoolId: input.schoolId, actorId: input.actorId, action: "report_card.automation_queued", entityType: "ReportCard", entityId: report.id, after: { guardianCount: queued.recipientCount, channelCount: queued.channels.length, queued: queued.queued, publicPdf: true } }); }
...
export async function sendApprovedReportCardPublic(tx: TenantDb, input: { schoolId: string; actorId: string; reportCardId: string; origin: string }) {
  ...
  const final = await tx.reportCard.update({ where: { id: queued.report.id }, data: { status: "sent", sentAt } });
  await appendSchoolAudit(tx, { schoolId: input.schoolId, actorId: input.actorId, action: "report_card.released", entityType: "ReportCard", entityId: final.id, before: { status: queued.report.status, sentAt: queued.report.sentAt }, after: { status: final.status, sentAt: final.sentAt, recipientCount: queued.recipientCount, queued: queued.queued, channels: queued.channels, publicPdf: true } });
