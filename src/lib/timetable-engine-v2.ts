...
type DayConfig={dayOfWeek:number;name:string;enabled:boolean;start:string;end:string};
type BreakConfig={name:string;start:string;end:string};
type TimetableConfig={days:DayConfig[];periodMinutes:number;breaks:BreakConfig[];periodsPerDay:number;published:boolean;weeklyPeriods?:Record<string,number>};
...
export function dayBlocks(day:DayConfig,config:TimetableConfig){
  ...
  const periods:LessonPeriod[]=[];let cursor=start,period=1;
...
export async function generateBalancedTimetable(tx:TenantDb,input:{schoolId:string;actorId:string;replaceExisting:boolean;classIds?:string[]}){
  ...
  for(const day of days){const built=dayBlocks(day,timetable);if(!built.periods.length)throw new AppError(`${day.name} has no usable teaching periods after breaks.`,400,"NO_USABLE_PERIODS");periodsByDay.set(day.dayOfWeek,built);}
  ...
  const assignments=await tx.classSubjectTeacher.findMany({where:{classId:{in:classIds}},include:{class:true,subject:true,teacher:true}});
  if(!assignments.length)throw new AppError("Assign subjects to classes and teachers before generating the timetable.",409,"NO_ASSIGNMENTS");
  ...
  const dayLoad=new Map<string,number>(), teacherLoad=new Map<string,number>();
  const placedClass=new Set<string>(), placedTeacher=new Set<string>(), chosen:Array<{schoolId:string;classId:string;subjectId:string;teacherId:string;dayOfWeek:number;period:number}>=[];
  ...
  for(const assignment of work){
    const key=`${assignment.classId}:${assignment.subjectId}:${assignment.teacherId}`;const target=rawWeekly[key]??2;let placed=0;
    for(let occurrence=0;occurrence<target;occurrence++){
      let best:{score:number;day:DayConfig;period:LessonPeriod}|null=null;
      for(const day of days){
        const built=periodsByDay.get(day.dayOfWeek);if(!built)continue;
        for(const period of built.periods){
          const classKey=`${assignment.classId}:${day.dayOfWeek}:${period.period}`;const teacherKey=`${assignment.teacherId}:${day.dayOfWeek}:${period.period}`;
          if(placedClass.has(classKey)||placedTeacher.has(teacherKey))continue;
          const classDay=`${assignment.classId}:${day.dayOfWeek}`,teacherDay=`${assignment.teacherId}:${day.dayOfWeek}`;
          const classCount=dayLoad.get(classDay)??0,teacherCount=teacherLoad.get(teacherDay)??0;
          let score=classCount*4+teacherCount*5;
          const subjectDay=chosen.filter(x=>x.classId===assignment.classId&&x.subjectId===assignment.subjectId&&x.dayOfWeek===day.dayOfWeek).length;
          score+=subjectDay*18;
          const adjacent=chosen.some(x=>x.dayOfWeek===day.dayOfWeek&&x.period===period.period-1&& (x.classId===assignment.classId||x.teacherId===assignment.teacherId));
          if(adjacent)score+=8;
          const sameSubjectAdjacent=chosen.some(x=>x.dayOfWeek===day.dayOfWeek&&Math.abs(x.period-period.period)===1&&x.classId===assignment.classId&&x.subjectId===assignment.subjectId);
          if(sameSubjectAdjacent)score+=14;
          score+=period.period*0.1;
          if(!best||score<best.score)best={score,day,period};
        }
      }
      if(!best)break;
      const chosenSlot={schoolId:input.schoolId,classId:assignment.classId,subjectId:assignment.subjectId,teacherId:assignment.teacherId,dayOfWeek:best.day.dayOfWeek,period:best.period.period};
      chosen.push(chosenSlot);placedClass.add(`${assignment.classId}:${best.day.dayOfWeek}:${best.period.period}`);placedTeacher.add(`${assignment.teacherId}:${best.day.dayOfWeek}:${best.period.period}`);dayLoad.set(`${assignment.classId}:${best.day.dayOfWeek}`,(dayLoad.get(`${assignment.classId}:${best.day.dayOfWeek}`)??0)+1);teacherLoad.set(`${assignment.teacherId}:${best.day.dayOfWeek}`,(teacherLoad.get(`${assignment.teacherId}:${best.day.dayOfWeek}`)??0)+1);placed++;
    }
    if(placed<target)throw new AppError(`I could not fit ${assignment.subject.name} for ${assignment.class.name} ${target} time(s) a week without double-booking the class or teacher. Add another school day, increase the daily window, reduce another subject's frequency, or change a teacher assignment.`,409,"TIMETABLE_UNSATISFIABLE");
  }
  ...
  const result={scheduled:chosen.length,classes:classes.length,teachers:new Set(chosen.map(x=>x.teacherId)).size,days:days.length,periodsPerDay:Math.max(...days.map(d=>periodsByDay.get(d.dayOfWeek)?.periods.length??0)),published:false,breaks:timetable.breaks};
