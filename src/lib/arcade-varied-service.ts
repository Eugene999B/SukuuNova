...
import { createNovaMillionaireQuestions } from "./nova-millionaire-content";
...
function generate(game: string, difficulty: number, length: number, weakKeys: readonly string[] = []): StoredQuestion[] {
  if (game === "logic") return createNovaMillionaireQuestions(difficulty, length);
  ...
  if (game === "culture-heritage") return createStyleStudioQuestions(difficulty, length);
  if (game === "space-explorer") return createSolarNavigatorQuestions(difficulty, length);
