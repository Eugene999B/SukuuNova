...
async function callStructuredAi(prompt: string, allowedEvidence: string[], system: string) {
  ...
  try {
    ...
    throw new AppError("AI provider returned non-JSON output.", 502, "AI_INVALID_OUTPUT");
  }
  ...
  if (structured.data.evidence.some((item) => !allowedEvidence.includes(item))) {
    throw new AppError("AI provider returned evidence not present in deterministic school data.", 502, "AI_INVALID_EVIDENCE");
  }
  return structured.data;
}

export async function createStructuredAiDraft(input: {
  schoolId: string;
  actorId: string;
  type: "lesson_note" | "report_card_remark";
  context: Record<string, unknown>;
}) {
  return requirePermissionAndRun(input, async (tx) => {
    let safe: Record<string, unknown>;
    let evidence: string[];
    let recordIds: string[];

    if (input.type === "report_card_remark") {
      const studentId = text(input.context.studentId, "studentId", 100);
      const termId = text(input.context.termId, "termId", 100);
      ...
      const scores = await tx.score.findMany({
        where: { studentId },
        select: { id: true, value: true, assessment: { select: { maxScore: true } } },
        orderBy: { enteredAt: "desc" },
        take: 8
      });
      const attendance = await tx.attendanceEvent.count({ where: { studentId, type: "in" } });
      const recentAverage = scores.length
        ? Number((scores.reduce((sum, row) => sum + Number(row.value) / Number(row.assessment.maxScore) * 100, 0) / scores.length).toFixed(1))
        : null;
      evidence = [
        `Student: ${student.name}`,
        `Class: ${student.class?.name ?? "Unassigned"}`,
        `Recent score average: ${recentAverage === null ? "no scored assessments" : recentAverage + "%"}`,
        `Recorded attendance check-ins: ${attendance}`
      ];
      ...
      safe = {
        subject: optionalText(input.context.subject, 120),
        topic: text(input.context.topic, "topic", 240),
        objectives: optionalText(input.context.objectives, 1000),
        className: optionalText(input.context.className, 120)
      };
      evidence = [
        `Subject: ${safe.subject ?? "unspecified"}`,
        `Topic: ${safe.topic}`,
        `Class: ${safe.className ?? "unspecified"}`
      ];
      recordIds = [];
    }

    const system =
      "Return JSON only with exactly {draft,evidence,cautions}. draft is the proposed language only. evidence must contain only exact strings from the supplied deterministic evidence list. Do not invent school facts, diagnoses, grades, attendance, or events. cautions should state uncertainty when the evidence is incomplete.";
    const prompt = JSON.stringify({ data: safe, evidence });
    const structured = await callStructuredAi(prompt, evidence, system);
    const requestId = createId();
    const hash = inputHash(safe);
    await tx.$executeRaw`
      INSERT INTO "AiRequest" (
        "id","schoolId","userId","featureName","promptVersion","model","inputRecordIds","inputDataHash","output","approvalStatus"
      ) VALUES (
        ${requestId},${input.schoolId},${input.actorId},${input.type},${PROMPT_VERSION},${providerModel()},
        ${JSON.stringify(recordIds)}::jsonb,${hash},${JSON.stringify(structured)}::jsonb,'SUGGESTED'
      )
    `;

    const draftId = createId();
    await tx.$executeRaw`
      INSERT INTO "AiDraft" ("id","schoolId","type","context","draftText","status")
      VALUES (${draftId},${input.schoolId},${input.type},${JSON.stringify({ ...safe, requestId, evidence, cautions: structured.cautions })}::jsonb,${structured.draft},'suggested')
    `;
    return {
      id: draftId,
      requestId,
      ...
      draftText: structured.draft,
      evidence: structured.evidence,
      cautions: structured.cautions
    };
  });
}

export async function acceptStructuredAiDraft(input: {
  schoolId: string;
  actorId: string;
  draftId: string;
  editedText?: string;
  ...
  return requirePermissionAndRun(input, async (tx) => {
    const row = (await tx.$queryRaw<Array<{ id: string; type: string; context: Record<string, unknown>; draftText: string; status: string }>>`
      SELECT "id","type","context","draftText","status"
      FROM "AiDraft"
      WHERE "id"=${input.draftId} AND "schoolId"=${input.schoolId}
      FOR UPDATE
    `)[0];
    ...
    if (row.type === "report_card_remark") {
      await generateReportCard(tx, {
        schoolId: input.schoolId,
        actorId: input.actorId,
        studentId: text(row.context.studentId, "studentId", 100),
        termId: text(row.context.termId, "termId", 100),
        remarks: finalText
      });
    } else {
    ...
    if (requestId) {
      await tx.$executeRaw`UPDATE "AiRequest" SET "approvalStatus"='ACCEPTED',"approvedAt"=NOW(),"approvedBy"=${input.actorId} WHERE "id"=${requestId} AND "schoolId"=${input.schoolId}`;
    }
...
export async function discardStructuredAiDraft(input: { schoolId: string; actorId: string; draftId: string }) {
  return requirePermissionAndRun(input, async (tx) => {
    const row = (await tx.$queryRaw<Array<{ context: Record<string, unknown> }>>`
      SELECT "context" FROM "AiDraft" WHERE "id"=${input.draftId} AND "schoolId"=${input.schoolId} AND "status"='suggested' FOR UPDATE
    `)[0];
...
async function requirePermissionAndRun<T>(
  input: { schoolId: string; actorId: string },
  work: (tx: TenantDb) => Promise<T>
) {
  return import("./db").then(({ withTenant }) => withTenant(input.schoolId, async (tx) => {
    const allowed = await import("./rbac").then(({ hasPermission }) => hasPermission(tx, input.actorId, "ai_drafts:accept"));
    if (!allowed) throw new ForbiddenError("AI draft access is not permitted.");
    return work(tx);
  }));
}
