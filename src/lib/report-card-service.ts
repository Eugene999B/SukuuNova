...
async function reportData(tx: TenantDb, studentId: string, termId: string) {
  const [student, term, settings, school] = await Promise.all([
    ...
    tx.schoolSettings.findUnique({ where: { schoolId: (await tx.school.findFirst({ select: { id: true } }))?.id ?? "" } }),
    tx.school.findFirst()
  ]);
  ...
  if (missing.length > 0 && !settings.allowPartialReportCards) {
    throw new AppError("Missing scores block report-card generation. Enable partial reports to override.", 409, "MISSING_SCORES");
  }

  const grouped = new Map<string, typeof assessments>();
  for (const assessment of assessments) {
    grouped.set(assessment.subject.name, [...(grouped.get(assessment.subject.name) ?? []), assessment]);
  }

  const caWeight = Number(settings.gradeCaWeight);
  const examWeight = Number(settings.gradeExamWeight);
  if (!Number.isFinite(caWeight) || caWeight < 0 || !Number.isFinite(examWeight) || examWeight < 0 || caWeight + examWeight <= 0) {
    throw new AppError("The school's grading weights are invalid.", 409, "INVALID_GRADING_CONFIGURATION");
  }
  ...
  for (const [subject, rows] of grouped) {
    const average = (type: string) => {
      ...
      return typed.reduce((sum, row) => sum + (Number(row.scores[0].value) / Number(row.maxScore)) * 100, 0) / typed.length;
    };
    const ca = average("ca");
    const exam = average("exam");
    const weightedParts = [
      ca == null ? null : { value: ca, weight: caWeight },
      exam == null ? null : { value: exam, weight: examWeight }
    ].filter((part): part is { value: number; weight: number } => part !== null && part.weight > 0);
    const denominator = weightedParts.reduce((sum, part) => sum + part.weight, 0);
    const total = denominator > 0
      ? Math.round((weightedParts.reduce((sum, part) => sum + part.value * part.weight, 0) / denominator) * 100) / 100
      : null;
    results.push({ subject, ca, exam, total });
  }

  const attendance = await tx.attendanceEvent.findMany({
    where: { studentId, type: "in", attendanceDate: { gte: term.startDate, lte: term.endDate } },
...
async function makePdf(data: Awaited<ReturnType<typeof reportData>>, remarks?: string) {
  if (data.results.length > 20) throw new AppError("This legacy PDF renderer supports at most 20 subjects. Use the Report Card Print Studio for a full multi-page report.", 409, "PDF_LIMIT");
...
export async function generateReportCard(tx: TenantDb, input: { schoolId: string; actorId: string; studentId: string; termId: string; remarks?: string }) {
  ...
  const calculationSnapshot = {
    ...
    assessments: data.results.map((row) => ({ subject: row.subject, ca: row.ca, exam: row.exam, total: row.total })),
    attendance: {
      presentDays: new Set(data.attendance.map((row) => row.attendanceDate.toISOString().slice(0, 10))).size,
      lateDays: data.attendance.filter((row) => row.isLate).length
    }
  };
  const report = await tx.reportCard.upsert({
    where: { studentId_termId: { studentId: input.studentId, termId: input.termId } },
    update: { pdfData, remarks: input.remarks, templateId: data.template.id, calculationSnapshot, calculationVersion: 2 },
    create: { schoolId: input.schoolId, studentId: input.studentId, termId: input.termId, templateId: data.template.id, pdfData, remarks: input.remarks, calculationSnapshot, calculationVersion: 2, generatedPdfUrl: "/api/mvp/report-cards/pending/pdf" }
  });
  const generatedPdfUrl = "/api/mvp/report-cards/" + report.id + "/pdf";
  await tx.reportCard.update({ where: { id: report.id }, data: { generatedPdfUrl } });
...
export async function submitReportCard(tx: TenantDb, input: { schoolId: string; actorId: string; reportCardId: string }) { await requirePermission(tx, input.actorId, "report_cards:submit"); const report = await tx.reportCard.findUnique({ where: { id: input.reportCardId }, include: { student: { include: { class: true } } } }); if (!report) throw new AppError("Report card not found.", 404, "NOT_FOUND"); if (report.status !== "draft") throw new AppError("Only draft reports can be submitted.", 409, "INVALID_STATE"); if (report.student.class?.classTeacherId !== input.actorId) throw new ForbiddenError("Only the student's class teacher may submit this report."); const updated = await tx.reportCard.update({ where: { id: report.id }, data: { status: "submitted", submittedBy: input.actorId, submittedAt: new Date() } }); await appendSchoolAudit(tx, { schoolId: input.schoolId, actorId: input.actorId, action: "report_card.submitted", entityType: "ReportCard", entityId: report.id, before: { status: report.status }, after: { status: updated.status } }); return updated; }

async function autoNotifyApprovedReportCard(tx: TenantDb, input: { schoolId: string; actorId: string; reportCardId: string }) {
  const settings = await tx.schoolSettings.findUnique({ where: { schoolId: input.schoolId }, select: { notificationChannels: true } }); const channelConfig = settings?.notificationChannels; const config = channelConfig && !Array.isArray(channelConfig) && typeof channelConfig === "object" ? channelConfig as Record<string, unknown> : {}; const automation = config.automation && typeof config.automation === "object" ? config.automation as Record<string, unknown> : {}; if (automation.report_card_ready !== true) return { attempted: 0, delivered: 0, skipped: true }; const channelsRaw = config.channels; const channels = Array.isArray(channelsRaw) ? channelsRaw.filter((value): value is "sms"|"whatsapp" => value === "sms" || value === "whatsapp") : []; if (!channels.length) return { attempted: 0, delivered: 0, skipped: true };
  const report = await tx.reportCard.findUnique({ where: { id: input.reportCardId }, include: { student: { include: { guardians: { include: { guardian: true } } } }, term: true } }); if (!report) throw new AppError("Report card not found.", 404, "NOT_FOUND"); const base = (process.env.APP_URL || process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000").replace(/\/$/, ""); const pdfUrl = base + "/api/mvp/report-cards/" + report.id + "/pdf"; const guardians = report.student.guardians.filter((link) => Boolean(link.guardian.phone)); let delivered = 0;
  for (const link of guardians) { const results = await enqueueSms(tx, { schoolId: input.schoolId, recipientType: "guardian", recipientId: link.guardianId, recipientPhone: link.guardian.phone, body: `SukuuNova: ${report.student.name}’s ${report.term.name} report card is ready. It has been added to the parent portal.`, templateKey: "report_card_ready", templateVariables: { "1": report.student.name, "2": report.term.name, "3": pdfUrl }, mediaUrl: pdfUrl }); delivered += results.filter((message) => message.status === "sent").length; }
  await appendSchoolAudit(tx, { schoolId: input.schoolId, actorId: input.actorId, action: "report_card.automation_dispatched", entityType: "ReportCard", entityId: report.id, after: { guardianCount: guardians.length, channelCount: channels.length, delivered } }); return { attempted: guardians.length, delivered, skipped: false };
}

export async function approveReportCard(tx: TenantDb, input: { schoolId: string; actorId: string; reportCardId: string }) {
  await requirePermission(tx, input.actorId, "report_cards:approve"); const report = await tx.reportCard.findUnique({ where: { id: input.reportCardId } }); if (!report) throw new AppError("Report card not found.", 404, "NOT_FOUND"); if (report.status !== "submitted") throw new AppError("Only submitted reports can be approved.", 409, "INVALID_STATE"); if (report.submittedBy === input.actorId) throw new ForbiddenError("The submitter cannot approve the same report."); const updated = await tx.reportCard.update({ where: { id: report.id }, data: { status: "approved", approvedBy: input.actorId, approvedAt: new Date() } }); await appendSchoolAudit(tx, { schoolId: input.schoolId, actorId: input.actorId, action: "report_card.approved", entityType: "ReportCard", entityId: report.id, before: { status: report.status }, after: { status: updated.status } }); const automation = await autoNotifyApprovedReportCard(tx, input); return { ...updated, notification: automation };
}

export async function sendReportCard(tx: TenantDb, input: { schoolId: string; actorId: string; reportCardId: string }) { await requirePermission(tx, input.actorId, "report_cards:approve"); const report = await tx.reportCard.findUnique({ where: { id: input.reportCardId }, include: { student: { include: { guardians: { where: { isPrimary: true }, include: { guardian: true } } } }, term: true } }); if (!report) throw new AppError("Report card not found.", 404, "NOT_FOUND"); if (report.status !== "approved") throw new AppError("Only approved reports can be sent.", 409, "INVALID_STATE"); const path = "/api/mvp/report-cards/" + report.id + "/pdf"; const url = (process.env.APP_URL || process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000").replace(/\/$/, "") + path; for (const link of report.student.guardians) { await enqueueSms(tx, { schoolId: input.schoolId, recipientType: "guardian", recipientId: link.guardianId, recipientPhone: link.guardian.phone, body: "SukuuNova: " + report.student.name + "'s approved report card is available in the parent portal.", templateKey: "report_card_ready", templateVariables: { "1": report.student.name, "2": report.term.name, "3": url }, mediaUrl: url }); } const final = await tx.reportCard.update({ where: { id: report.id }, data: { sentAt: new Date() } }); await appendSchoolAudit(tx, { schoolId: input.schoolId, actorId: input.actorId, action: "report_card.sent", entityType: "ReportCard", entityId: report.id, before: { status: report.status, sentAt: report.sentAt }, after: { status: final.status, sentAt: final.sentAt } }); return final; }

export async function getVisibleReportPdf(tx: TenantDb, input: { actorId: string; reportCardId: string }) { const report = await tx.reportCard.findUnique({ where: { id: input.reportCardId }, include: { student: { include: { guardians: { include: { guardian: true } } } } } }); const pdfData = report?.pdfData; if (!report || !pdfData) throw new AppError("Report PDF not found.", 404, "NOT_FOUND"); if (await hasPermission(tx, input.actorId, "report_cards:view")) { const isParent = await hasPermission(tx, input.actorId, "parents:read_linked"); if (!isParent) return { ...report, pdfData }; const linked = report.student.guardians.some((link) => link.guardian.userId === input.actorId); if (linked && ["approved", "sent"].includes(report.status)) return { ...report, pdfData }; } throw new ForbiddenError("This report card is not visible to this account."); }
