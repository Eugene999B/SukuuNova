...
function hex(value: string) {
  ...
  if (!match) return rgb(0.11, 0.3, 0.72);
  return rgb(
...
async function reportData(tx: TenantDb, studentId: string, termId: string) {
  ...
  const template = await tx.reportCardTemplate.findUnique({
    where: { id: settings.reportCardTemplateId ?? "preset-classic-blue" }
  });
...
async function makePdf(data: Awaited<ReturnType<typeof reportData>>, remarks?: string) {
  const raw = object(data.template.layoutConfig);
  const brand = object(data.school.brandColors);
  const config: TemplateConfig = {
    style: text(raw.style, "classic"),
    primary: text(brand.primary, text(raw.primary, "#1d4ed8")),
    accent: text(brand.accent, text(raw.accent, "#dbeafe")),
    watermark: data.settings.reportCardWatermark || text(raw.watermark, "SUKUUNOVA")
  };
  ...
  const accent = hex(config.accent);

  if (config.style === "modern") {
    page.drawRectangle({ x: 0, y: 750, width: 595, height: 92, color: primary });
  } else if (config.style === "formal") {
    page.drawRectangle({ x: 35, y: 35, width: 525, height: 772, borderColor: primary, borderWidth: 2 });
  } else {
    page.drawRectangle({ x: 0, y: 770, width: 595, height: 72, color: accent });
  }
  ...
  if (data.school.logoUrl?.startsWith("data:image/")) {
    try {
      ...
      // Branding should never make a report impossible to generate.
    }
  }

  if (config.watermark) {
    page.drawText(config.watermark.slice(0, 36), {
      x: 145,
      y: 390,
      size: 42,
      font: bold,
  ...
  page.drawText(data.school.name, { x: 50, y: 798, size: 20, font: bold, color: headerColor });
  page.drawText("SukuuNova Report Card · " + data.template.name, {
    x: 50, y: 774, size: 10, font: regular, color: headerColor
  });
  page.drawText("Student: " + data.student.name, { x: 50, y: 730, size: 11, font: bold });
  page.drawText("Class: " + (data.student.class?.name ?? "Unassigned"), {
    x: 320, y: 730, size: 11, font: regular
  });
  page.drawText("Term: " + data.term.name, { x: 50, y: 710, size: 11, font: regular });
  page.drawRectangle({ x: 45, y: 664, width: 500, height: 24, color: accent });
  page.drawText("Subject", { x: 50, y: 675, size: 10, font: bold, color: primary });
  page.drawText("CA", { x: 310, y: 675, size: 10, font: bold, color: primary });
  page.drawText("Exam", { x: 380, y: 675, size: 10, font: bold, color: primary });
  page.drawText("Total", { x: 465, y: 675, size: 10, font: bold, color: primary });
  ...
  page.drawText("Generated securely by SukuuNova", {
    x: 50, y: 60, size: 8, font: regular, color: rgb(0.4, 0.4, 0.4)
  });
...
export async function generateReportCard(
  ...
  const report = await tx.reportCard.upsert({
    where: { studentId_termId: { studentId: input.studentId, termId: input.termId } },
    update: { pdfData, remarks: input.remarks, templateId: data.template.id },
    create: {
  ...
  await appendSchoolAudit(tx, {
    schoolId: input.schoolId, actorId: input.actorId, action: "report_card.generated",
    entityType: "ReportCard", entityId: report.id,
    after: { studentId: input.studentId, termId: input.termId, templateId: data.template.id }
  });
...
export async function submitReportCard(
  ...
  const report = await tx.reportCard.findUnique({
    where: { id: input.reportCardId }, include: { student: { include: { class: true } } }
  });
  ...
  await appendSchoolAudit(tx, {
    ...
    entityType: "ReportCard", entityId: report.id,
    before: { status: report.status }, after: { status: updated.status }
  });
  return updated;
}
...
export async function sendReportCard(
  ...
  for (const link of report.student.guardians) {
    await enqueueSms(tx, {
      ...
      body: "SukuuNova: " + report.student.name + "'s approved report card is available at " + url,
      templateKey: "report_card_ready",
      templateVariables: { "1": report.student.name, "2": url },
      mediaUrl: url
    });
  }
  const updated = await tx.reportCard.update({
    where: { id: report.id }, data: { status: "sent", sentAt: new Date() }
  });
  await appendSchoolAudit(tx, {
    schoolId: input.schoolId, actorId: input.actorId, action: "report_card.sent",
    entityType: "ReportCard", entityId: report.id,
    before: { status: report.status }, after: { status: updated.status }
  });
  return updated;
