...
function SecurityBody({ name, required, target }: { name: string; required: boolean; target: string }) {
  return <div className="security-centre">
    <div className="security-hero"><div><span className="security-kicker">Account protection</span><h2>{required ? "Secure your account before continuing" : "Your SukuuNova security centre"}</h2><p>{required ? "A temporary password is active on this account. Replace it now with a private password." : `Review the protection of ${name}'s SukuuNova account from one place.`}</p></div><div className="security-score"><span>Security posture</span><strong>{required ? "Action required" : "Protected"}</strong><small>{required ? "Password update required" : "Password controls available"}</small></div></div>
    <div className="security-grid security-grid-top"><section className="security-card security-card-password"><div className="security-card-head"><div><span className="security-label">Credentials</span><h3>Change password</h3><p>Use a unique password and keep it private.</p></div><span className="security-icon">●</span></div><form action={changePassword} className="security-form"><label>Current password<input name="currentPassword" type="password" autoComplete="current-password" required /></label><label>New password<input name="newPassword" type="password" autoComplete="new-password" minLength={12} required /></label><label>Confirm new password<input name="confirmPassword" type="password" autoComplete="new-password" minLength={12} required /></label><button type="submit">Update password</button></form></section><section className="security-card"><div className="security-card-head"><div><span className="security-label">Protection</span><h3>Security checklist</h3><p>Active protections available on this account.</p></div><span className="security-status-good">Ready</span></div><div className="security-checklist"><div><span className="check">✓</span><div><strong>Password protection</strong><small>Active for this account</small></div></div><div><span className="check">✓</span><div><strong>School-scoped access</strong><small>Permissions are governed by the school workspace</small></div></div><div><span className="pending">•</span><div><strong>Multi-factor authentication</strong><small>Not enabled for this account</small></div></div><div><span className="pending">•</span><div><strong>Session & device review</strong><small>Not available yet</small></div></div></div></section></div>
    <div className="security-grid security-grid-three"><section className="security-card"><span className="security-label">Account recovery</span><h3>Recovery & sign-in</h3><p>Keep recovery information and sign-in protections easy to find.</p><div className="security-link-list"><Link href="/account/security">Password & recovery <span>→</span></Link><Link href={target === "school workspace" ? "/school/settings/access" : "/account/security"}>Access & delegated accounts <span>→</span></Link></div></section><section className="security-card"><span className="security-label">Access governance</span><h3>Who can reach school data?</h3><p>Review roles, delegated administrators and the scope of staff access.</p><div className="security-link-list"><Link href="/school/settings/roles">Roles & permissions <span>→</span></Link><Link href="/school/settings/access">Sub-accounts & access <span>→</span></Link></div></section><section className="security-card"><span className="security-label">Security operations</span><h3>Recommended controls</h3><p>Centralize the actions that protect high-value school information.</p><ul className="security-mini-list"><li>Require stronger passwords for privileged roles</li><li>Review dormant accounts before term changes</li><li>Use least-privilege roles for delegated administrators</li><li>Keep communication and data exports permission-controlled</li></ul></section></div>
    <section className="security-card security-future"><div><span className="security-label">Security roadmap</span><h3>Advanced account controls</h3><p>MFA, passkeys, active-session revocation, recovery codes, trusted devices, security alerts and privileged-action re-authentication remain future controls and are not presented as active features.</p></div><div className="security-future-grid"><span>Multi-factor</span><span>Passkeys</span><span>Active sessions</span><span>Device trust</span><span>Recovery codes</span><span>Security alerts</span></div></section>
  </div>;
}
