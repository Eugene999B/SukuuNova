...
export default async function TeacherPortalPage({ searchParams }: Props) {
  ...
  return <AppShell universe="teacher" title="Teacher workspace" subtitle="Your classes, subjects, attendance, marks, homework, planning and school conversations." active="Teacher Home" userName={data.name} schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} role={roleLabel}>
    <div className="staff-workspace">
      <section className="staff-hero"><div><span className="staff-eyebrow">TEACHER PORTAL · {roleLabel}</span><h2>Good morning, {data.name.split(/\s+/)[0]}.</h2><p>This workspace is separate from school administration. Your teaching scope comes from the classes and subjects assigned to your staff profile.</p><div className="staff-hero-points"><span>✓ {classIds.size} classes in scope</span><span>✓ {assignments.length} subject assignments</span><span>✓ Mark attendance</span><span>✓ Enter marks</span></div></div><div className="flex flex-wrap gap-3"><Link className="staff-primary-cta" href="/teacher/attendance">Take class attendance →</Link><Link className="staff-primary-cta" href="/school/attendance/check-in">Check in to school →</Link></div></section>
      <section className="staff-metrics"><article><span>Classes</span><strong>{classIds.size}</strong><small>Only your assigned groups</small></article><article><span>Subjects</span><strong>{new Set(assignments.map((a) => a.subject.id)).size}</strong><small>Teaching areas</small></article><article><span>Students in scope</span><strong>{totalStudents}</strong><small>Across assigned groups</small></article><Link href="/teacher/messages" className="staff-card" style={{ textDecoration: "none" }}><span>My messages</span><strong>{data.messageCount}</strong><small>Open your inbox</small></Link></section>
