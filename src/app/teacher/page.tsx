...
export default async function TeacherPortalPage() {
  ...
  return <AppShell universe="school" title="Teacher workspace" subtitle="A focused teaching cockpit: only your classes, subjects, attendance, marks, homework and conversations." active="Staff & Teachers" userName={data.name} schoolName="School Workspace" schoolCode="" role={roles[0] ?? "Teacher"}>
    <div className="staff-workspace">
      ...
      <section className="staff-metrics"><article><span>Classes</span><strong>{classIds.size}</strong><small>Only your assigned groups</small></article><article><span>Subjects</span><strong>{new Set(assignments.map((a) => a.subject.id)).size}</strong><small>Teaching areas</small></article><article><span>Students in scope</span><strong>{scopedStudentCount}</strong><small>Across assigned groups</small></article><article><span>Messages</span><strong>0</strong><small>Parent conversations ready for inbox</small></article></section>
