  ...
  return <AppShell universe="teacher" title="Teacher workspace" subtitle="Your classes, subjects, attendance, marks, homework, planning and school conversations." active="Teacher Home" userName={data.name} schoolName="School Workspace" schoolCode="" role={roleLabel}>
    <div className="staff-workspace">
      <section className="staff-hero">
        ...
        <Link className="staff-primary-cta" href="/teacher/module?view=My%20Homework">＋ Create homework</Link>
      </section>
      <section className="staff-metrics"><article><span>Classes</span><strong>{classIds.size}</strong><small>Only your assigned groups</small></article><article><span>Subjects</span><strong>{new Set(assignments.map((a) => a.subject.id)).size}</strong><small>Teaching areas</small></article><article><span>Students in scope</span><strong>{scopedStudentCount}</strong><small>Across assigned groups</small></article><article><span>My messages</span><strong>{data.messageCount}</strong><small>Messages addressed to you</small></article></section>
      <section className="staff-command-grid">
        ...
        <article className="staff-card"><div className="staff-card-head"><div><span>Today</span><h3>Teaching actions</h3><p>Fast links for the work you do most.</p></div></div><div className="staff-role-stack"><Link href="/teacher/module?view=My%20Attendance" className="staff-link-grid"><span>Take attendance</span><span>→</span></Link><Link href="/teacher/module?view=My%20Gradebook" className="staff-link-grid"><span>Enter marks</span><span>→</span></Link><Link href="/teacher/module?view=My%20Homework" className="staff-link-grid"><span>Homework & exercises</span><span>→</span></Link><Link href="/teacher/module?view=My%20Timetable" className="staff-link-grid"><span>My timetable</span><span>→</span></Link><Link href="/teacher/module?view=My%20Messages" className="staff-link-grid"><span>My messages</span><span>→</span></Link><Link href="/account/security" className="staff-link-grid"><span>Account security</span><span>→</span></Link></div></article>
      </section>
      <section className="staff-card"><div className="staff-card-head"><div><span>Classroom workflow</span><h3>Assignment → submission → marking</h3><p>Homework can be tied to an assigned class and subject. Future family submissions can return through the same class-scoped workflow for marking and feedback.</p></div></div><div className="staff-link-grid"><Link href="/teacher/module?view=My%20Homework">Create assignment <span>→</span></Link><Link href="/teacher/module?view=My%20Messages">Student / family conversation <span>→</span></Link><Link href="/teacher/module?view=My%20Assessments">Report-card connection <span>→</span></Link></div></section>
    </div>
  </AppShell>;
}
