...
export default function TeacherHomeworkPage() {
  ...
  return (
    <AppShell universe="teacher" title="My Homework" subtitle="Homework for your classes." active="My Homework">
      <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
          <div><span className="text-xs font-bold uppercase tracking-widest">Teacher · Homework</span><h1 className="mt-1 text-3xl font-bold">My homework</h1></div>
          <div className="flex flex-wrap gap-3"><Link href="/teacher" className="rounded-xl border px-4 py-2 font-semibold">Teacher home →</Link>{data?.manage ? <button type="button" className="rounded-xl bg-slate-900 px-4 py-2 font-semibold text-white" onClick={() => setCreateOpen(true)}>+ New homework</button> : null}</div>
        </div>
        ...
        <Dialog open={createOpen} onClose={() => setCreateOpen(false)} title="New homework" description="Draft work for one of your assigned classes." size="md">
          <form className="grid gap-4" onSubmit={createHomework}>
            ...
            <div className="flex justify-end gap-3"><button type="button" className="rounded-xl border px-4 py-3 font-semibold" onClick={() => setCreateOpen(false)}>Cancel</button><button className="rounded-xl bg-slate-900 px-4 py-3 font-semibold text-white">Save draft →</button></div>
          </form>
        </Dialog>
          <section className="rounded-2xl border bg-white p-6 shadow-sm"><span className="text-xs font-bold uppercase tracking-widest">My assignments</span><h2 className="mt-1 text-xl font-bold">Homework already created</h2><div className="mt-5 grid gap-3">{data.rows.map((row) => <article key={row.id} className="rounded-xl border p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><strong>{row.title}</strong><p className="text-sm text-slate-600">{row.className} · {row.subjectName}{row.termName ? ` · ${row.termName}` : ""}</p></div><span className="rounded-full border px-2 py-1 text-xs font-semibold">{row.assignmentStatus}</span></div><p className="mt-3 whitespace-pre-wrap text-sm text-slate-700">{row.instructions}</p><small className="mt-3 block text-slate-500">Due {new Date(row.dueDate).toLocaleDateString()} · Review: {row.reviewStatus}{row.points != null ? ` · ${row.points} points` : ""}</small></article>)}{data.rows.length === 0 ? <div className="rounded-xl border border-dashed p-6 text-sm text-slate-600"><strong>No homework yet.</strong><p className="mt-1">Choose one of your assigned class-subjects and create your first draft.</p></div> : null}</div></section>
        </>}
      </main>
    </AppShell>
  );
