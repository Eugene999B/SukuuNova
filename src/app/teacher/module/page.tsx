...
export default async function TeacherModulePage({ searchParams }: Props) {
  ...
  const data = await withTenant(session.schoolId, (tx) => tx.user.findUnique({
  ...
  return (
    <AppShell universe="teacher" title={copy.title} subtitle={copy.description} active={view} userName={data.name} schoolName="School Workspace" role={roles[0] ?? "Teacher"}>
