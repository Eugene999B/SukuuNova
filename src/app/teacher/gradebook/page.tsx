    ...
    <AppShell universe="teacher" title="My Gradebook" subtitle="Choose your assigned class, subject and week, then open the worksheet." active="My Gradebook" schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} userName={session.name} role={data.role || "Teacher"}>
      <div className="teacher-workspace weekly-gradebook-home">
        <section className="weekly-gradebook-hero">
          <div>
            <span className="teacher-eyebrow">TEACHER · GRADEBOOK</span>
            <h2>Enter marks without setting up a complicated assessment.</h2>
            <p>SukuuNova already knows the active term. Choose only a class assigned to you, its assigned subject, and the teaching week.</p>
          </div>
          <div className="weekly-active-term">
            <span>ACTIVE TERM</span>
            <strong>{data.activeTerm?.name ?? "No active term"}</strong>
            <small>{data.activeTerm?.academicYear.name ?? "School calendar controls the working term"}</small>
          </div>
        </section>

        {!data.activeTerm ? <section className="teacher-surface weekly-alert"><strong>No writable academic term is active today.</strong><p>School leadership must configure the academic calendar before teachers can enter new marks.</p></section> : null}
        {!data.assignments.length ? <section className="teacher-surface weekly-alert"><strong>No teaching assignment is available.</strong><p>Only class-subject combinations assigned to this teacher account can appear in the gradebook.</p></section> : null}

        <section className="teacher-surface weekly-launcher-surface">
        ...
        {data.endedOpenTerms.length ? <section className="teacher-surface weekly-history-note"><span className="teacher-eyebrow">LEADERSHIP ACTION NEEDED</span><h3>{data.endedOpenTerms.length} ended term{data.endedOpenTerms.length === 1 ? " is" : "s are"} still unlocked.</h3><p>Teachers stay on the current term automatically. Leadership can review and close older terms from Academic Terms.</p></section> : null}
