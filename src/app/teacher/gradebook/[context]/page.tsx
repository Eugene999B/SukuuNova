...
export default async function TeacherGradebookContextPage({ params, searchParams }: {
  ...
  const data = await withTenant(session.schoolId, async (tx) => {
    ...
    const results = performance ? performance.rows.map((row) => {
      const result = calculateSubjectResult(row.scores.map((score) => ({ id: score.assessmentId, name: score.name, type: score.type, maxScore: score.maxScore, weight: score.weight, score: score.rawScore, status: score.status })), performance.config);
      return { student: row.student, ...result };
    }) : [];

    return { school, assignment, selectedTerm, activeTerm, performance, workRows, readOnly, terms: config.terms, gradeScale: scale, teachingWeeks, results, weights: performance ? assessmentBucketWeights(performance.config) : { ca: 0, exam: 0 } };
  });
  ...
  const weekRows = assessmentRows.filter((assessment) => assessment.work?.weekNumber === selectedWeek).sort((a, b) => (a.work?.workNumber ?? 0) - (b.work?.workNumber ?? 0));
  ...
  const selectedScoreRows = selectedAssessment && data.performance ? data.performance.rows.map((row) => ({ student: row.student, expected: row.scores.find((score) => score.assessmentId === selectedAssessment.id)?.expected ?? null })) : [];
  ...
  return <AppShell universe="teacher" title="My Gradebook" subtitle="Weekly worksheets for the classes and subjects assigned to you." active="My Gradebook" schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} userName={session.name} role="Teacher">
    <div className="module-workspace teacher-gradebook-redesign">
      <section className="weekly-context-card">
        <div><span>ASSIGNED GRADEBOOK</span><h2>{data.assignment.class.name} · {data.assignment.subject.name}</h2><p>{data.selectedTerm ? `${data.selectedTerm.name} · ${data.readOnly ? "Read-only history" : "Active term"}` : "No active term"} · {learnerCount} learners</p></div>
      ...
      {!data.selectedTerm || !data.performance ? <section className="module-card module-empty"><strong>No academic term is available.</strong><p>School leadership controls the calendar. A writable worksheet appears automatically when a term is active.</p></section> : <>
        ...
        {view === "worksheet" ? <section className="weekly-sheet" id="marks">
          <div className="weekly-sheet-head">
            <div><span>WEEKLY MARKS</span><h1>Week {selectedWeek} Worksheet</h1><p>Open an existing work to continue where you stopped, or add another work for this same week.</p></div>
