...
export default async function TeacherTimetablePage() {
  ...
  return <AppShell universe="teacher" title="My timetable" subtitle="Your timetable." active="My Timetable" schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} userName={session.name} role="Teacher">
    <div className="teacher-workspace">
      <section className="teacher-page-head"><div><span className="teacher-eyebrow">TEACHER · TIMETABLE</span><h2>My timetable</h2></div><Link className="teacher-primary-action" href="/teacher">Teacher home →</Link></section>
      <section className="teacher-scope-strip"><div><span>Scheduled periods</span><strong>{data.slots.length}</strong></div></section>
