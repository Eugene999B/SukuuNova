...
export default async function TeacherStudentsPage() {
  ...
  return (
    <AppShell universe="teacher" title="My Students" subtitle="Your students." active="My Students" schoolName="School Workspace" userName={data.name} role={data.userRoles[0]?.role.name ?? "Teacher"}>
      <div className="staff-workspace">
        <section className="staff-header"><div><span className="staff-eyebrow">TEACHER PORTAL · MY STUDENTS</span><h2>My student roster</h2><p>This list is calculated from your assigned class relationships.</p></div><Link className="staff-primary-cta" href="/teacher">← Teacher home</Link></section>
        <section className="staff-metrics"><article><span>Students in scope</span><strong>{students.length}</strong></article><article><span>Classes in scope</span><strong>{classIds.length}</strong></article></section>
        <section className="staff-directory"><div className="staff-directory-head"><div><span>Roster</span><h3>Students you teach</h3></div><div className="staff-tools"><Link href="/teacher/attendance">Attendance</Link><Link href="/teacher/homework">Homework</Link><Link href="/teacher/gradebook">Gradebook</Link></div></div>{students.length === 0?<div className="staff-empty"><strong>No students are currently in your teaching scope.</strong></div>:<div className="staff-table-wrap"><table><thead><tr><th>Student</th><th>Admission no.</th><th>Class</th><th>Status</th></tr></thead><tbody>{students.map((student)=><tr key={student.id}><td><div className="staff-person"><span>{student.name.split(/\s+/).map((part)=>part[0]).slice(0,2).join("").toUpperCase()}</span><div><b>{student.name}</b><small>{student.dob ? `DOB · ${new Date(student.dob).toLocaleDateString("en-GB")}` : "Learner profile"}</small></div></div></td><td><small>{student.admissionNo}</small></td><td><small>{student.class ? `${student.class.level ? `${student.class.level} · ` : ""}${student.class.name}` : "Unassigned"}</small></td><td><span className={`staff-status ${student.status}`}>{student.status}</span></td></tr>)}</tbody></table></div>}</section>
      </div>
    </AppShell>
  );
}
