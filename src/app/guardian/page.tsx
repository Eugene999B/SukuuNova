...
export default async function GuardianPortalPage() {
  ...
  const balance = children.reduce(
    (n, s) => n + s.invoices.reduce(
      (sum, inv) => sum + Number(inv.totalAmount) - inv.payments.reduce((paid, p) => paid + Number(p.amount), 0),
      0,
    ),
    0,
  );
  ...
  return (
    <AppShell
      ...
      subtitle="Attendance, school updates, results and fees for the children connected to you."
      active="Overview"
    ...
    >
      <div className="guardian-command-center">
        <section className="guardian-hero app-card">
          <div>
            <span className="guardian-eyebrow">Family workspace</span>
            <h2>Good morning, {firstName}.</h2>
            <p>Everything here is limited to your linked children. Start with the child or task you need today.</p>
          </div>
          <div className="guardian-balance">
            <span>Family balance</span>
            <strong>GH₵{balance.toFixed(2)}</strong>
            <Link href="/guardian/fees">View fees <ArrowRight size={13} aria-hidden="true" /></Link>
          </div>
        </section>
        ...
        <section className="guardian-panel app-card">
          <div className="guardian-panel-head"><div><span className="guardian-eyebrow">Your children</span><h2>Open a child</h2><p>Attendance, results and fees are available from each child’s page.</p></div></div>
          <div className="guardian-child-grid">
                ...
                <article className="guardian-child" key={student.id}>
                  <div className="guardian-child-head">
                    <div className="guardian-child-avatar">
                      {student.photoUrl ? <Image src={student.photoUrl} alt="" width={54} height={54} unoptimized /> : student.name.slice(0, 2).toUpperCase()}
                    </div>
            ...
            }) : <div className="guardian-empty"><strong>No children are linked yet.</strong><p>Ask your school to connect your guardian account to a learner.</p></div>}
          </div>
        </section>
      </div>
