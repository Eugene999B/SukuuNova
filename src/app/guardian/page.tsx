...
export default async function GuardianPortalPage() {
  ...
  return <AppShell universe="guardian" title="Family overview" subtitle="Your children, school updates and next steps." active="Overview" schoolName={session.schoolName} schoolCode="" userName={data.guardian.name} role="Guardian">
    <div className="gd2">
      <section className="gd2-hero"><div className="gd2-hero-copy"><span>YOUR FAMILY AT SCHOOL</span><h1>Welcome back, {firstName}.</h1><p>See how the school day is going, read a message, or check a payment. Choose a child below for their details.</p><div className="gd2-hero-actions"><Link className="gd2-button primary" href={children[0]?`/guardian/children/${children[0].id}`:"/guardian/children"}>Open learner profiles <ArrowRight size={13}/></Link><Link className="gd2-button" href="/guardian/messages">Family inbox <MessageSquareText size={13}/></Link></div></div><div className="gd2-hero-orbit"><div><strong>{children.length}</strong><span>linked learners</span></div></div></section>
      ...
      <section className="gd2-kpis">
        ...
        <article className="gd2-kpi"><header><span>Family balance</span><WalletCards size={16}/></header><strong>GH₵{familyBalance.toFixed(2)}</strong><p>What remains to pay after recorded payments and adjustments.</p><Link href="/guardian/fees">View fees and payments →</Link></article>
