...
export default async function GuardianModulePage({ params, searchParams }: Props) {
  ...
  const studentFinance = (student:typeof data.children[number]) => {
    const billed=student.invoices.reduce((sum,invoice)=>sum.plus(new Prisma.Decimal(String(invoice.totalAmount))),new Prisma.Decimal(0));
  ...
  const studentAcademic = (student:typeof data.children[number]) => {
    ...
    return {scores,average,latest,reports:student.reportCards.length};
  };
  const studentAttendance = (student:typeof data.children[number]) => {
    ...
    const latest=student.attendanceEvents[0];
    return {incoming,outgoing,days,latest,total:student.attendanceEvents.length};
  };

  const finances=data.children.map(studentFinance);
  ...
  const totalResults=data.children.reduce((sum,student)=>sum+visibleScores(student).length,0);
  const totalReports=data.children.reduce((sum,student)=>sum+student.reportCards.length,0);
  const totalAttendanceDays=data.children.reduce((sum,student)=>sum+studentAttendance(student).days,0);
  const totalCheckins=data.children.reduce((sum,student)=>sum+studentAttendance(student).incoming.length,0);
  ...
  const metric = (label:string,value:string|number,detail:string,icon:React.ReactNode,meter?:number) => <article className="gmv-kpi"><div className="gmv-kpi-head"><span>{label}</span>{icon}</div><strong>{value}</strong><p>{detail}</p>{meter!==undefined?<div className="gmv-meter"><span style={{width:`${meter}%`}}/></div>:null}</article>;
  const avatar = (student:{name:string;photoUrl?:string|null}) => <span className="gmv-avatar">{student.photoUrl?<Image src={student.photoUrl} width={50} height={50} alt="" unoptimized/>:student.name.slice(0,2).toUpperCase()}</span>;
...
function metricLocal(label:string,value:string|number,detail:string,icon:React.ReactNode,meter?:number){return <article className="gmv-kpi"><div className="gmv-kpi-head"><span>{label}</span>{icon}</div><strong>{value}</strong><p>{detail}</p>{meter!==undefined?<div className="gmv-meter"><span style={{width:`${meter}%`}}/></div>:null}</article>}
