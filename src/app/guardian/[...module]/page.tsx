...
export default async function GuardianModulePage({ params }: Props) {
  ...
  const data = await withTenant(session.schoolId, async (tx) => {
    ...
    if (childId) {
      ...
      const child = await tx.student.findFirst({ where: { id: childId, schoolId: session.schoolId }, include: { class: true, attendanceEvents: true, scores: { include: { subject: true, assessment: true } }, reportCards: true, invoices: { include: { payments: true } } } });
      return { guardian, child, children: [] };
    }
    const children = await tx.student.findMany({ where: { id: { in: ids }, schoolId: session.schoolId }, orderBy: { name: "asc" }, include: { class: true, attendanceEvents: true, scores: { include: { subject: true, assessment: true } }, invoices: { include: { payments: true } } } });
  ...
  const totalBalance = childData.reduce((n, s) => n + s.invoices.reduce((sum, inv) => sum + Number(inv.amount) - inv.payments.reduce((p, x) => p + Number(x.amount), 0), 0), 0);
