...
import { Banknote, CircleDollarSign, CreditCard, FileText, ReceiptText, TrendingUp, UsersRound, WalletCards } from "lucide-react";
...
type Invoice = { id:string; studentId:string; termName:string; academicYearName:string; totalAmount:Prisma.Decimal; status:string; createdAt:Date };
...
const money = (value:Prisma.Decimal) => `GH₵${Number(value).toLocaleString("en-GH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
...
export default async function GuardianFeesPage({ searchParams }:Props) {
  ...
  const data=await withTenant(session.schoolId,async(tx)=>{
    ...
    const [learners,invoices,lines,payments,reversals]=await Promise.all([
      ...
      tx.$queryRawUnsafe<Invoice[]>(`SELECT i."id",i."studentId",t."name" AS "termName",ay."name" AS "academicYearName",i."totalAmount",i."status",i."createdAt" FROM "Invoice" i INNER JOIN "StudentGuardian" sg ON sg."studentId"=i."studentId" AND sg."schoolId"=i."schoolId" INNER JOIN "Term" t ON t."id"=i."termId" AND t."schoolId"=i."schoolId" INNER JOIN "AcademicYear" ay ON ay."id"=t."academicYearId" AND ay."schoolId"=i."schoolId" WHERE i."schoolId"=$1 AND sg."guardianId"=$2 AND ($3::text IS NULL OR i."studentId"=$3) ORDER BY i."createdAt" DESC`,session.schoolId,session.guardianId,selectedId),
  ...
  const netPayment=(payment:Payment)=>nonNegative(payment.amount.minus((reversalsByPayment.get(payment.id)||[]).reduce((sum,row)=>sum.plus(row.amount),zero())));
  ...
  const outstanding=nonNegative(totalBilled.minus(totalPaid));
  ...
  const learnerSummary=(learner:Learner)=>{
    ...
    return {invoices,billed,paid,due:nonNegative(billed.minus(paid)),coverage:billed.gt(0)?Math.max(0,Math.min(100,Number(paid.div(billed).mul(100).toFixed(0)))):100};
  };
  ...
  return <AppShell universe="guardian" title="Fees & receipts" subtitle="A complete family finance picture, broken down by learner, invoice, fee item and payment." active="Fees & Receipts" schoolName={session.schoolName} schoolCode="" userName={data.family.guardian.name} role="Guardian">
    <div className="gfi-page">
      <section className="gfi-hero"><div><span>FEES & RECEIPTS</span><h1>Your family fees</h1><p>View each learner’s charges, payments, outstanding balance and receipts.</p></div></section>
      ...
      <section className="gfi-section"><header><div><span>INVOICE INTELLIGENCE</span><h2>What was charged?</h2><p>Every invoice expands into its actual fee items, payments and remaining balance.</p></div></header>{data.invoices.length?<div className="gfi-invoices">{data.invoices.map(invoice=>{const learner=data.learners.find(row=>row.id===invoice.studentId);const rows=linesByInvoice.get(invoice.id)||[];const paid=invoicePaid(invoice.id);const due=nonNegative(invoice.totalAmount.minus(paid));const coverage=invoice.totalAmount.gt(0)?Math.max(0,Math.min(100,Number(paid.div(invoice.totalAmount).mul(100).toFixed(0)))):100;return <article key={invoice.id} className="gfi-invoice"><div className="gfi-invoice-head"><div><span className={`gfi-status ${due.lte(0)?"paid":paid.gt(0)?"partial":"unpaid"}`}>{due.lte(0)?"Paid":paid.gt(0)?"Part paid":statusLabel(invoice.status)}</span><h3>{learner?.name||"Linked learner"} · {invoice.academicYearName} · {invoice.termName}</h3><p>Issued {shortDate(invoice.createdAt)} · Invoice {invoice.id.slice(-8).toUpperCase()}</p></div><div><strong>{money(due)}</strong><span>remaining</span></div></div><div className="gfi-invoice-progress"><i><b style={{width:`${coverage}%`}}/></i><span>{coverage}% covered</span></div><div className="gfi-lines">{rows.map(line=><div key={line.feeItemId}><span>{line.name}</span><strong>{money(line.amount)}</strong></div>)}<div className="total"><span>Total billed</span><strong>{money(invoice.totalAmount)}</strong></div></div><div className="gfi-invoice-payments">{(paymentsByInvoice.get(invoice.id)||[]).length?(paymentsByInvoice.get(invoice.id)||[]).map(payment=>{const reversed=(reversalsByPayment.get(payment.id)||[]).reduce((sum,row)=>sum.plus(row.amount),zero());return <div key={payment.id}><span className="gfi-payment-icon">{payment.method==="cash"?<Banknote size={15}/>:<CreditCard size={15}/>}</span><div><strong>{money(netPayment(payment))} net payment</strong><p>{methodLabel(payment.method)} · {shortDate(payment.createdAt)} · Ref {payment.reference||payment.id.slice(-8).toUpperCase()}</p>{reversed.gt(0)?<small>{money(reversed)} reversed: {(reversalsByPayment.get(payment.id)||[]).map(row=>row.reason).join(" · ")}</small>:null}</div><Link className="gfi-receipt-link" href={`/guardian/fees/receipt/${encodeURIComponent(payment.id)}`}><ReceiptText size={13}/>Receipt</Link></div>}):<p className="gfi-no-payment">No payment has been posted against this invoice yet.</p>}</div></article>})}</div>:<div className="gfi-empty"><ReceiptText size={28}/><strong>No fee invoices in this learner scope</strong><p>When the school issues fees, the full breakdown will appear here.</p></div>}</section>
    </div>
  </AppShell>;
}
