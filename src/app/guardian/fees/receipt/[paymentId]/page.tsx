...
const methodLabel=(value:string)=>value==="momo"?"Mobile Money":value==="card"?"Card / Bank":value==="cash"?"Cash":value.replaceAll("_"," ").replace(/\b\w/g,c=>c.toUpperCase());
...
export default async function GuardianReceiptPage({params}:Props){
  ...
  const data=await withTenant(session.schoolId,async(tx)=>{
    const rows=await tx.$queryRawUnsafe<ReceiptRow[]>(`SELECT p."id",p."invoiceId",i."studentId",COALESCE(rs."snapshot"->>'studentName',s."name") AS "studentName",COALESCE(rs."snapshot"->>'admissionNo',s."admissionNo") AS "admissionNo",CASE WHEN rs."paymentId" IS NOT NULL THEN rs."snapshot"->>'className' ELSE c."name" END AS "className",COALESCE(rs."snapshot"->>'termName',t."name") AS "termName",i."totalAmount" AS "invoiceTotal",i."status" AS "invoiceStatus",p."amount",p."method",p."reference",p."createdAt" FROM "Payment" p INNER JOIN "Invoice" i ON i."id"=p."invoiceId" AND i."schoolId"=p."schoolId" INNER JOIN "Student" s ON s."id"=i."studentId" AND s."schoolId"=i."schoolId" INNER JOIN "StudentGuardian" sg ON sg."studentId"=s."id" AND sg."schoolId"=s."schoolId" LEFT JOIN "PaymentReceiptSnapshot" rs ON rs."paymentId"=p."id" AND rs."schoolId"=p."schoolId" LEFT JOIN "Enrollment" en ON en."studentId"=i."studentId" AND en."termId"=i."termId" AND en."schoolId"=i."schoolId" LEFT JOIN "Class" c ON c."id"=COALESCE(i."classId",en."classId") AND c."schoolId"=i."schoolId" INNER JOIN "Term" t ON t."id"=i."termId" AND t."schoolId"=i."schoolId" WHERE p."schoolId"=$1 AND p."id"=$2 AND sg."guardianId"=$3 LIMIT 1`,session.schoolId,paymentId,session.guardianId);
    ...
    return {school,receipt,lines,reversals,paymentTotal:paymentTotal[0]?.total??zero(),reversalTotal:reversalTotal[0]?.total??zero()};
  });
  if(!data)notFound();
  ...
  const schoolName=data.school?.name||session.schoolName;
  const fullyPaid=remaining.lte(0);

  return <AppShell universe="guardian" title="Payment receipt" subtitle="Official school finance record." active="Fees & Receipts" schoolName={schoolName} schoolCode="" userName={session.name} role="Guardian">
    <div className="gfr-page">
      <div className="gfr-actions"><Link href={`/guardian/fees?studentId=${encodeURIComponent(data.receipt.studentId)}`}><ArrowLeft size={14}/>Back to fees</Link><GuardianReceiptPrintButton/></div>
      <article className="gfr-paper" aria-label={`Payment receipt ${receiptNo}`}>
        <header className="gfr-document-head">
          <div className="gfr-school-identity"><div className="gfr-school-mark">{data.school?.logoUrl?<img src={data.school.logoUrl} alt={`${schoolName} logo`}/>:<span>{schoolName.split(/\s+/).map(part=>part[0]).join("").slice(0,2).toUpperCase()}</span>}</div><div><span className="gfr-brand">OFFICIAL SCHOOL RECEIPT</span><h1>{schoolName}</h1><p>Finance Department · School code {data.school?.uniqueCode||"—"}</p></div></div>
          <div className="gfr-receipt-mark"><span className="gfr-document-type">PAYMENT RECEIPT</span><strong>{receiptNo}</strong><span>Issued {dateTime(data.receipt.createdAt)}</span></div>
        </header>

        <section className="gfr-status-row"><div className="gfr-status-seal"><CheckCircle2 size={20}/><span><b>PAYMENT RECORDED</b><small>Verified in the school finance ledger</small></span></div><div className="gfr-paid-hero"><span>Amount received</span><strong>{money(netTransaction)}</strong><small>{reversed.gt(0)?`${money(reversed)} reversed from the original ${money(data.receipt.amount)} payment`:`Original transaction ${money(data.receipt.amount)}`}</small></div></section>
        ...
        <section className={`gfr-settlement ${fullyPaid?"paid":"open"}`}><CheckCircle2 size={20}/><div><strong>{fullyPaid?"Invoice fully settled":"Payment accepted — balance remains"}</strong><p>{fullyPaid?"No outstanding amount remains on this invoice after recorded payments and reversals.":`${money(remaining)} remains outstanding on ${invoiceNo}.`}</p></div></section>

        {data.reversals.length?<section className="gfr-reversals"><div className="gfr-section-title"><div><span>AUDIT ADJUSTMENTS</span><h2>Payment reversal history</h2></div><b>{data.reversals.length} record{data.reversals.length===1?"":"s"}</b></div>{data.reversals.map(row=><div key={row.id}><strong>-{money(row.amount)}</strong><span>{row.reason}</span><small>{dateTime(row.createdAt)}</small></div>)}</section>:null}

        <section className="gfr-verification"><ShieldCheck size={20}/><div><strong>Protected and auditable</strong><p>This document was generated from the school’s SukuuNova finance ledger for a learner linked to the signed-in guardian. Reversals remain separate audit records; the original payment is never silently rewritten.</p></div><div><span>Receipt record</span><b>{shortId(data.receipt.id)}</b><span>Invoice record</span><b>{shortId(data.receipt.invoiceId)}</b></div></section>

        <footer className="gfr-footer"><div><ReceiptText size={15}/><span>{schoolName} · Official payment receipt</span></div><p>Keep this receipt for your records. For payment questions, contact the school finance office and quote <b>{receiptNo}</b>.</p></footer>
      </article>
    </div>
  </AppShell>;
}
