...
type Ticket = { id: string; subject: string; status: string; createdAt: string; raisedByUserId: string; messages?: Array<{ senderId: string; body: string; sentAt: string }> };
...
export default function SupportConsole() {
  ...
  async function selectSchool(id: string) {
    setSchoolId(id); setTicketId(""); setMessage("");
  ...
  async function updateTicket() {
    ...
    try {
      ...
      setMessage(response.ok ? "Case updated and the action was audited." : (data.error ?? "Could not update the case."));
      if (response.ok) { setBody(""); await selectSchool(schoolId); }
    } finally { setLoading(false); }
  }
  ...
  async function impersonate() {
    ...
    try {
      ...
      const data = await response.json() as { error?: string; expiresInSeconds?: number };
      setMessage(response.ok ? `Support access approved for ${Math.round((data.expiresInSeconds ?? 0) / 60)} minutes and audited.` : (data.error ?? "Could not start support access."));
  ...
  return <div className="support-page-stack">
    ...
    <div className="support-workspace">
      <section className="app-card app-panel support-cases"><div className="support-section-head"><div><h2>Case queue</h2><p>Keep the school context fixed while you work through cases.</p></div><select aria-label="School" value={schoolId} onChange={(event) => void selectSchool(event.target.value)}><option value="">Choose school…</option>{schools.map((school) => <option key={school.id} value={school.id}>{school.name} · {school.uniqueCode}</option>)}</select></div>
        ...
        <div className="support-case-list">{visibleTickets.map((ticket) => <button type="button" className={`support-case ${ticketId===ticket.id ? "is-selected" : ""}`} key={ticket.id} onClick={() => { setTicketId(ticket.id); setStatus((ticket.status as TicketStatus) || "in_progress"); }}><span className="support-case-icon"><MessageSquare size={15}/></span><span className="support-case-copy"><b>{ticket.subject}</b><small>{ticket.id} · opened {new Date(ticket.createdAt).toLocaleDateString()} · {ticket.messages?.length ?? 0} messages</small></span><span className={`support-badge support-badge-${ticket.status}`}>{STATUS_LABEL[ticket.status as TicketStatus] ?? ticket.status}</span></button>)}{visibleTickets.length===0&&<div className="support-empty">{schoolId ? "No cases match the current view." : "Choose a school to load its support queue."}</div>}</div>
      </section>

      <section className="app-card app-panel support-detail"><div className="support-section-head"><div><h2>{selectedTicket ? selectedTicket.subject : "Case workspace"}</h2><p>{selectedTicket ? `${selectedTicket.id} · ${STATUS_LABEL[selectedTicket.status as TicketStatus] ?? selectedTicket.status}` : "Select a case from the queue to see its evidence and controls."}</p></div>{selectedTicket && <Link className="app-pill" href={`/platform/schools/${schoolId}`}>School 360</Link>}</div>{selectedTicket ? <><div className="support-conversation">{(selectedTicket.messages ?? []).map((item,index)=><div className="support-message" key={`${item.sentAt}-${index}`}><small>{item.senderId} · {new Date(item.sentAt).toLocaleString()}</small><p>{item.body}</p></div>)}{!(selectedTicket.messages?.length) && <div className="support-empty">No messages were attached to this case yet.</div>}</div><div className="support-action-box"><label><span>Status</span><select value={status} onChange={(event)=>setStatus(event.target.value as TicketStatus)}>{Object.entries(STATUS_LABEL).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label><span>Reply / operator note</span><textarea value={body} onChange={(event)=>setBody(event.target.value)} placeholder="Record the next useful action or reply…" rows={4}/></label><button type="button" className="app-action" disabled={loading} onClick={()=>void updateTicket()}><CheckCircle2 size={14}/><strong>Save case update</strong>Write to audit trail</button></div></> : <div className="support-empty large">The detail pane becomes the working surface once a case is selected.</div>}</section>
