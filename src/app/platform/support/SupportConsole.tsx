...
export default function SupportConsole() {
  ...
  return <div className="support-page-stack">
    <div className="support-header"><div><span className="platform-eyebrow">Operations desk</span><h2>Support command</h2></div><button type="button" className="app-pill" onClick={() => void loadSchools()} disabled={loading}><RefreshCw size={14}/> Refresh</button></div>
    ...
    <div className="support-workspace">
      <section className="app-card app-panel support-cases"><div className="support-section-head"><div><h2>Case queue</h2></div><select aria-label="School" value={schoolId} onChange={(event) => void selectSchool(event.target.value)}><option value="">Choose school…</option>{schools.map((school) => <option key={school.id} value={school.id}>{school.name} · {school.uniqueCode}</option>)}</select></div>
      ...
      <section className="app-card app-panel support-detail"><div className="support-section-head"><div><h2>{selectedTicket ? selectedTicket.subject : "Case workspace"}</h2><p>{selectedTicket ? `${selectedTicket.id} · ${STATUS_LABEL[selectedTicket.status as TicketStatus] ?? selectedTicket.status}` : "Select a case from the queue to see its evidence and controls."}</p></div>{selectedTicket && <Link className="app-pill" href={`/platform/schools/${schoolId}`}>School 360</Link>}</div>{selectedTicket ? detailLoading && !ticketDetail ? <div className="support-empty large">Loading case evidence…</div> : <><div className="support-conversation">{(ticketDetail?.messages ?? []).map((item)=><div className="support-message" key={item.id}><small>{item.senderId} · {new Date(item.sentAt).toLocaleString()}</small><p>{item.body}</p></div>)}{!ticketDetail?.messages?.length && <div className="support-empty">No messages were attached to this case.</div>}</div><div className="support-action-box"><label><span>Status</span><select value={status} onChange={(event)=>setStatus(event.target.value as TicketStatus)}>{Object.entries(STATUS_LABEL).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label><span>Reply / operator note</span><textarea value={body} onChange={(event)=>setBody(event.target.value)} placeholder="Record the next useful action or reply…" rows={4}/></label><button type="button" className="app-action" disabled={loading || detailLoading} onClick={()=>void updateTicket()}><CheckCircle2 size={14}/><strong>Save case update</strong>Write to audit trail</button></div></> : <div className="support-empty large">Select a case from the queue.</div>}</section>

      <section className="app-card app-panel support-access"><div className="support-section-head"><div><h2>Audited access</h2></div><ShieldAlert size={20} color="var(--sn-muted)"/></div><p>Time-limited, reason-gated entry to a school workspace.</p><button type="button" className="app-action" onClick={() => setAccessOpen(true)}><ShieldAlert size={14}/><strong>Start temporary access</strong></button><PlatformWorkflowDialog open={accessOpen} onClose={() => setAccessOpen(false)} eyebrow="BREAK-GLASS ACCESS" title="Enter school workspace" description="The session is reason-gated, time-limited, visibly marked to the school and written to both platform and school audit logs."><div className="platform-dialog-form"><label><span>School user</span><select value={targetUser} onChange={(event)=>setTargetUser(event.target.value)} disabled={!schoolId}><option value="">Choose user…</option>{users.map((user)=><option key={user.id} value={user.id}>{user.name} · {user.email || user.phone || "No contact"}</option>)}</select></label><label><span>Reason</span><textarea value={reason} onChange={(event)=>setReason(event.target.value)} placeholder="Why is support access necessary?" rows={4}/></label><button type="button" className="app-action" disabled={loading || !schoolId || !targetUser || reason.trim().length < 5} onClick={()=>void impersonate()}><ShieldAlert size={14}/><strong>Start temporary support access</strong>Audit the session</button>{message && <div className="support-result" role="status">{message}</div>}</div></PlatformWorkflowDialog></section>
    </div>
    <style jsx global>{`\n      .support-page-stack{display:grid;gap:16px;margin-top:22px}.support-header{display:flex;align-items:flex-end;justify-content:space-between;gap:16px}.support-header h2{margin:6px 0 5px;font-size:28px;letter-spacing:-.04em;color:var(--sn-ink)}.support-header p{margin:0;max-width:780px;color:var(--sn-ink);font-size:12px;line-height:1.5}.support-workspace{display:grid;grid-template-columns:minmax(0,1.15fr) minmax(0,1.2fr) minmax(280px,.65fr);gap:16px;align-items:start}.support-cases,.support-detail,.support-access{overflow:hidden}.support-section-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;padding:18px 20px 14px}.support-section-head h2{margin:0;font-size:15px;color:var(--sn-ink)}.support-section-head p{margin:4px 0 0;font-size:10px;color:var(--sn-ink);line-height:1.45}.support-section-head select{min-width:170px;height:34px;border:1px solid var(--sn-line);border-radius:9px;background:var(--sn-surface);color:var(--sn-ink);font:inherit;font-size:9.5px;padding:0 8px}.support-toolbar{display:grid;gap:9px;padding:0 20px 12px}.support-search{display:flex;align-items:center;gap:8px;height:36px;border:1px solid var(--sn-line);border-radius:9px;background:var(--sn-surface);padding:0 10px;color:var(--sn-ink)}.support-search input{border:0;outline:0;background:transparent;min-width:0;flex:1;font:inherit;font-size:10px}.support-status-tabs{display:flex;gap:5px;flex-wrap:wrap}.support-status-tabs button{height:28px;padding:0 9px;border:1px solid var(--sn-line);border-radius:999px;background:var(--sn-surface);color:var(--sn-ink);font-size:8.5px;font-weight:800;cursor:pointer}.support-status-tabs button.is-active{background:var(--sn-surface);border-color:var(--sn-line);color:var(--sn-ink)}.support-case-list{border-top:1px solid var(--sn-line)}.support-case{display:grid;grid-template-columns:29px minmax(0,1fr) auto;align-items:center;gap:9px;width:100%;padding:11px 20px;border:0;border-bottom:1px solid var(--sn-line);background:var(--sn-surface);text-align:left;cursor:pointer}.support-case:hover{background:var(--sn-surface)}.support-case:disabled{cursor:wait}.support-case.is-selected{background:var(--sn-surface)}.support-case-icon{display:grid;place-items:center;width:29px;height:29px;border-radius:8px;background:var(--sn-surface);color:var(--sn-ink)}.support-case-copy{min-width:0}.support-case-copy b{display:block;font-size:10px;color:var(--sn-ink);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.support-case-copy small{display:block;margin-top:2px;color:var(--sn-ink);font-size:8px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.support-badge{display:inline-flex;padding:4px 6px;border-radius:999px;font-size:7.5px;font-weight:850;white-space:nowrap}.support-badge-open{background:var(--color-warning-soft);color:var(--color-warning)}.support-badge-in_progress{background:var(--sn-surface);color:var(--sn-ink)}.support-badge-resolved{background:var(--color-success-soft);color:var(--color-success)}.support-empty{display:grid;place-items:center;gap:5px;min-height:140px;padding:25px;text-align:center;color:var(--sn-ink);font-size:10px}.support-empty.large{min-height:260px}.support-conversation{padding:0 20px 12px;max-height:410px;overflow:auto;border-top:1px solid var(--sn-line)}.support-message{padding:11px 0;border-bottom:1px solid var(--sn-line)}.support-message small{display:block;color:var(--sn-ink);font-size:8px}.support-message p{margin:4px 0 0;color:var(--sn-ink);font-size:10px;line-height:1.55;white-space:pre-wrap}.support-action-box{display:grid;gap:9px;padding:14px 20px;background:var(--sn-surface);border-top:1px solid var(--sn-line)}.support-action-box label,.support-access label{display:grid;gap:5px}.support-action-box label>span,.support-access label>span{font-size:8.5px;font-weight:850;color:var(--sn-ink);text-transform:uppercase;letter-spacing:.07em}.support-action-box textarea,.support-access textarea{resize:vertical}.support-access{padding-bottom:18px}.support-access>.support-section-head{padding-bottom:12px}.support-access label{padding:0 20px;margin-top:10px}.support-access .app-action{margin:12px 20px 0}.support-result{margin:12px 20px 0;padding:10px;border:1px solid var(--sn-line);border-radius:9px;background:var(--sn-surface);color:var(--sn-ink);font-size:9px;line-height:1.45}.support-page-stack input,.support-page-stack select,.support-page-stack textarea{box-sizing:border-box;border:1px solid var(--sn-line);border-radius:9px;background:var(--sn-surface);color:var(--sn-ink);font:inherit;font-size:10px;padding:9px 10px;outline:none}.support-page-stack input:focus,.support-page-stack select:focus,.support-page-stack textarea:focus{border-color:var(--sn-line);box-shadow:0 0 0 3px var(--sn-shadow-sm)}@media(max-width:1150px){.support-workspace{grid-template-columns:minmax(0,1fr) minmax(0,1fr)}.support-access{grid-column:1 / -1}}@media(max-width:760px){.support-header{align-items:flex-start}.support-workspace{grid-template-columns:1fr}.support-access{grid-column:auto}.support-section-head{flex-direction:column}.support-section-head select{width:100%}}\n    `}</style>
  </div>;
}
