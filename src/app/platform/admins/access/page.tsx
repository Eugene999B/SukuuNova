...
export default function WorkerAccessPage(){
 ...
 return <AppShell universe="platform" active="Worker School Scope" title="Worker school scope" subtitle="Access review.">
  <div className="app-dashboard-grid">
   <section className="app-card app-panel">
    <div className="app-card-head"><div><span className="app-eyebrow">STEP 1 · OPERATOR</span><h2>Choose a worker</h2></div><Link href="/platform/admins" className="app-pill">Manage workers</Link></div>
   ...
   <section className="app-card app-panel">
    <div className="app-card-head"><div><span className="app-eyebrow">STEP 2 · TENANT SCOPE</span><h2 id="allowed-schools-heading">Allowed schools</h2></div><button className="app-action" disabled={saving||!workerId||isProtected||worker?.status!=="active"} onClick={()=>void save()}><strong>{saving?"Saving…":"Save scope"}</strong>Apply access</button></div>
    ...
    {message&&<div className="app-banner" style={{marginTop:12}><div><h3>{message}</h3></div></div>}
   </section>

   <section className="app-card app-panel" style={{gridColumn:"1/-1"}><div className="app-card-head"><div><span className="app-eyebrow">STEP 3 · VERIFY</span><h2>Effective access summary</h2></div></div><div className="app-dashboard-grid" style={{gridTemplateColumns:"repeat(4,minmax(0,1fr))"}><div className="app-kpi"><span>School access</span><strong>{isProtected?"All":selected.length}</strong><small>{isProtected?"Super Admin boundary":`${data?.schools.length?Math.round(selected.length/data.schools.length*100):0}% of network`}</small></div><div className="app-kpi"><span>Blocked schools</span><strong>{isProtected?"—":Math.max(0,(data?.schools.length??0)-selected.length)}</strong><small>{isProtected?"Not scope-limited":"No customer-school access"}</small></div><div className="app-kpi"><span>Effective permissions</span><strong style={{fontSize:18}>{permissionCount}</strong><small>Role: {worker?.role??"—"}</small></div><div className="app-kpi"><span>Account state</span><strong style={{fontSize:18}>{worker?.status??"—"}</strong><small>{isProtected?"Protected identity":"Scope editor requires active account"}</small></div></div>{worker&&worker.permissions.length>0&&<div style={{marginTop:14}><span className="app-eyebrow">PERMISSION SET</span><div style={{display:"flex",gap:7,flexWrap:"wrap",marginTop:8}>{worker.permissions.map(permission=><span className="app-pill" key={permission}>{permission}</span>)}</div></div>}</section>
  </div>
 </AppShell>;
}
