  ...
  return (
    <main className="platform-auth-shell">
      <section className="platform-auth-card">
        ...
        <label className="platform-auth-field"><span>New password</span><input value={pw} onChange={(e) => setPw(e.target.value)} type="password" placeholder="At least 6 characters" autoComplete="new-password" /></label>
        <button className="platform-auth-submit" disabled={!email.trim() || token.length !== 6 || pw.length < 6} onClick={async () => { const r = await fetch("/api/auth/platform/reset", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ mode: "confirm", email, token, newPassword: pw }) }); const d = await r.json() as { message?: string; error?: string }; setMsg(d.message || d.error || "Request failed"); }}>
