...
export default function PlanManager() {
  ...
  return <div style={{ display: "grid", gap: 18, marginTop: 20 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1.05fr) minmax(360px,.95fr)", gap: 18 }}>
      ...
      <section className="app-card app-panel" style={{ padding: 20 }}><div className="app-card-head"><div><h2>Assign entitlements</h2><p>Choose a plan, then apply it directly to an authorised school.</p></div></div><div style={{ display: "grid", gap: 10, marginBottom: 12 }}><input aria-label="Search schools" value={schoolQuery} onChange={(event) => setSchoolQuery(event.target.value)} placeholder="Search school name or code"/><select aria-label="Plan to assign" value={selectedPlan} onChange={(event) => setSelectedPlan(event.target.value)}><option value="">Choose a plan…</option>{plans.map((plan) => <option key={plan.id} value={plan.id}>{plan.name} · {money(plan.price)}/mo</option>)}</select></div><div style={{ display: "grid", gap: 5, maxHeight: 420, overflow: "auto" }}>{visibleSchools.map((school) => <div key={school.id} className="app-list-row"><div style={{ minWidth: 0, flex: 1 }}><b>{school.name}</b><span>{school.uniqueCode} · {school.subscriptionPlan?.name || "No plan assigned"}</span></div><button type="button" className="app-pill" disabled={!selectedPlan || busy || school.subscriptionPlan?.id === selectedPlan} onClick={() => void assignPlan(school.id, selectedPlan)}>{school.subscriptionPlan?.id === selectedPlan ? "Assigned" : "Assign"}</button></div>)}{visibleSchools.length === 0 ? <div style={{ padding: 28, textAlign: "center", color: "var(--sn-muted)" }}>No schools match this search.</div> : null}</div><div style={{ marginTop: 12, padding: 11, borderRadius: 11, background: "var(--sn-surface-2)", color: "var(--sn-muted)", fontSize: 10, lineHeight: 1.5 }}>Subscription operations benefit from clear packaging, centralized management, customer history and entitlement-aware access controls. citeturn210887search0turn210887search2</div><Link href="/platform/billing" className="app-action" style={{ marginTop: 10 }}><strong>Open billing</strong><ArrowRight size={14}/></Link></section>
    </div>
  </div>;
}
