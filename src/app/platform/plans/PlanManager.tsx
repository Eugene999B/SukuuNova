...
export default function PlanManager() {
  ...
  const assignedCount = useMemo(() => schools.filter((school) => Boolean(school.subscriptionPlan?.id)).length, [schools]);
  const selectedPlanRecord = plans.find((plan) => plan.id === selectedPlan) ?? null;

  return <div className="platform-plans-v3 platform-page-stack">
    ...
    <section className="platform-page-header">
      <div><span className="platform-eyebrow">Commercial packaging</span><h2>Plans & entitlements</h2><p>Build clear packages, see adoption at a glance, and assign each school the capabilities it is entitled to use.</p></div>
    ...
    <div className="app-grid kpis platform-kpis">
      ...
      <div className="app-card app-kpi"><div className="app-kpi-top"><span className="app-kpi-label">Assigned schools</span><span className="app-kpi-icon"><Check size={17}/></span></div><div className="app-kpi-value">{assignedCount}</div><div className="app-kpi-meta">Schools with an active package</div></div>
      <div className="app-card app-kpi"><div className="app-kpi-top"><span className="app-kpi-label">Need assignment</span><span className="app-kpi-icon"><CircleHelp size={17}/></span></div><div className="app-kpi-value">{Math.max(0, schools.length - assignedCount)}</div><div className="app-kpi-meta">Schools without a package</div></div>
      <div className="app-card app-kpi"><div className="app-kpi-top"><span className="app-kpi-label">Available entitlements</span><span className="app-kpi-icon"><Sparkles size={17}/></span></div><div className="app-kpi-value">{ALL_FEATURES.length}</div><div className="app-kpi-meta">Product capabilities available to package</div></div>
    </div>
    <section className="app-card app-panel platform-catalog-card">
      <div className="app-card-head"><div><span className="app-eyebrow">Plan catalog</span><h2>Packages at a glance</h2><p>Price, adoption and enabled capabilities stay visible together so commercial changes are easier to review.</p></div></div>
      ...
      <div className="platform-plan-list">{visiblePlans.map((plan) => { const planFlags = flagsOf(plan); const adoption = schools.filter((school) => school.subscriptionPlan?.id === plan.id).length; return <div className="platform-plan-row" key={plan.id}><div className="platform-plan-title"><span className="platform-workflow-icon"><Layers3 size={17}/></span><div><b>{plan.name}</b><small>{planFlags.length ? `${planFlags.length} entitlements enabled` : "Core-only package"}</small></div></div><strong>{money(plan.price)}<small>/ month</small></strong><span className="app-pill">{adoption} schools</span><div className="platform-plan-entitlements">{planFlags.slice(0,5).map((flag) => <span className="app-pill" key={flag}>{flag.replaceAll("_", " ")}</span>)}{planFlags.length > 5 ? <span className="app-pill">+{planFlags.length - 5} more</span> : null}</div><button type="button" className="app-pill" onClick={() => { setSelectedPlan(plan.id); setDialog("assign"); }}>Assign plan <ArrowRight size={13}/></button></div>; })}{visiblePlans.length === 0 ? <div className="platform-empty"><strong>{plans.length ? "No plans match this search." : "No plans have been created yet."}</strong><span>{plans.length ? "Clear the search to see the full catalog." : "Create the first package, choose its capabilities, then assign it to a school."}</span></div> : null}</div>
    </section>
    <section className="platform-plan-next app-card app-panel">
      <div><span className="platform-eyebrow">Next step</span><h2>{schools.length - assignedCount > 0 ? `${schools.length - assignedCount} school${schools.length - assignedCount === 1 ? "" : "s"} still need a package` : "Assign or change a school package"}</h2></div>
