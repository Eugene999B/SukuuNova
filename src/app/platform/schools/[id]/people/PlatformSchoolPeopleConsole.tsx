...
export default function PlatformSchoolPeopleConsole({ schoolId, people, canImpersonate }: { schoolId: string; people: Person[]; canImpersonate: boolean }) {
  ...
  return <div className="school-people-console">
    ...
    <aside className="app-card app-panel people-support-panel">
      <div className="app-card-head"><div><span className="app-eyebrow">CONTROLLED SUPPORT</span><h2>{selected ? selected.name : "Select a person"}</h2>{selected ? <p>{`${selected.role} · ${selected.status}`}</p> : null}</div><ShieldCheck size={20}/></div>
      {selected ? <>
    ...
    <style jsx global>{`
      ...
      .people-toolbar label{display:block;flex:1}.people-toolbar input{width:100%;min-height:42px;border: 1px solid var(--sn-line);border-radius: 11px;padding:9px 11px;font:inherit;font-size:12px;color: var(--sn-ink);background: var(--color-surface);outline: none}.people-toolbar input:focus{border-color: var(--sn-line);box-shadow: 0 0 0 3px var(--sn-shadow-sm)}
      .people-table{border-top: 1px solid var(--sn-line)}
      .people-row{display:grid;grid-template-columns:36px minmax(0,1fr) minmax(100px,.5fr) auto 16px;gap:11px;align-items:center;width:100%;padding:13px 22px;border: 0;border-bottom: 1px solid var(--sn-line);background: var(--color-surface);text-align:left;cursor:pointer;color: var(--sn-ink)}
      .people-row:hover{background: var(--sn-surface)}.people-row.is-selected{background: var(--sn-surface);box-shadow: inset 3px 0 0 var(--sn-shadow-sm)}.people-avatar{display:grid;place-items:center;width:36px;height:36px;border-radius: 10px;background: var(--sn-surface);color: var(--sn-ink);font-weight:900;font-size:10px}.people-primary{min-width:0;display:grid;gap:3px}.people-primary strong{font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.people-primary small,.people-role{font-size:10px;color: var(--sn-muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.people-status{padding:4px 8px;border-radius: 999px;font-size:9px;font-weight:900;text-transform:capitalize;background: var(--sn-surface);color: var(--sn-ink)}.people-status-active{background: var(--sn-surface);color: var(--sn-ink)}.people-status-suspended,.people-status-inactive{background: var(--sn-surface);color: var(--sn-ink)}
      .people-detail-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:0 0 18px}.people-detail-grid>div{display:grid;gap:4px;padding:11px 12px;border: 1px solid var(--sn-line);border-radius: 11px;background: var(--color-bg);min-width:0}.people-detail-grid span{font-size:8px;text-transform:uppercase;letter-spacing:.08em;color: var(--sn-muted);font-weight:900}.people-detail-grid strong{font-size:10px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .people-impersonation{display:grid;gap:12px;padding:15px;border: 1px solid var(--sn-line);border-radius: 13px;background: var(--sn-surface)}.people-impersonation h3{margin:4px 0;font-size:13px}.people-impersonation p{margin:0;font-size:10px;line-height:1.5;color: var(--sn-muted)}.people-impersonation label{display:grid;gap:6px;font-size:9px;font-weight:900;color: var(--sn-ink)}.people-impersonation textarea{min-height:92px;resize:vertical;border: 1px solid var(--sn-line);border-radius: 10px;padding:10px;font:inherit;font-size:11px;outline: none;background: var(--color-surface)}.people-impersonation textarea:focus{border-color: var(--sn-line);box-shadow: 0 0 0 3px var(--sn-shadow-sm)}
