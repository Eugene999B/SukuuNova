...
export default async function PlatformSchool360Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const audits = canAudit ? await db.$queryRawUnsafe<Array<{ id:string; actorId:string|null; actorName:string|null; actorEmail:string|null; action:string; targetEntity:string|null; createdAt:Date }>>(
    `SELECT l."id",l."actorId",a."name" AS "actorName",a."email" AS "actorEmail",l."action",l."targetEntity",l."createdAt"
     ...
     WHERE l."targetSchoolId"=$1 ORDER BY l."createdAt" DESC LIMIT 20`,
    id,
  ) : [];
  ...
  return <AppShell universe="platform" title={data.school.name} subtitle="A 360° view of this school’s account, people, academics, finance and platform activity." active="Schools">
    <div className="app-banner"><div><h3>{data.school.name}</h3><p>{data.school.uniqueCode} · {data.school.status} · {data.school.subscriptionPlan?.name ?? "No plan assigned"}</p></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Link className="app-pill" href="/platform/schools">Back to schools</Link>{canSupport ? <Link className="app-action" href={`/platform/support?schoolId=${encodeURIComponent(id)}`}><strong>Support</strong>Open school support</Link> : null}{canBilling ? <Link className="app-pill" href={`/platform/billing?schoolId=${encodeURIComponent(id)}`}>Billing</Link> : null}</div></div>
    ...
    <div className="app-dashboard-grid">
      ...
      <section className="app-card app-panel"><div className="app-card-head"><div><h2>Commercial activity</h2><p>Recent invoices and collection activity.</p></div>{canBilling ? <Link className="app-pill" href={`/platform/billing?schoolId=${encodeURIComponent(id)}`}>Billing</Link> : null}</div>{data.invoices.slice(0,8).map((invoice)=><div className="app-list-row" key={invoice.id}><div><b>{invoice.student.name}</b><span>₵{Number(invoice.totalAmount).toLocaleString()} · {invoice.status}</span></div><small>{new Date(invoice.createdAt).toLocaleDateString()}</small></div>)}{data.invoices.length===0&&<p>No invoices have been recorded yet.</p>}</section>
    </div>
    {canAudit ? <section className="app-card app-panel"><div className="app-card-head"><div><h2>Recent platform activity</h2><p>Support and control actions involving this school.</p></div><Link className="app-pill" href="/platform/audit">Open audit</Link></div>{audits.length===0?<p>No platform audit events are recorded for this school yet.</p>:audits.map((event)=><div className="app-list-row" key={event.id}><div><b>{event.action}</b><span>{event.targetEntity ?? "School"} · {event.actorName ? `${event.actorName}${event.actorEmail ? ` · ${event.actorEmail}` : ""}` : event.actorId ? `actor ${event.actorId}` : "system"}</span></div><small>{new Date(event.createdAt).toLocaleString()}</small></div>)}</section> : null}
  </AppShell>;
}
