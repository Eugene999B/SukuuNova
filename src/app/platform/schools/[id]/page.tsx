...
export default async function PlatformSchool360Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell universe="platform" title={data.school.name} subtitle="School 360 · account, people, academics, attendance, finance, support, security and platform activity." active="Schools">
    ...
    <section className="app-card app-panel"><div className="app-card-head"><div><span className="app-eyebrow">OPERATOR WORKBENCH</span><h2>Jump into the school without losing context</h2><p>Each action preserves the school scope while taking you to the workflow that owns the data.</p></div></div><div className="app-dashboard-grid">{quickLinks.map(([title,href,body])=><Link key={title} href={href} className="platform-settings-link-card"><div><span className="app-eyebrow">WORKFLOW</span><h3>{title}</h3><p>{body}</p></div><span>→</span></Link>)}</div></section>
