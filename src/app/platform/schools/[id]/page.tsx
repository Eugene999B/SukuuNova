...
export default async function PlatformSchool360Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell universe="platform" title={data.school.name} subtitle="School 360 · account, people, finance, support, security and platform activity." active="Schools">
    <div className="app-banner"><div><span className="app-eyebrow">SCHOOL 360</span><h3>{data.school.name}</h3><p>{data.school.uniqueCode} · {data.school.status} · {data.school.subscriptionPlan?.name ?? "No plan assigned"} · created {new Date(data.school.createdAt).toLocaleDateString()}</p></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Link className="app-pill" href="/platform/schools">Back to schools</Link><Link className="app-pill" href={`/platform/schools/${encodeURIComponent(id)}/people`}>People & access</Link>{canSupport ? <Link className="app-action" href={`/platform/support?schoolId=${encodeURIComponent(id)}`}><strong>Support</strong>Open case queue</Link> : null}{canBilling ? <Link className="app-pill" href={`/platform/billing?schoolId=${encodeURIComponent(id)}`}>Billing</Link> : null}</div></div>
