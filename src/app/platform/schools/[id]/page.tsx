...
export default async function PlatformSchool360Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const quickLinks = [
    ["People & access", `/platform/schools/${id}/people`, "Inspect school users and enter a reason-gated support session when permitted."],
  ...
  return <AppShell universe="platform" title={data.school.name} subtitle="School 360 · account, people, finance, support, security and platform activity." active="Schools">
    <div className="app-banner"><div><span className="app-eyebrow">SCHOOL 360</span><h3>{data.school.name}</h3><p>{data.school.uniqueCode} · {data.school.status} · {data.school.subscriptionPlan?.name ?? "No plan assigned"} · created {new Date(data.school.createdAt).toLocaleDateString()}</p></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Link className="app-pill" href="/platform/schools">Back to schools</Link>{canImpersonate ? <Link className="app-pill" href={`/platform/schools/${encodeURIComponent(id)}/people`}>People & access</Link> : null}{canSupport ? <Link className="app-action" href={`/platform/support?schoolId=${encodeURIComponent(id)}`}><strong>Support</strong>Open case queue</Link> : null}{canBilling ? <Link className="app-pill" href={`/platform/billing?schoolId=${encodeURIComponent(id)}`}>Billing</Link> : null}</div></div>
    ...
    <section className="app-card app-panel"><div className="app-card-head"><div><span className="app-eyebrow">OPERATOR WORKBENCH</span><h2>Every jump stays inside the platform control plane</h2><p>These are the platform-safe workflows currently available for this school. School-only workspaces remain behind the explicit, audited support-session bridge.</p></div></div><div className="app-dashboard-grid">{quickLinks.map(([title,href,body])=><Link key={title} href={href} className="platform-settings-link-card"><div><span className="app-eyebrow">WORKFLOW</span><h3>{title}</h3><p>{body}</p></div><span>→</span></Link>)}</div></section>
