...
export default async function SchoolActivityPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ q?: string; sensitive?: string }> }) {
  ...
  return <AppShell universe="platform" title={`${data.school.name} · Activity`} subtitle="Tenant activity." active="Schools">
    ...
    <section className="app-card app-panel" style={{ padding: 18 }}><form style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: 8, alignItems: "end" }}><label style={{ display: "grid", gap: 5, fontSize: 9, fontWeight: 850, color: "var(--color-text-muted)" }}><span>Search activity, action or target</span><div style={{ display: "flex", gap: 7, alignItems: "center", border: "1px solid var(--sn-line)", borderRadius: 11, padding: "0 10px", background: "var(--color-surface)" }}><Search size={14}/><input name="q" defaultValue={query} placeholder="e.g. student.created, payment, settings" style={{ border: 0, outline: 0, minHeight: 38, flex: 1, font: "inherit", color: "var(--sn-ink)" }}/></div></label><label style={{ display: "flex", alignItems: "center", gap: 6, minHeight: 40, padding: "0 10px", border: "1px solid var(--sn-line)", borderRadius: 11, background: "var(--color-surface)", fontSize: 9, fontWeight: 800 }}><input type="checkbox" name="sensitive" value="1" defaultChecked={sensitiveOnly}/>Sensitive only</label><button type="submit" className="app-action"><Filter size={14}/><strong>Filter</strong></button></form></section>
    <section className="app-card app-panel"><div className="app-card-head"><div><span className="app-eyebrow">EVIDENCE STREAM</span><h2>{combined.length.toLocaleString()} events</h2></div><Activity size={20}/></div>{combined.length ? combined.map((event)=><details key={event.id} className="platform-activity-row" style={{ display: "block", padding: "13px 0", borderBottom: "1px solid var(--sn-line)" }}><summary style={{ cursor: "pointer", listStyle: "none", display: "grid", gridTemplateColumns: "90px 1fr auto", gap: 12, alignItems: "center" }}><span className={`platform-status ${event.level === "Sensitive" ? "platform-status-critical" : event.level === "Warning" ? "platform-status-watch" : "platform-status-healthy"}`}>{event.level}</span><span style={{ display: "grid", gap: 3 }}><strong style={{ fontSize: 11 }}>{event.action}</strong><small style={{ fontSize: 9, color: "var(--sn-muted)" }}>{event.source} · {event.actor}{event.target ? ` · ${event.target}` : ""}</small></span><time style={{ fontSize: 9, color: "var(--sn-muted)" }}>{new Date(event.createdAt).toLocaleString()}</time></summary><div style={{ marginTop: 10, marginLeft: 102, padding: 12, borderRadius: 11, background: "var(--color-bg)", border: "1px solid var(--sn-line)" }}><div style={{ display: "flex", gap: 7, alignItems: "center", fontSize: 9.5, fontWeight: 800, color: "var(--color-text-muted)" }}><ShieldAlert size={13}/>Event evidence</div><pre style={{ margin: "8px 0 0", whiteSpace: "pre-wrap", wordBreak: "break-word", fontSize: 9, lineHeight: 1.5, color: "var(--color-text-secondary)" }}>{JSON.stringify(event.meta ?? {}, null, 2)}</pre></div></details>) : <div className="platform-empty"><strong>No activity matches this investigation.</strong></div>}</section>
