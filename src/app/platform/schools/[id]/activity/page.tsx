...
export default async function SchoolActivityPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ q?: string; sensitive?: string }> }) {
  ...
  const data = await withTenant(id, async (tx) => {
    ...
    const events = await tx.auditLogSchool.findMany({ where: query ? { OR: [{ action: { contains: query, mode: "insensitive" } }, { targetEntity: { contains: query, mode: "insensitive" } }] } : undefined, orderBy: { createdAt: "desc" }, take: 200 });
    ...
    return { school, events: sensitiveOnly ? events.filter((event) => severity(event.action) === "Sensitive") : events, actors };
  });
  if (!data) notFound();
  const actorMap = new Map(data.actors.map((actor) => [actor.id, actor]));
  const platformEvents = canAudit ? await db.$queryRawUnsafe<Array<{ id: string; actorId: string | null; actorName: string | null; action: string; targetEntity: string | null; createdAt: Date; meta: unknown }>>(`SELECT l."id",l."actorId",a."name" AS "actorName",l."action",l."targetEntity",l."createdAt",l."meta" FROM "AuditLogPlatform" l LEFT JOIN "PlatformAdmin" a ON a."id"=l."actorId" WHERE l."targetSchoolId"=$1 ORDER BY l."createdAt" DESC LIMIT 100`, id) : [];
  const combined = [...data.events.map((event) => { const actor = actorMap.get(event.actorId); return { id: `school:${event.id}`, source: "School", actor: actor?.name ?? event.actorId ?? "System", action: event.action, target: event.targetEntity, createdAt: event.createdAt, meta: event.meta, level: severity(event.action) }; }), ...platformEvents.map((event) => ({ id: `platform:${event.id}`, source: "Platform", actor: event.actorName ?? event.actorId ?? "System", action: event.action, target: event.targetEntity, createdAt: event.createdAt, meta: event.meta, level: severity(event.action) }))].sort((a,b)=>new Date(b.createdAt).getTime()-new Date(a.createdAt).getTime());
