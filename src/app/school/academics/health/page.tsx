...
export default async function AcademicHealthPage(){
  ...
  const data=await withTenant(session.schoolId,async tx=>{
    ...
    const now=new Date();
    const currentTerm=terms.find((term)=>now>=term.startDate&&now<=term.endDate)??terms[0];
  ...
  const readinessChecks=[
    {label:"Academic calendar & term dates",detail:!termSet?"Create the academic year and first term":termDatesValid?`${data.terms.length} term(s) connected · active period has valid dates`:"Active term dates are invalid",ok:termSet&&termDatesValid,href:"/school/terms"},
    ...
    {label:"Current-term gradebook activity",detail:data.termAssessments===0?"No assessments have been created for the working term":`${data.termAssessments} assessment(s) · ${data.termScores} score record(s)`,ok:gradebookReady,href:"/school/gradebook/studio"},
