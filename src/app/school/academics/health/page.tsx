...
export default async function AcademicHealthPage(){
  ...
  return <AppShell universe="school" title="Academic Readiness" subtitle="Academic readiness." active="Academic Readiness" schoolName={data.school?.name??"School Workspace"} schoolCode={data.school?.uniqueCode??""} userName={session.name} role="Academic leadership">
    <div className="readiness-shell">
      <section className="readiness-hero"><div><span className="readiness-kicker">ACADEMIC QUALITY CONTROL</span><h2>Know what is ready before teaching starts.</h2><div className="readiness-pills"><span>{data.school?.name??"School"}</span><span>{currentTerm?`${currentTerm.name} · ${currentTerm.academicYear.name}`:"No active term"}</span><span>{teacherCount} assigned teacher(s)</span></div></div><div className="readiness-score"><span>Readiness</span><strong>{score}%</strong><b>{stage}</b></div></section>
      ...
      <section className="readiness-layout"><div className="readiness-card"><div className="readiness-card-head"><div><span className="readiness-kicker">PRE-FLIGHT CHECK</span><h3>What the academic system needs</h3></div><span className={`readiness-state ${score===100?"good":"warn"}`}>{score===100?"All clear":"Review items"}</span></div><div className="readiness-checks">{readinessChecks.map((check,index)=><div className={`readiness-check ${check.ok?"ready":"attention"}`} key={check.label}><span className="readiness-index">{String(index+1).padStart(2,"0")}</span><div className="readiness-check-copy"><b>{check.label}</b><small>{check.detail}</small></div><span className="readiness-check-status">{check.ok?"Ready":"Needs attention"}</span><Link href={check.href}>Fix →</Link></div>)}</div></div>
        <aside className="readiness-card readiness-side"><div className="readiness-card-head"><div><span className="readiness-kicker">ACADEMIC OPERATING MODEL</span><h3>Who does what</h3></div></div><div className="role-panels"><div><span className="role-icon">O</span><div><b>Owner</b></div></div><div><span className="role-icon">P</span><div><b>Principal / Vice Principal</b></div></div><div><span className="role-icon">T</span><div><b>Teachers</b></div></div><div><span className="role-icon">S</span><div><b>Support & specialist staff</b></div></div></div><div className="readiness-actions"><Link href="/school/settings/access" className="readiness-action">Manage people & roles <span>→</span></Link><Link href="/school/settings/roles" className="readiness-action">Design custom roles <span>→</span></Link><Link href="/teacher" className="readiness-action">Open teacher workspace <span>→</span></Link></div></aside></section>
      <section className="readiness-footer"><div><span className="readiness-kicker">CONNECTED ACADEMIC CHAIN</span><h3>Setup → assignment → teaching → review → results → reporting</h3></div><div className="chain"><span>01 Setup</span><i>→</i><span>02 Assign</span><i>→</i><span>03 Teach</span><i>→</i><span>04 Review</span><i>→</i><span>05 Report</span></div></section>
    </div>
  </AppShell>;
}
