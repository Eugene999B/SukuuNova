...
export default async function GuardianAlertsPage() {
  ...
  return <AppShell universe="school" title="Guardian Alerts" subtitle="Family alerts." active="Guardian Alerts" schoolName={data.school.name} schoolCode={data.school.uniqueCode} userName={session.name}>
    <div className="module-shell">
      <section className="module-hero"><div><span className="module-kicker">Family communication</span><h2>Guardian alert centre</h2></div><div className="module-actions"><Link className="module-button secondary" href="/school/attendance/exceptions">Attendance exceptions</Link><Link className="module-button secondary" href="/school/communications/messages">All messages</Link></div></section>
      ...
      {manage ? <section className="module-layout"><div className="module-panel"><div className="module-toolbar"><div><strong style={{fontSize:12}}>Queue attendance notification</strong></div></div><form action={queueAttendanceAlert} style={{display:"grid",gridTemplateColumns:"minmax(210px,1.6fr) minmax(150px,.7fr) auto",gap:9}}><select className="module-select" name="studentId" defaultValue="" required><option value="">Choose learner</option>{data.students.map(s=><option value={s.id} key={s.id}>{s.name} · {s.class?.name ?? "Unassigned"}{s.guardians.length ? "" : " · no primary guardian"}</option>)}</select><select className="module-select" name="kind" defaultValue="absence"><option value="absence">Absence notice</option><option value="late">Late arrival notice</option></select><button className="module-button primary" type="submit">Queue alert →</button></form></div><aside className="module-side-stack"><section className="module-side-card"><h3>Safe workflow</h3><div className="module-pill-row"><span className="module-pill">Review → notify</span><span className="module-pill">Primary guardian</span><span className="module-pill">SMS queue</span></div></section><section className="module-side-card"><h3>No primary guardian?</h3><p>Link a primary guardian first.</p></section></aside></section> : <section className="module-card"><div className="module-empty"><strong>View-only alert centre</strong><span>Your role can inspect guardian notifications, but cannot queue new messages.</span></div></section>}
      <section className="module-card"><div className="module-toolbar"><div><strong style={{fontSize:12}}>Recent guardian alerts</strong></div></div><div className="module-responsive"><table className="module-table"><thead><tr><th>Channel</th><th>Recipient</th><th>Message</th><th>Status</th><th>Created</th></tr></thead><tbody>{data.messages.length ? data.messages.map(m=><tr key={m.id}><td>{m.channel.toUpperCase()}</td><td>{m.recipientPhone}</td><td><span>{m.body}</span>{m.lastError ? <small className="module-muted">{m.lastError}</small> : null}</td><td><span className="module-pill">{m.status}</span></td><td>{m.createdAt.toLocaleString()}</td></tr>) : <tr><td colSpan={5}><div className="module-empty"><strong>No guardian alerts yet</strong></div></td></tr>}</tbody></table></div></section>
