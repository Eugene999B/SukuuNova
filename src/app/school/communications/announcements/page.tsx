...
export default async function AnnouncementsPage() {
  ...
  return <main className="app-shell app-shell-school">
    ...
    <section className="app-main"><header className="app-topbar"><div><div className="app-breadcrumb">SukuuNova <span>/</span> Communications</div><h1>Announcements</h1></div></header><div className="app-content">
      <div className="module-topline"><div><span className="module-overline">Communication centre</span><h2>Reach the right people.</h2></div><Link className="module-secondary-button" href="/dashboard"><ArrowLeft size={14} aria-hidden="true" /> Overview</Link></div>
      <AnnouncementComposer roles={data.roles} users={data.users} />
      <section className="module-card" style={{ marginTop: 14 }}><div className="module-section-title"><div><span>Delivery strategy</span><h3>Keep everyday communication inside the portal</h3></div></div><div className="module-linked-grid"><Link href="/school/communications/messages">Direct messages <ArrowRight size={13} aria-hidden="true" /></Link><Link href="/school/communications/broadcasts">SMS / WhatsApp <ArrowRight size={13} aria-hidden="true" /></Link><Link href="/school/events">Events & calendar <ArrowRight size={13} aria-hidden="true" /></Link><Link href="/school/communications/settings">Communication settings <ArrowRight size={13} aria-hidden="true" /></Link></div></section>
    </div></section>
  </main>;
}
