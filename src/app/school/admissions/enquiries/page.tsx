...
export default async function AdmissionsEnquiriesPage({ searchParams }: { searchParams: Promise<{ q?: string; view?: string }> }) {
  ...
  return <AppShell universe="school" title="Admissions Enquiries" subtitle="Enquiries." active="Enquiries" schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} userName={session.name}>
    <div className="enquiries-page">
      <section className="enquiries-hero">
        <div><span className="eyebrow">Admissions · Enquiry desk</span><h2>Turn first contact into a clear next step.</h2></div>
