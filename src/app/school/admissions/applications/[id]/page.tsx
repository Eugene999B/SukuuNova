...
import { admissionStatusLabel, appendAdmissionEvent, canTransitionAdmission, getAdmissionApplication, getAdmissionEvents, isAdmissionStatus } from "@/lib/admissions-v2";
...
async function changeAdmissionStatus(formData: FormData) {
  ...
  await withTenant(session.schoolId, async (tx) => {
    ...
    const currentRows = await tx.$queryRawUnsafe<Array<{ status: string; convertedStudentId: string | null }>>(
      `SELECT "status","convertedStudentId" FROM "AdmissionApplication" WHERE "id"=$1 AND "schoolId"=$2 LIMIT 1 FOR UPDATE`,
      id,
      session.schoolId,
    );
    ...
    await tx.$executeRawUnsafe(
      `UPDATE "AdmissionApplication" SET "status"=$1,"decisionNote"=COALESCE($2,"decisionNote"),"updatedAt"=CURRENT_TIMESTAMP${timestampField} WHERE "id"=$3 AND "schoolId"=$4`,
      toRaw,
      note,
      id,
      session.schoolId,
...
async function completeAdmission(formData: FormData) {
  ...
  await withTenant(session.schoolId, async (tx) => {
    await requirePermission(tx, session.userId, "students:write");
    await tx.$executeRawUnsafe(`SELECT pg_advisory_xact_lock(hashtext($1))`, `admission-application:${session.schoolId}:${id}`);
    const locked = await tx.$queryRawUnsafe<Array<{ status: string; convertedStudentId: string | null }>>(
      `SELECT "status","convertedStudentId" FROM "AdmissionApplication" WHERE "id"=$1 AND "schoolId"=$2 LIMIT 1 FOR UPDATE`,
      id,
      session.schoolId,
    );
    ...
    const changed = await tx.$executeRawUnsafe(
      `UPDATE "AdmissionApplication" SET "status"='enrolled',"convertedStudentId"=$1,"enrolledAt"=CURRENT_TIMESTAMP,"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=$2 AND "schoolId"=$3 AND "convertedStudentId" IS NULL`,
      studentId,
      id,
      session.schoolId,
    );
    ...
    if (application.enquiryId) {
      await tx.$executeRawUnsafe(
        `UPDATE "AdmissionEnquiry" SET "stage"='converted',"convertedStudentId"=$1,"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=$2 AND "schoolId"=$3 AND "convertedStudentId" IS NULL`,
        studentId,
...
export default async function AdmissionApplicationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const data = await withTenant(session.schoolId, async (tx) => {
    ...
    const [school, application, events, canWrite] = await Promise.all([
    ...
    return { school, application, events, canWrite };
  });
        ...
        <div className="admission-detail-grid">
          ...
          <aside className="admissions-card">
            ...
            <div className="admission-workflow">
              {!data.canWrite ? <p className="module-muted">You have read-only access to this application.</p> : null}
              {data.canWrite && application.status === "draft" ? <form action={changeAdmissionStatus}><input type="hidden" name="id" value={application.id}/><input type="hidden" name="to" value="submitted"/><button className="button primary" type="submit">Submit application</button></form> : null}
              {data.canWrite && application.status === "submitted" ? <form action={changeAdmissionStatus}><input type="hidden" name="id" value={application.id}/><input type="hidden" name="to" value="under_review"/><button className="button secondary" type="submit">Start review</button></form> : null}
              {data.canWrite && (application.status === "submitted" || application.status === "under_review") ? <form action={changeAdmissionStatus}><input type="hidden" name="id" value={application.id}/><input type="hidden" name="to" value="offered"/><textarea name="note" placeholder="Optional offer / decision note"/><button className="button primary" type="submit">Issue admission offer</button></form> : null}
              {data.canWrite && application.status === "offered" ? <form action={changeAdmissionStatus}><input type="hidden" name="id" value={application.id}/><input type="hidden" name="to" value="accepted"/><button className="button primary" type="submit">Record offer accepted</button></form> : null}
              {data.canWrite && application.status === "accepted" ? <form action={completeAdmission}><input type="hidden" name="id" value={application.id}/><button className="button primary" type="submit">Complete admission & enrol student</button><small>This creates the official Student record and confirmed term/class enrolment.</small></form> : null}
              {data.canWrite && canTransitionAdmission(application.status, "declined") ? <form action={changeAdmissionStatus}><input type="hidden" name="id" value={application.id}/><input type="hidden" name="to" value="declined"/><textarea name="note" required placeholder="Reason / note for declining"/><button className="button secondary" type="submit">Decline application</button></form> : null}
