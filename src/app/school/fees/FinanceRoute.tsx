...
export default async function FinanceRoute({ mode }: { mode: "overview"|"fees"|"invoices"|"payments"|"arrears"|"reports"|"payroll" }) {
  ...
  return <AppShell universe="school" title={title} subtitle="Connected billing, collections, balances and payroll." active={title} schoolName={school.name} schoolCode={school.uniqueCode} userName={session.name}><FinanceWorkspace mode={mode} schoolName={school.name} /></AppShell>;
}
