...
export default async function DevicesPage(){const session=await requireSchoolSession();const school=await withTenant(session.schoolId,tx=>tx.school.findUnique({where:{id:session.schoolId},select:{name:true,uniqueCode:true}}));return <AppShell universe="school" title="Devices" subtitle="Devices." active="Devices" schoolName={school?.name??"School Workspace"} schoolCode={school?.uniqueCode??""} userName={session.name}><DevicesDesk/></AppShell>}
