...
async function logVisitor(formData: FormData){"use server";const session=await requireSchoolSession();const name=String(formData.get("name")??"").trim();const phone=String(formData.get("phone")??"").trim();const purpose=String(formData.get("purpose")??"").trim();const hostStaffId=String(formData.get("hostStaffId")??"").trim();if(!name||!purpose)throw new Error("Visitor name and purpose are required.");await withTenant(session.schoolId,async tx=>{await requirePermission(tx,session.userId,"visitors:log");if(hostStaffId){const host=await tx.user.findFirst({where:{id:hostStaffId,schoolId:session.schoolId},select:{id:true}});if(!host)throw new Error("Host staff member does not belong to this school.");}await tx.visitorLog.create({data:{schoolId:session.schoolId,name,phone:phone||null,purpose,hostStaffId:hostStaffId||null}});});redirect("/school/visitors");}
async function checkOutVisitor(formData:FormData){"use server";const session=await requireSchoolSession();const id=String(formData.get("id")??"");if(!id)return;await withTenant(session.schoolId,async tx=>{await requirePermission(tx,session.userId,"visitors:log");await tx.visitorLog.updateMany({where:{id,schoolId:session.schoolId,timeOut:null},data:{timeOut:new Date()}});});redirect("/school/visitors");}

export default async function VisitorsPage(){
 const session=await requireSchoolSession();
 const data=await withTenant(session.schoolId,async tx=>{await requirePermission(tx,session.userId,"visitors:log");const[school,staff,visitors]=await Promise.all([tx.school.findUnique({where:{id:session.schoolId},select:{name:true,uniqueCode:true}}),tx.user.findMany({where:{schoolId:session.schoolId,status:"active"},orderBy:{name:"asc"},select:{id:true,name:true}}),tx.visitorLog.findMany({where:{schoolId:session.schoolId},orderBy:{timeIn:"desc"},take:100,select:{id:true,name:true,phone:true,purpose:true,timeIn:true,timeOut:true,hostStaff:{select:{name:true}}}})]);return{school,staff,visitors};});
