...
export default function PrintStudio({data}:{data:Data}){
  ...
  const columns=useMemo(()=>{
    const day=days[0]??{dayOfWeek:1,name:"Monday",enabled:true,start:"08:00",end:"15:00"};
    return timeline(day,data.timetableConfig);
  ...
  return <main className={`print-studio ${design}`}>
    <section className="studio-top">
      <div className="studio-intro"><span className="studio-kicker">ACADEMICS / PRINT TIMETABLE</span><h1>Make the timetable ready for people, paper and sharing.</h1><p>The editor controls the schedule. This workspace controls how that schedule looks when a school prints it, distributes it, edits it in Word, opens it in Excel, or shares it as HTML.</p></div>
      <Link href="/school/timetable" className="studio-back">← Back to timetable editor</Link>
    </section>

    <section className="studio-toolbar">
      <div className="toolbar-block"><label>Audience</label><div className="pill-tabs"><button type="button" className={audience==="class"?"active":""} onClick={()=>{setAudience("class");setSelectedId(data.classes[0]?.id??"")}}>Class</button><button type="button" className={audience==="teacher"?"active":""} onClick={()=>{setAudience("teacher");setSelectedId(data.teachers[0]?.id??"")}}>Teacher</button><button type="button" className={audience==="master"?"active":""} onClick={()=>{setAudience("master");setSelectedId("")}}>Master</button></div></div>
      {audience!=="master"?<div className="toolbar-block select-block"><label>{audience==="class"?"Class":"Teacher"}</label><select value={selectedId} onChange={e=>setSelectedId(e.target.value)}>{options.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></div>:null}
    </section>
    ...
    <section className="paper-wrap"><div className="paper-label"><span>LIVE PREVIEW</span><strong>{title}</strong><span>{filtered.length} lessons</span></div>
      <div className="paper" id="print-paper">
        <header className="paper-header"><div className="school-mark">{data.school?.logoUrl?<img src={data.school.logoUrl} alt={`${data.school.name} logo`}/>:null}</div><div className="paper-heading"><small>WEEKLY TIMETABLE</small><h2>{data.school?.name??"School"}</h2><strong>{title}</strong></div><div className="paper-badge"><span>{audience.toUpperCase()}</span><b>{data.timetableConfig.published?"PUBLISHED":"DRAFT"}</b></div></header>
        <div className="paper-rule"/>
        <div className="paper-note"><span>School day: {days[0]?.start??""} – {days[0]?.end??""}</span><span>{designs.find(d=>d.id===design)?.name}</span></div>
        <div className="paper-table-scroll"><table className="timetable-print-table"><thead><tr><th className="day-heading">DAY</th>{columns.map((c,i)=><th key={i} className={c.kind==="break"?"break-head":"time-head"}>{c.kind==="break"?<span>{c.name}</span>:<><b>{c.start}</b><small>{c.end}</small></>}</th>)}</tr></thead><tbody>{days.map(day=><tr key={day.dayOfWeek}><th className="day-cell"><b>{day.name.slice(0,3).toUpperCase()}</b><span>{day.name}</span></th>{columns.map((c,i)=>{if(c.kind==="break")return <td key={i} className="break-cell"><span>{c.name}</span></td>;const s=cell(day,c);return <td key={i} className={`lesson-cell ${s?"filled":""}`} style={s?{"--subject":hashColour(s.subject.name)} as React.CSSProperties:undefined}>{s?<><b>{s.subject.name}</b><span>{audience==="master"?s.class.name:s.teacher.name}</span>{audience==="master"?<small>{s.teacher.name}</small>:null}</>:null}</td>})}</tr>)}</tbody></table></div>
        <footer className="paper-footer"><span>{data.school?.uniqueCode??""}</span><span>Generated by SukuuNova • {new Date().toLocaleDateString()}</span></footer>
      </div>
    </section>
