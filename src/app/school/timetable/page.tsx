...
async function saveSlot(formData:FormData){
  ...
  const classId=String(formData.get("classId")??"").trim(); const subjectId=String(formData.get("subjectId")??"").trim(); const teacherId=String(formData.get("teacherId")??"").trim();
  const dayOfWeek=Number(formData.get("dayOfWeek")); const period=Number(formData.get("period")); const slotId=String(formData.get("slotId")??"").trim();
  ...
  await withTenant(session.schoolId,async tx=>{
    ...
    const clash=await tx.timetableSlot.findFirst({where:{schoolId:session.schoolId,dayOfWeek,period,OR:[{classId},{teacherId}],...(slotId?{NOT:{id:slotId}}:{})},select:{id:true,classId:true,teacherId:true}});
    ...
    if(slotId) await tx.timetableSlot.update({where:{id:slotId},data:{classId,subjectId,teacherId,dayOfWeek,period}});
    else await tx.timetableSlot.create({data:{schoolId:session.schoolId,classId,subjectId,teacherId,dayOfWeek,period}});
  });
...
async function deleteSlot(formData:FormData){
  "use server";
  const session=await requireSchoolSession(); const slotId=String(formData.get("slotId")??"").trim();
  if(!slotId) return;
...
export default async function TimetablePage({searchParams}:{searchParams:Promise<{classId?:string;edit?:string}>}){
  ...
  const selectedClassId=String(params.classId??"").trim(); const editId=String(params.edit??"").trim();
  ...
  return <AppShell universe="school" title="Timetable" subtitle="Build the weekly teaching schedule with real school hours and breaks." active="Timetable" schoolName={data.school?.name??"School Workspace"} schoolCode={data.school?.uniqueCode??""} userName={session.name}>
    <main className="timetable-page">
      <section className="timetable-hero"><div><span className="timetable-eyebrow">ACADEMICS / SCHEDULE</span><h1>Your school week, clearly arranged.</h1><p>Days run down the left. The actual school times run across the top. Breaks and lunch form their own vertical columns.</p></div><div className="timetable-actions"><Link href="/school/timetable/print" className="secondary-action">Print & export</Link><Link href={`/school/timetable?edit=new:1:1&classId=${encodeURIComponent(selectedClassId)}`} className="primary-action">+ Add lesson</Link></div></section>
      ...
      <section className="schedule-card"><div className="schedule-head"><div><span className="timetable-eyebrow">WEEKLY GRID</span><h2>{selectedClassName}</h2></div><div className="schedule-head-actions"><Link href="/school/academics/setup">School hours</Link><Link href="/school/timetable/print">Export</Link></div></div>
        <div className="schedule-scroll"><div className="schedule-grid" style={{gridTemplateColumns:gridTemplateColumns}}>
          <div className="schedule-corner"><span>DAY</span><small>TIME →</small></div>
          {columns.map((c,i)=><div key={`${c.kind}-${c.start}-${i}`} className={`schedule-time ${c.kind==="break"?"break-band":""}`}>{c.kind==="break"?<span>{c.name}</span>:<><strong>{c.start}</strong><small>{c.end}</small></>}</div>)}
          {enabledDays.map(day=>{const dayMap=new Map(data.slots.filter(s=>s.dayOfWeek===day.dayOfWeek).map(s=>[s.period,s]));const blocks=dayBlocks(day,data.config).blocks;return <div key={day.dayOfWeek} style={{display:"contents"}}><div className="schedule-day"><strong>{day.name.slice(0,3).toUpperCase()}</strong><span>{day.name}</span></div>{columns.map((column,i)=>{if(column.kind==="break") return <div key={`${day.dayOfWeek}-b-${i}`} className="schedule-cell break-cell"><span>{column.name}</span></div>;const b=blocks.find(x=>x.kind==="lesson"&&x.period===column.period);const slot=b?.period?dayMap.get(b.period):undefined;return <div key={`${day.dayOfWeek}-${i}`} className={`schedule-cell ${slot?"has-lesson":"open-slot"}`}>{slot?<Link href={`/school/timetable?classId=${encodeURIComponent(slot.classId)}&edit=${encodeURIComponent(slot.id)}`} className="lesson-card"><strong>{slot.subject.name}</strong><span>{slot.teacher.name}</span>{!selectedClassId?<small>{slot.class.name}</small>:null}</Link>:<Link href={`/school/timetable?classId=${encodeURIComponent(selectedClassId)}&edit=new:${day.dayOfWeek}:${b?.period??1}`} className="add-slot" aria-label={`Add lesson on ${day.name}`}>+</Link>}</div>})}</div>})}
        </div></div>
      ...
      <section className="timetable-lower"><div className="workload-panel"><div className="section-head"><div><span className="timetable-eyebrow">TEACHER LOAD</span><h3>Teaching periods</h3></div><Link href="/school/staff">Staff</Link></div>{data.teachers.slice(0,10).map(t=>{const count=data.slots.filter(s=>s.teacherId===t.id).length;return <div className="load-item" key={t.id}><div><strong>{t.name}</strong><span>{count} periods</span></div><div className="load-track"><i style={{width:`${Math.min(100,count*8)}%`}}/></div></div>})}</div><div className="smart-panel"><div className="section-head"><div><span className="timetable-eyebrow">QUICK TOOLS</span><h3>Keep scheduling moving</h3></div></div><Link href="/school/timetable/print">Create class, teacher or master schedule</Link><Link href="/school/academics/setup">Change days, hours or breaks</Link><Link href="/school/lessons">Open lesson planning</Link><Link href="/school/staff-attendance">Review staff cover</Link></div></section>
