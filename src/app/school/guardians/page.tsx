...
async function createGuardian(formData: FormData) {
  ...
  await withTenant(session.schoolId, async (tx) => {
    ...
    if (existingUser && !existingGuardian) {
      throw new Error("This phone number already belongs to another school account. Use a different number or open that account instead.");
    }
    if (existingUser && existingGuardian && existingGuardian.userId === null) {
      throw new Error("This guardian has a pending profile but no login identity. Open the guardian record to finish access setup instead of creating another account.");
    }
    ...
    if (!guardian.userId) {
      ...
      const user = await tx.user.create({
        data: {
          ...
          phone,
          email,
          passwordHash: await hash(opaqueInitialSecret, 12),
          status: "pending"
        },
        select: { id: true }
      });
...
export default async function GuardiansPage() {
  ...
  return <AppShell universe="school" title="Guardians" subtitle="Manage family contacts, children and secure guardian access." active="Guardians" schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} userName={session.name}>
    <div className="guardians-page">
      <section className="guardians-hero"><div><span className="eyebrow">People · Family directory</span><h2>Keep every family relationship clear.</h2><p>Find a guardian, see the children they are connected to, and manage portal access without mixing family records with login administration.</p></div><div className="guardians-hero-actions"><Link href="/school/communications/messages" className="button secondary">Message families</Link><a href="#create-guardian" className="button primary">+ Add guardian</a></div></section>
      <section className="guardian-metrics" aria-label="Guardian overview"><article><span>Total guardians</span><strong>{data.guardians.length}</strong><small>Family contacts on record</small></article><article><span>Portal active</span><strong>{portalEnabled}</strong><small>{data.guardians.length ? `${Math.round((portalEnabled / data.guardians.length) * 100)}% active` : "No accounts yet"}</small></article><article><span>Children linked</span><strong>{linkedChildren}</strong><small>Connected learner relationships</small></article><article className={familiesNeedingChildren || pendingAccess ? "attention" : "ok"}><span>Needs attention</span><strong>{familiesNeedingChildren + pendingAccess}</strong><small>{pendingAccess ? `${pendingAccess} awaiting access activation` : familiesNeedingChildren ? "Family records need a child link" : "All family records are connected"}</small></article></section>
