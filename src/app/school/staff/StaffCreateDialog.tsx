...
export function StaffCreateDialog({ classes, subjects }: { classes: Item[]; subjects: Item[] }) {
  ...
  const fieldStyle: React.CSSProperties = darkMode
    ? { backgroundColor: "var(--sn-surface)", color: "var(--sn-surface)", WebkitTextFillColor: "var(--sn-surface)", caretColor: "var(--sn-surface)", colorScheme: "dark", opacity: 1 }
    : { backgroundColor: "var(--sn-surface)", color: "var(--sn-surface)", WebkitTextFillColor: "var(--sn-surface)", caretColor: "var(--sn-surface)", colorScheme: "light", opacity: 1 };
  ...
  return <>
    ...
    {open ? <div className="staff-modal-backdrop" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
      <section className="staff-modal staff-modal-wide staff-dialog-region" role="dialog" aria-modal="true" aria-labelledby="staff-dialog-title">
        <style>{`.staff-dialog-region input.staff-dialog-field,.staff-dialog-region select.staff-dialog-field{background-color:${darkMode ? "var(--color-bg)" : "var(--color-surface)"}!important;color:${darkMode ? "var(--color-bg)" : "var(--color-text-primary)"}!important;-webkit-text-fill-color:${darkMode ? "var(--color-bg)" : "var(--color-text-primary)"}!important;caret-color:${darkMode ? "var(--color-bg)" : "var(--color-text-primary)"}!important;color-scheme:${darkMode ? "dark" : "light"}!important;opacity:1!important;mix-blend-mode:normal!important}.staff-dialog-region input.staff-dialog-field::placeholder{color:${darkMode ? "var(--color-text-subtle)" : "var(--color-text-muted)"}!important;opacity:1!important}.staff-dialog-region input.staff-dialog-field:-webkit-autofill,.staff-dialog-region input.staff-dialog-field:-webkit-autofill:hover,.staff-dialog-region input.staff-dialog-field:-webkit-autofill:focus{-webkit-text-fill-color:${darkMode ? "var(--color-bg)" : "var(--color-text-primary)"}!important;-webkit-box-shadow:0 0 0 1000px ${darkMode ? "var(--color-bg)" : "var(--color-surface)"} inset!important;box-shadow:0 0 0 1000px ${darkMode ? "var(--color-bg)" : "var(--color-surface)"} inset!important}.staff-dialog-region select.staff-dialog-field option{background:${darkMode ? "var(--color-bg)" : "var(--color-surface)"}!important;color:${darkMode ? "var(--color-bg)" : "var(--color-text-primary)"}!important}`}</style>
