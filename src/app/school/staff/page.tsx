...
export default async function StaffPage() {
  ...
  const data = await withTenant(session.schoolId, async (tx) => {
    const [school, users, classes, subjects] = await Promise.all([
      ...
      tx.user.findMany({
        where: { status: { in: ["active", "pending", "suspended"] } }, orderBy: { name: "asc" },
        select: {
    ...
    <AppShell universe="school" title="Staff & Teachers" subtitle="Manage staff profiles, teaching assignments and login activation from one workforce directory." active="Staff & Teachers" schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} userName={session.name}>
      <div className="staff-workspace">
        <section className="staff-header">
          <div><span className="staff-eyebrow">PEOPLE · WORKFORCE</span><h2>Staff directory</h2><p>A staff profile represents the person. A login is activated separately from Sub-accounts & Access.</p></div>
          <StaffCreateDialog classes={data.classes} subjects={data.subjects} />
        </section>

        {pending.length ? <section className="staff-form-note wide"><strong>{pending.length} staff profile{pending.length === 1 ? " is" : "s are"} waiting for login activation.</strong><span>Teaching assignments are already attached to the staff profile. Open <Link href="/school/settings/access">Sub-accounts & Access</Link> to create the login deliberately.</span></section> : null}
