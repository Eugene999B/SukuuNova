...
async function assignHouse(formData: FormData) {
  ...
  await withTenant(session.schoolId, async (tx) => {
    ...
    const student = await tx.student.findFirst({ where: { id: studentId, schoolId: session.schoolId }, select: { id:true, name:true, houseId:true } });
    ...
    await appendSchoolAudit(tx, { schoolId: session.schoolId, actorId: session.userId, action: "student.house_assigned", entityType: "Student", entityId: studentId, before: { houseId: student.houseId }, after: { houseId, houseName: house[0].name } });
  });
  redirect("/school/classes?houses=1");
}

async function autoBalanceHouses(formData: FormData) {
  "use server";
  const session = await requireSchoolSession();
  await withTenant(session.schoolId, async (tx) => {
    await requirePermission(tx, session.userId, "students:write");
    ...
    const students = await tx.student.findMany({ where: { schoolId: session.schoolId, status: "active" }, orderBy: [{ class: { level: "asc" } }, { name: "asc" }], select: { id:true, houseId:true } });
...
export default async function ClassesPage({ searchParams }: { searchParams: Promise<{ class?: string; houses?: string; q?: string }> }) {
  ...
  const showCreateClass = params.houses !== "1" && params.class === undefined && params.houses !== "1" && false;
