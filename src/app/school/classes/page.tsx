...
import "./classes-official-structure.css";

type CurrentSectionRow = { classId: string; displayName: string; sectionCode: string };

async function readClassesWorkspace(schoolId: string, actorId: string) {
  return withTenant(schoolId, async (tx) => {
    await provisionOfficialTemplateClasses(tx, { schoolId, actorId });
    const currentYear = await tx.academicYear.findFirst({ where: { schoolId, isLocked: false }, orderBy: { startDate: "desc" }, select: { id: true, name: true } });
    const [school, classes, teachers, houses, learners, sections] = await Promise.all([
      ...
      currentYear ? tx.$queryRawUnsafe<CurrentSectionRow[]>(`SELECT "classId","displayName","sectionCode" FROM "ClassSection" WHERE "schoolId"=$1 AND "academicYearId"=$2 AND "isActive"=true`, schoolId, currentYear.id) : Promise.resolve([] as CurrentSectionRow[]),
    ]);
    return { school, classes, teachers, houses, learners, currentYear, sections };
  });
}

const getCachedClassesWorkspace = (schoolId: string, actorId: string) => unstable_cache(
  () => readClassesWorkspace(schoolId, actorId),
  ["sukuunova", "classes-workspace", schoolId, actorId],
  { revalidate: 60 }
)();
...
export default async function ClassesPage() {
  ...
  const workspace = await getCachedClassesWorkspace(session.schoolId, session.userId);
  const sectionByClass = new Map(workspace.sections.map((item) => [item.classId, item]));
  const classRows = workspace.classes.map((item) => {
    const section = sectionByClass.get(item.id);
    return { id: item.id, name: section?.displayName ?? item.name, level: item.level, teacher: item.classTeacher, students: item._count.students, subjects: item._count.subjectAssignments, timetable: item._count.timetableSlots };
  });
  ...
  return <AppShell universe="school" title="Classes" subtitle="Official class levels from your installed academic structure. Split a class into A/B/C only when needed." active="Classes" schoolName={workspace.school?.name ?? "School Workspace"} schoolCode={workspace.school?.uniqueCode ?? ""} userName={session.name}>
