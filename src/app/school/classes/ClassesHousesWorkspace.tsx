...
export function ClassesHousesWorkspace({ classes, teachers, houses, learners, schoolName }: { classes:ClassRow[]; teachers:Teacher[]; houses:House[]; learners:Learner[]; schoolName:string }) {
  ...
  const onHouseSubmit=(event:React.FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget);startTransition(async()=>{const r=await createHouse({name:String(form.get("name")??""),code:String(form.get("code")??""),color:String(form.get("color")??"var(--sn-primary)"),description:String(form.get("description")??"")});setMessage(r);if(r.ok){setHouseOpen(false);setTimeout(()=>window.location.reload(),500);}})};
