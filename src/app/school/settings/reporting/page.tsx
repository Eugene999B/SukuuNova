...
export default async function ReportPolicyPage() {
  ...
  const data = await withTenant(session.schoolId, async (tx) => {
    ...
    const rows = await tx.$queryRawUnsafe<Array<{ showOverallPosition:boolean; positionScope:string; remarkSource:string; positionBandLabels:unknown; behaviorRatingFields:unknown; promotionRule:string }>>(`SELECT "showOverallPosition","positionScope","remarkSource","positionBandLabels","behaviorRatingFields","promotionRule" FROM "SchoolSettings" WHERE "schoolId"=$1`, session.schoolId);
    return { school, settings: rows[0] };
  });
  if (!data.school) return null;
  return <AppShell universe="school" title="Report Card Policy" subtitle="Choose how positions, remarks, conduct and promotion appear on official report cards." active="School Settings" schoolName={data.school.name} schoolCode={data.school.uniqueCode} userName={session.name}><ReportPolicySettings initial={{ showOverallPosition:data.settings?.showOverallPosition??true, positionScope:data.settings?.positionScope==="year_group"?"year_group":"class", remarkSource:data.settings?.remarkSource==="position_band"?"position_band":"grade_band", positionBandLabels:Array.isArray(data.settings?.positionBandLabels)?data.settings?.positionBandLabels:[], behaviorRatingFields:Array.isArray(data.settings?.behaviorRatingFields)?data.settings?.behaviorRatingFields:[], promotionRule:data.settings?.promotionRule==="pass_mark"||data.settings?.promotionRule==="overall_position"?data.settings.promotionRule:"manual" }} /></AppShell>;
}
