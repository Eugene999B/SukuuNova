  ...
  const data = await withTenant(session.schoolId, async (tx) => {
    const [school, settings, academicYears, terms] = await Promise.all([
      tx.school.findUnique({
        ...
        select: { id: true, name: true, uniqueCode: true, status: true, logoUrl: true },
      }),
    ...
    return {
      ...
      terms: terms.map((term) => ({
        ...term,
        status: (now < term.startDate ? "upcoming" : now > term.endDate ? "completed" : "current") as WorkspaceTerm["status"],
      })),
    };
  });
  ...
  const workspaceData: WorkspaceData = {
    ...
    settings: data.settings
      ? {
          ...
          gradeExamWeight: Number(data.settings.gradeExamWeight),
          allowPartialReportCards: data.settings.allowPartialReportCards,
          smsSenderId: data.settings.smsSenderId ?? null,
        }
      : null,
    ...
    terms: data.terms.map((term) => ({
      ...
      academicYear: {
        ...
        startDate: term.academicYear.startDate.toISOString(),
        endDate: term.academicYear.endDate.toISOString(),
      },
    })),
  };
