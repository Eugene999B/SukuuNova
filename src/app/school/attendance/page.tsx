...
export default async function AttendancePage() {
  ...
  const presentIds = new Set(data.todayEvents.filter(e => e.type === "in").map(e => e.studentId).filter(Boolean));
  const present = presentIds.size;
  const absent = Math.max(0, data.students.length - present);
  const lateIds = new Set(data.todayEvents.filter(e => e.type === "in" && e.isLate).map(e => e.studentId).filter(Boolean));
  const late = lateIds.size;
  const missing = absent;
  const firstIncompleteClass = data.classes.find(c => {
    const classPresent = new Set(data.todayEvents.filter(e => e.student?.classId === c.id && e.type === "in").map(e => e.studentId).filter(Boolean)).size;
    return classPresent < c._count.students;
  });

  return <AppShell universe="school" title="Student Attendance" subtitle="Daily register." active="Student Attendance" schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} userName={session.name}>
    <div className="module-workspace">
      <section className="module-setup-card module-card"><div><span className="module-overline">Daily operations</span><h3>Attendance is a daily source of truth.</h3></div><div className="module-setup-list"><Link href="#today"><span>1</span>Review today <b>Present · absent · late · missing</b></Link><Link href="/school/attendance/exceptions"><span>2</span>Resolve exceptions <b>Follow-up queue</b></Link><Link href="/school/communications/alerts"><span>3</span>Notify families <b>Guardian alerts</b></Link>{data.canRecord && firstIncompleteClass ? <Link href={`/school/attendance/register?classId=${encodeURIComponent(firstIncompleteClass.id)}&date=${encodeURIComponent(data.today)}`}><span>4</span>Continue register <b>{firstIncompleteClass.level ? `${firstIncompleteClass.level} · ` : ""}{firstIncompleteClass.name}</b></Link> : <Link href="/school/reports/analytics"><span>4</span>Monitor trends <b>Leadership view</b></Link>}</div></section>
      <div className="module-metrics"><DataCard label="Active learners" value={data.students.length} meta="Students expected today" icon={UsersRound} /><DataCard label="Present" value={present} meta="Recorded with an IN event today" icon={CircleCheckBig} tone="success" /><DataCard label="Late" value={late} meta="Present events marked late" icon={Clock3} tone={late ? "warning" : "success"} /><DataCard label="Absent" value={absent} meta="No IN event recorded today" icon={CircleAlert} tone={absent ? "warning" : "success"} /><DataCard label="Not yet recorded" value={missing} meta={missing ? "Complete the class registers" : "Register is complete"} icon={UserRoundCheck} tone={missing ? "warning" : "success"} /></div>
      <section className="module-card" id="today"><div className="module-section-title"><div><span>Today · {data.today}</span><h3>Class coverage</h3></div><div className="modal-actions"><Link className="button secondary" href="/school/attendance/exceptions">Exception queue</Link>{data.canRecord && firstIncompleteClass ? <Link className="button primary" href={`/school/attendance/register?classId=${encodeURIComponent(firstIncompleteClass.id)}&date=${encodeURIComponent(data.today)}`}>Continue register →</Link> : null}</div></div><div className="module-workflow">{data.classes.map(c => { const classEvents = data.todayEvents.filter(e => e.student?.classId === c.id && e.type === "in"); const classRecorded = new Set(classEvents.map(e => e.studentId).filter(Boolean)).size; const classMissing = Math.max(0, c._count.students - classRecorded); return <Link className="module-workflow-step" key={c.id} href={`/school/attendance/register?classId=${encodeURIComponent(c.id)}&date=${encodeURIComponent(data.today)}`}><span>{classMissing}</span><div><strong>{c.level ? `${c.level} · ` : ""}{c.name}</strong><small>{classRecorded} recorded of {c._count.students} learners · {classMissing ? `${classMissing} still missing` : "Register complete · open to review"}</small></div><span>→</span></Link>; })}{!data.classes.length ? <EmptyState title="No class groups" description="Create classes before relying on class-based attendance workflows." action={<Link href="/school/classes?action=create" className="ui-button ui-button-primary">Create class</Link>} /> : null}</div></section>
      <section className="module-card"><div className="module-section-title"><div><span>Recorded today</span><h3>Daily attendance activity</h3></div></div><div className="module-table-wrap"><table><thead><tr><th>Student</th><th>Class</th><th>State</th><th>Method</th><th>Time</th></tr></thead><tbody>{data.todayEvents.length ? data.todayEvents.map(event => <tr key={event.id}><td style={{padding:12}}><strong>{event.student?.name ?? "Unknown"}</strong><div style={{color:"var(--sn-muted)",fontSize:8}}>{event.student?.admissionNo}</div></td><td style={{padding:12}}>{event.student?.class ? `${event.student.class.level ?? ""}${event.student.class.level ? " · " : ""}${event.student.class.name}` : "Unassigned"}</td><td style={{padding:12}}>{event.type === "in" ? "Present" : event.type === "out" ? "Checked out" : event.type}{event.isLate ? " · late" : ""}</td><td style={{padding:12}}>{event.method}</td><td style={{padding:12}}>{event.timestamp.toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}</td></tr>) : <tr><td colSpan={5}><EmptyState icon={UserRoundCheck} title="No attendance recorded today" description={data.canRecord ? "Open a class register to record today's attendance." : "No attendance decisions have been recorded yet."} action={data.canRecord && firstIncompleteClass ? <Link href={`/school/attendance/register?classId=${encodeURIComponent(firstIncompleteClass.id)}&date=${encodeURIComponent(data.today)}`} className="ui-button ui-button-primary">Open class register</Link> : undefined} /></td></tr>}</tbody></table></div>{missing ? <div style={{marginTop:12,padding:12,borderRadius:12,border:"1px solid var(--color-bg)",background:"var(--color-bg)",color:"var(--sn-warning)"}}>{missing} active learner{missing === 1 ? "" : "s"} still need{missing === 1 ? "s" : ""} an attendance decision today.</div> : null}</section>
      <section className="module-card"><div className="module-section-title"><div><span>Recent history</span><h3>Latest attendance activity</h3></div></div><div className="module-workflow">{data.recentEvents.map(e => <div className="module-workflow-step" key={e.id}><span>{e.type.slice(0,1).toUpperCase()}</span><div><strong>{e.student?.name ?? "Unknown student"}</strong><small>{e.student?.admissionNo} · {e.attendanceDate.toISOString().slice(0,10)} · {e.type === "in" ? "present" : e.type}</small></div></div>)}{!data.recentEvents.length ? <EmptyState title="No attendance history" description="Recorded events will appear here." /> : null}</div></section>
    </div>
  </AppShell>;
}