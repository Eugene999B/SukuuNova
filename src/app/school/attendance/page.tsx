...
function dateOnly(value: string, fallback: string) { return /^\d{4}-\d{2}-\d{2}$/.test(value) ? value : fallback; }
...
async function recordAttendance(formData: FormData) {
  ...
  if (!studentId || !/^\d{4}-\d{2}-\d{2}$/.test(attendanceDate) || !["present", "absent", "late", "excused"].includes(type)) throw new Error("Student, date and attendance status are required.");
  ...
  await withTenant(session.schoolId, async tx => {
    ...
    const existing = await tx.attendanceEvent.findFirst({ where: { studentId, attendanceDate: attendanceDateValue }, select: { id: true } });
...
async function readAttendanceWorkspace(schoolId: string, today: string) {
  return withTenant(schoolId, async tx => {
    const [school, settings, students, classes, todayEvents, recentEvents] = await Promise.all([
      ...
      tx.student.findMany({ where: { status: "active" }, orderBy: { name: "asc" }, select: { id: true, name: true, admissionNo: true, classId: true, class: { select: { id: true, name: true, level: true } } } }),
      tx.class.findMany({ orderBy: [{ level: "asc" }, { name: "asc" }], select: { id: true, name: true, level: true, _count: { select: { students: true } } } }),
      tx.attendanceEvent.findMany({ where: { attendanceDate: new Date(`${today}T00:00:00.000Z`), studentId: { not: null } }, include: { student: { select: { id: true, name: true, admissionNo: true, classId: true, class: { select: { id: true, name: true, level: true } } } } }, orderBy: { timestamp: "desc" }, take: 500 }),
    ...
    return { school, settings, students, classes, todayEvents, recentEvents };
  });
}

export default async function AttendancePage() {
  const session = await requireSchoolSession();
  const data = await readAttendanceWorkspace(session.schoolId, "__pending__");
