...
export default async function AttendancePage() {
  ...
  const timezone = await withTenant(session.schoolId, async tx => {
  ...
  return <AppShell universe="school" title="Student Attendance" subtitle="Run the daily class register, record exceptions once per day, and keep a clear queue of learners who still need a decision." active="Student Attendance" schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} userName={session.name}>
    <div className="module-workspace">
      ...
      <section className="module-card" id="today"><div className="module-section-title"><div><span>Today · {today}</span><h3>Class coverage</h3><p>Open the roster for any class that is incomplete or needs correction.</p></div><div className="modal-actions"><Link className="button secondary" href="/school/attendance/exceptions">Exception queue</Link>{auth.canRecord && firstIncompleteClass ? <Link className="button primary" href={`/school/attendance/register?classId=${encodeURIComponent(firstIncompleteClass.id)}&date=${encodeURIComponent(today)}`}>Continue register →</Link> : null}</div></div><div className="module-workflow">{data.classes.map(c => { const classEvents = data.todayEvents.filter(e => e.student?.classId === c.id); const classMissing = Math.max(0, c._count.students - classEvents.length); return <Link className="module-workflow-step" key={c.id} href={auth.canRecord ? `/school/attendance/register?classId=${encodeURIComponent(c.id)}&date=${encodeURIComponent(today)}` : `/school/attendance/register?classId=${encodeURIComponent(c.id)}&date=${encodeURIComponent(today)}`}><span>{classMissing}</span><div><strong>{c.level ? `${c.level} · ` : ""}{c.name}</strong><small>{classEvents.length} recorded of {c._count.students} learners · {classMissing ? `${classMissing} still missing` : "Register complete · open to review"}</small></div><span>→</span></Link>; })}{!data.classes.length ? <EmptyState title="No class groups" description="Create classes before relying on class-based attendance workflows." action={<Link href="/school/classes?action=create" className="ui-button ui-button-primary">Create class</Link>} /> : null}</div></section>
      <section className="module-card"><div className="module-section-title"><div><span>Recorded today</span><h3>Daily attendance activity</h3><p>Review the actual entries after completing a class register.</p></div></div><div className="module-table-wrap"><table><thead><tr><th>Student</th><th>Class</th><th>State</th><th>Method</th><th>Time</th></tr></thead><tbody>{data.todayEvents.length ? data.todayEvents.map(event => <tr key={event.id}><td style={{padding:12}}><strong>{event.student?.name ?? "Unknown"}</strong><div style={{color:"var(--sn-muted)",fontSize:8}}>{event.student?.admissionNo}</div></td><td style={{padding:12}}>{event.student?.class ? `${event.student.class.level ?? ""}${event.student.class.level ? " · " : ""}${event.student.class.name}` : "Unassigned"}</td><td style={{padding:12}}>{event.type}{event.isLate ? " · late" : ""}</td><td style={{padding:12}}>{event.method}</td><td style={{padding:12}}>{event.timestamp.toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}</td></tr>) : <tr><td colSpan={5}><EmptyState icon={UserRoundCheck} title="No attendance recorded today" description={auth.canRecord ? "Open a class register to record today's attendance." : "No attendance decisions have been recorded yet."} action={auth.canRecord && firstIncompleteClass ? <Link href={`/school/attendance/register?classId=${encodeURIComponent(firstIncompleteClass.id)}&date=${encodeURIComponent(today)}`} className="ui-button ui-button-primary">Open class register</Link> : undefined} /></td></tr>}</tbody></table></div>{missing ? <div style={{marginTop:12,padding:12,borderRadius:12,border:"1px solid rgba(220,160,80,.3)",background:"rgba(220,160,80,.05)",color:"var(--sn-warning)"}}>{missing} active learner{missing === 1 ? "" : "s"} still need{missing === 1 ? "s" : ""} an attendance decision today.</div> : null}</section>
