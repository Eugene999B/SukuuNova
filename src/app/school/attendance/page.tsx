...
export default async function AttendancePage() {
  ...
  return <AppShell universe="school" title="Student Attendance" subtitle="Run the daily student register, record exceptions once per day, and keep a clear queue of learners who still need a decision." active="Student Attendance" schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} userName={session.name}>
    <div className="module-workspace">
      ...
      <section className="module-card"><div className="module-section-title"><div><span>Class coverage</span><h3>Attendance readiness by class</h3><p>Classes with no or partial attendance are immediately visible before the register is considered complete.</p></div><Link href="/school/classes">Review class structure →</Link></div><div className="module-workflow">{data.classes.map(c => { const classEvents = data.todayEvents.filter(e => e.student?.classId === c.id); const classMissing = Math.max(0, c._count.students - classEvents.length); return <div className="module-workflow-step" key={c.id}><span>{classMissing}</span><div><strong>{c.level ? `${c.level} · ` : ""}{c.name}</strong><small>{classEvents.length} recorded of {c._count.students} learners · {classMissing ? `${classMissing} still missing` : "Register complete"}</small></div></div>; })}{!data.classes.length ? <EmptyState title="No class groups" description="Create classes before relying on class-based attendance workflows." action={<Link href="/school/classes?action=create" className="ui-button ui-button-primary">Create class</Link>} /> : null}</div></section>
