...
export default async function AttendanceRegisterPage({ searchParams }: { searchParams?: Promise<{ classId?: string; date?: string }> }) {
  ...
  return <AppShell universe="school" title="Class Attendance Register" subtitle="Choose one class and date, mark the roster quickly, then save the whole register as one audited operation." active="Student Attendance" schoolName={data.school.name} schoolCode={data.school.uniqueCode} userName={session.name}>
    <div className="module-workspace">
      ...
      <AttendanceRegister classId={data.selectedClass?.id || ""} attendanceDate={data.date} students={data.students} />
      {data.existing.length ? <section className="module-card"><strong>Existing records detected</strong><p>This date already has {data.existing.length} attendance record{data.existing.length === 1 ? "" : "s"} for this class. The register is read-only to prevent accidental duplicates.</p><Link className="button secondary" href={`/school/attendance?date=${encodeURIComponent(data.date)}`}>Review attendance overview</Link></section> : null}
    </div>
