...
export default async function AttendanceExceptionsPage() {
  ...
  return <AppShell universe="school" title="Late & Absence" subtitle="Exceptions." active="Late & Absence" schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} userName={session.name}>
    <div className="module-shell">
      <section className="module-hero"><div><span className="module-kicker">Attendance control room</span><h2>Late & absence exceptions</h2></div><div className="module-actions"><Link className="module-button secondary" href="/school/attendance">Back to register</Link>{canAlert ? <Link className="module-button primary" href="/school/communications/alerts">Guardian alerts →</Link> : null}</div></section>
      ...
      <section className="module-layout"><div className="module-panel"><div className="module-toolbar"><div><strong style={{fontSize:12}}>Exception queue</strong></div><Link className="module-button secondary" href="/school/communications/alerts">Open alert centre</Link></div><div className="module-responsive"><table className="module-table"><thead><tr><th>Student</th><th>Class</th><th>Exception</th><th>Date</th><th>Recorded by</th></tr></thead><tbody>{data.events.length ? data.events.map(e=><tr key={e.id}><td><b>{e.student?.name ?? "Unknown"}</b><span className="module-muted">{e.student?.admissionNo}</span></td><td>{e.student?.class ? `${e.student.class.level ?? ""}${e.student.class.level ? " · " : ""}${e.student.class.name}` : "Unassigned"}</td><td><span className="module-pill">{e.type === "absent" ? "ABSENT" : "LATE"}</span></td><td>{e.attendanceDate.toISOString().slice(0,10)}</td><td>{e.recorder?.name ?? "Attendance device"}</td></tr>) : <tr><td colSpan={5}><div className="module-empty"><strong>No late or absence exceptions</strong><span>The exception queue is clear for the current review window.</span></div></td></tr>}</tbody></table></div></div>
        <aside className="module-side-stack"><section className="module-side-card"><h3>Review rule</h3><div className="module-notice">Use this workspace for investigation and follow-up — not for re-entering duplicate attendance records.</div></section><section className="module-side-card"><h3>Alert delivery</h3><div className="module-pill-row"><span className="module-pill">{messagePhones.size} guardian recipients seen</span><span className="module-pill">{data.recentMessages.filter(m=>m.status==="sent").length} sent</span><span className="module-pill">{data.recentMessages.filter(m=>m.status==="failed").length} failed</span></div></section></aside></section>
    </div>
  </AppShell>;
}
