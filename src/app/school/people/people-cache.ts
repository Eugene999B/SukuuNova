import { cache } from "@/lib/tenant-cache";

export async function getPeopleSnapshot(schoolId: string) {
  return cache(`people:overview:${schoolId}`, 30, async () => {
...
export function invalidatePeopleSnapshot(schoolId: string) {
  // The tenant cache helper owns invalidation. This function intentionally lives
  // beside the query so writes can invalidate the same key without duplicating it.
  return import("@/lib/tenant-cache").then(({ invalidate }) => invalidate(`people:overview:${schoolId}`));
}
