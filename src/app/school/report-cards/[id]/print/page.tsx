...
export default async function ReportCardPrintPage({params}:{params:Promise<{id:string}>}){
 ...
 const data=await withTenant(session.schoolId,async(tx)=>{
  ...
  const gate=await tx.reportCard.findFirst({where:{id,schoolId:session.schoolId},select:{student:{select:{class:{select:{id:true,classTeacherId:true}}}}}});
  ...
  const signatures=Array.isArray(raw.signatureSlots)?raw.signatureSlots.filter((s):s is Signature=>Boolean(s)&&typeof s==="object"&&!Array.isArray(s)).map(s=>({role:typeof s.role==="string"?s.role:"School Official",name:typeof s.name==="string"?s.name:"",signatureDataUrl:typeof s.signatureDataUrl==="string"?s.signatureDataUrl:""})):[];
  return {report,signatures};
 });
 if(!data)notFound();return <OfficialReportCard data={data.report} signatures={data.signatures}/>;
}
