...
async function runReportCardAction(formData: FormData) {
  ...
  if (action === "generate") {
    await withTenant(session.schoolId, async (tx) => {
      ...
      const lockedIds = new Set(existing.filter((item) => item.status !== "draft").map((item) => item.studentId));
      let created = 0;
      for (const student of students) {
        if (existingIds.has(student.id) || lockedIds.has(student.id)) continue;
        try {
          await generateReportCard(tx, { schoolId: session.schoolId, actorId: session.userId, studentId: student.id, termId });
          created += 1;
        } catch {
          // Keep the rest of the class moving if one learner is not ready.
        }
      }
  ...
  await withTenant(session.schoolId, async (tx) => {
    if (action === "submit") {
      ...
      await approveAndQueuePublicReportCard(tx, { schoolId: session.schoolId, actorId: session.userId, reportCardId, origin: origin() });
...
export default async function ReportCardsPage({ searchParams }: { searchParams: Promise<{ term?: string; classId?: string; notice?: string }> }) {
  ...
  const data = await withTenant(session.schoolId, async (tx) => {
    ...
    const [students, reports, generate, submit, approve] = await Promise.all([
      ...
      tx.$transaction(async (inner) => requirePermission(inner, session.userId, "reports:generate")),
      tx.$transaction(async (inner) => requirePermission(inner, session.userId, "report_cards:submit")),
      tx.$transaction(async (inner) => requirePermission(inner, session.userId, "report_cards:approve")),
    ]);
    return { school, terms, classes, term, selectedClass, students, reports, permissions: { generate, submit, approve } };
  });
  ...
  const draft = data.reports.filter((item) => item.status === "draft").length;
    ...
    <AppShell universe="school" title="Report Cards" subtitle="Prepare, review, approve and release official student reports in a focused class-by-class workflow." active="Report Cards" schoolName={data.school.name} schoolCode={data.school.uniqueCode} userName={session.name}>
      <main className="report-cards-page">
        ...
        <section className="report-hero">
          <div>
            <span className="report-kicker">OFFICIAL ACADEMIC REPORTING</span>
            <h1>{data.term?.name ?? "Report cards"}</h1>
            <p>Work on one class at a time. Check that marks are ready, generate missing report records, then move each report through review, approval and release.</p>
          </div>
          <div className="report-hero-links">
            <Link href="/school/gradebook/studio">Gradebook</Link>
            <Link href="/school/reports">Archive</Link>
            <Link href="/school/downloads">Downloads</Link>
          </div>
        </section>

        <section className="report-controls">
          <div>
            <span className="report-kicker">WORKING CONTEXT</span>
            <strong>{data.selectedClass ? `${data.selectedClass.level ? `${data.selectedClass.level} · ` : ""}${data.selectedClass.name}` : "Choose a class"}</strong>
            <small>{data.term?.name ?? "Choose a reporting term"}</small>
          </div>
          <form method="get" className="report-selects">
            <label>Term<select name="term" defaultValue={data.term?.id ?? ""}>{data.terms.map((term) => <option key={term.id} value={term.id}>{term.name}</option>)}</select></label>
            <label>Class<select name="classId" defaultValue={params.classId ?? ""}><option value="">Choose class</option>{data.classes.map((item) => <option key={item.id} value={item.id}>{item.level ? `${item.level} · ` : ""}{item.name}</option>)}</select></label>
            <button className="report-button primary" type="submit">Open class</button>
          </form>
        </section>
        ...
        {!data.selectedClass ? (
          <section className="report-empty">
            ...
            <h2>Start with a class</h2>
            <p>Report cards work best as a class workflow. Choose the term and class above to see coverage, readiness and the learner review queue.</p>
          </section>
        ) : (
          <>
            <section className="report-summary">
              <article><span>Learners</span><strong>{learnerCount}</strong><small>Active in class</small></article>
              <article><span>Generated</span><strong>{reportCount}</strong><small>Report records</small></article>
              <article><span>For approval</span><strong>{submitted}</strong><small>Waiting for review</small></article>
              <article><span>Released</span><strong>{released}</strong><small>Published to families</small></article>
            </section>

            <section className="report-workflow">
              <div className="report-workflow-copy">
                <span className="report-kicker">01 · PREPARE</span>
                <h2>Make sure the class is ready</h2>
                <p>Report generation creates only missing draft records. Existing submitted, approved or released reports are not overwritten.</p>
                <div className="report-progress-line"><span style={{ width: `${percent(reportCount, learnerCount)}%` }} /></div>
                <small>{reportCount} of {learnerCount} learners have report records · {missing} missing</small>
              </div>
              <div className="report-workflow-action">
                {data.permissions.generate ? (
                  <form action={runReportCardAction}><input type="hidden" name="action" value="generate" /><input type="hidden" name="termId" value={data.term?.id ?? ""} /><input type="hidden" name="classId" value={data.selectedClass.id} /><button className="report-button primary" type="submit" disabled={!missing}>{missing ? `Generate ${missing} missing` : "All reports generated"}</button></form>
                ) : null}
                <Link className="report-text-link" href="/school/gradebook/studio">Review marks in Gradebook <span>→</span></Link>
              </div>
            </section>
            ...
            <section className="report-queue">
              <div className="report-queue-head">
                <div>
                  <span className="report-kicker">02 · REVIEW & MOVE</span>
                  <h2>Each report has one next step</h2>
                  <p>Preview the learner report, then take only the action allowed by its current state.</p>
                </div>
              </div>
              <div className="report-table-wrap">
                <table>
                  <thead><tr><th>Learner</th><th>Status</th><th>Created</th><th>Action</th></tr></thead>
                  <tbody>
                    {data.reports.map((report) => {
                      const status = String(report.status);
                      return <tr key={report.id}>
                        <td><strong>{report.student.name}</strong><small>{report.student.admissionNo}</small></td>
                        <td><span className={`report-status ${status}`}>{statusLabel[status] ?? status}</span><small>{statusHint[status] ?? "Current report state"}</small></td>
                        <td>{new Date(report.createdAt).toLocaleDateString("en-GH")}</td>
                        <td><div className="report-actions">
                          <Link href={`/school/report-cards/${report.id}/print`} className="report-action">Preview</Link>
                          {status === "draft" && data.permissions.submit ? <form action={runReportCardAction}><input type="hidden" name="action" value="submit" /><input type="hidden" name="termId" value={data.term?.id ?? ""} /><input type="hidden" name="classId" value={data.selectedClass.id} /><input type="hidden" name="reportCardId" value={report.id} /><button className="report-action" type="submit">Submit</button></form> : null}
                    ...
                    {!data.reports.length ? <tr><td colSpan={4}><div className="report-table-empty"><strong>No reports yet.</strong><span>Generate the missing reports above to begin the review queue.</span></div></td></tr> : null}
                  </tbody>
                </table>
              </div>
            </section>

            <section className="report-state-bar">
              <div><span>WORKFLOW</span><strong>Draft</strong><em>→</em><strong>For approval</strong><em>→</em><strong>Approved</strong><em>→</em><strong>Released</strong></div>
              <Link href="/school/settings">Report-card settings →</Link>
            </section>
          </>
        )}
      </main>
