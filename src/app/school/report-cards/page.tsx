...
async function runReportCardAction(formData: FormData) {
  ...
  if (action === "generate-class") {
    ...
    const result = await withTenant(session.schoolId, async tx => {
      ...
      const locked = new Set(existing.filter(r => r.status !== "draft").map(r => r.studentId));
      const done = new Set(existing.map(r => r.studentId));
      let generated = 0; let skipped = 0; let failed = 0;
      for (const student of students) {
        if (locked.has(student.id)) { skipped++; continue; }
        if (done.has(student.id)) continue;
        try {
          await withTenant(session.schoolId, tx2 => generateReportCard(tx2, { schoolId: session.schoolId, actorId: session.userId, studentId: student.id, termId }));
          generated++;
        } catch {
          failed++;
        }
      }
      return { generated, skipped, failed, total: students.length };
    });
    notice = `Generated ${result.generated} report${result.generated === 1 ? "" : "s"}. ${result.failed ? `${result.failed} could not be generated; review readiness and missing scores.` : ""}${result.skipped ? ` ${result.skipped} locked report${result.skipped === 1 ? " was" : "s were"} left unchanged.` : ""}`.trim();
...
export default async function ReportCardsPage({ searchParams }: { searchParams: Promise<{ term?: string; classId?: string; status?: string; notice?: string }> }) {
  ...
  for (const report of data.allReports) {
    const classId = report.student.classId;
    if (!classId) continue;
    const x = classCounts.get(classId) ?? { total: 0, draft: 0, submitted: 0, approved: 0, sent: 0, missing: 0 };
    if (report.status in x) (x as Record<string, number>)[report.status]++;
    classCounts.set(classId, x);
  }
  for (const [classId, x] of classCounts) x.missing = Math.max(0, x.total - x.draft - x.submitted - x.approved - x.sent);
  ...
  return <AppShell universe="school" title="Report Card Studio" subtitle="Select one reporting context, verify results, choose a presentation, then generate and move reports through the approval chain." active="Report Cards" schoolName={data.school.name} schoolCode={data.school.uniqueCode} userName={session.name}>
    <div className="module-workspace">
      ...
      {!data.allReports.length && data.canGenerate && data.term && data.classes.length ? <section className="module-card"><div className="module-section-title"><div><span>Generate first run</span><h3>Start from a class</h3><p>Each button creates only missing report records; readiness errors are isolated per learner.</p></div></div></section> : null}
    </div>
  </AppShell>;
}
