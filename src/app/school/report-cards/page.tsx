  ...
  return (
    <AppShell universe="school" title="Report Cards" subtitle="Prepare, review, approve and release official student reports in a focused class-by-class workflow." active="Report Cards" schoolName={data.school.name} schoolCode={data.school.uniqueCode} userName={session.name}>
      <main className="report-cards-page">
        ...
        <section className="report-hero"><div><span className="report-kicker">OFFICIAL ACADEMIC REPORTING</span><h1>{data.term?.name ?? "Report cards"}</h1><p>Work on one class at a time. Check that marks are ready, generate missing report records, then move each report through review, approval and release.</p></div><div className="report-hero-links"><Link href="/school/gradebook/studio">Gradebook</Link><Link href="/school/reports">Archive</Link><Link href="/school/downloads">Downloads</Link></div></section>
        <section className="report-controls"><div><span className="report-kicker">WORKING CONTEXT</span><strong>{data.selectedClass ? `${data.selectedClass.level ? `${data.selectedClass.level} · ` : ""}${data.selectedClass.name}` : "Choose a class"}</strong><small>{data.term?.name ?? "Choose a reporting term"}</small></div><form method="get" className="report-selects"><label>Term<select name="term" defaultValue={data.term?.id ?? ""}>{data.terms.map((term) => <option key={term.id} value={term.id}>{term.name}</option>)}</select></label><label>Class<select name="classId" defaultValue={params.classId ?? ""}><option value="">Choose class</option>{data.classes.map((item) => <option key={item.id} value={item.id}>{item.level ? `${item.level} · ` : ""}{item.name}</option>)}</select></label><button className="report-button primary" type="submit">Open class</button></form></section>
