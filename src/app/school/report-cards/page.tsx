  ...
  return (
    <AppShell universe="school" title="Report Cards" subtitle="Choose a student, view the result, and print." active="Report Cards" schoolName={data.school.name} schoolCode={data.school.uniqueCode} userName={session.name}>
      ...
      <style jsx>{`
        ...
        .status{display:inline-flex;padding:5px 8px;border-radius:999px;background:var(--sn-canvas);border:1px solid var(--sn-line);font-size:11px;font-weight:800}.status.sent,.status.approved{background:var(--sn-success-soft,#eefbf2)}.status.submitted{background:var(--sn-info-soft,#eef6ff)}.status.missing{background:transparent;color:var(--sn-muted)}
