  ...
  await withTenant(session.schoolId, async (tx) => {
    ...
    for (let attempt = 0; attempt < 8; attempt += 1) {
      const exists = await tx.student.findUnique({
        where: { schoolId_admissionNo: { schoolId: session.schoolId, admissionNo: indexNumber } },
        select: { id: true }
      });
      if (!exists) break;
...
export default async function StudentsPage() {
    ...
    const [school, classes, students] = await Promise.all([
      ...
      tx.student.findMany({
        ...
        select: {
          ...
          guardians: { select: { isPrimary: true, guardian: { select: { name: true, phone: true } } } },
          _count: { select: { attendanceEvents: true, reportCards: true, invoices: true } }
        }
      )
    ]);
    const [scores, attendance] = await Promise.all([
      tx.score.groupBy({ by: ["studentId"], _avg: { value: true } }),
  ...
  const scored = data.students.map((student) => performance.get(student.id)).filter((value): value is number => typeof value === "number");
      ...
      <div className="students-workspace">
        <section className="students-hero students-hero-rich">
          <div>
            ...
            <h2>One living record for every learner.</h2>
            <p>Create a student once and let SukuuNova connect their class, family, attendance, learning, assessments, fees and reports throughout the school journey.</p>
            <div className="hero-statline"><span><b>{data.students.length}</b> learners</span><span><b>{data.classes.length}</b> class groups</span><span><b>{averageSchoolPerformance ? `${averageSchoolPerformance.toFixed(1)}%` : "—"}</b> avg. score</span></div>
          </div>
        ...
        <section className="class-structure-card">
          <div className="section-head"><div><div className="eyebrow">Academic structure</div><h3>Grades, classes & streams</h3><p>Build the structure first: for example <b>Grade 5 → 5A, 5B, 5C</b>. Learners can then be placed in the correct group.</p></div><Link href="/school/classes?action=create" className="button primary">+ Create class</Link></div>
        ...
        <section className="student-register-card">
          <div className="section-head"><div><div className="eyebrow">Learner directory</div><h3>Students at a glance</h3><p>Every learner shows their photo, system-generated index number, class and performance before opening the profile.</p></div><div className="head-links"><Link href="/school/students?view=unassigned">Needs placement ({unassigned.length})</Link><Link href="/school/students?view=active">Active students</Link></div></div>
