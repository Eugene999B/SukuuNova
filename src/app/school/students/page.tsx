...
export default async function StudentsPage() {
  ...
  const data = await withTenant(session.schoolId, async (tx) => {
    ...
    const [school, classes, students] = await Promise.all([
      ...
      tx.student.findMany({ orderBy: [{ name: "asc" }], take: 250, select: { id: true, name: true, admissionNo: true, dob: true, status: true, photoUrl: true, class: { select: { id: true, name: true, level: true } }, guardians: { select: { isPrimary: true, guardian: { select: { name: true, phone: true } } } }, _count: { select: { attendanceEvents: true, reportCards: true, invoices: true } } } }),
    ]);
  ...
  const grouped = data.classes.reduce<Record<string, typeof data.classes>>((acc, item) => { const level = item.level?.trim() || "Unassigned grade"; (acc[level] ??= []).push(item); return acc; }, {});
    ...
    <AppShell universe="school" title="Students" subtitle="A complete learner information centre — identity, placement, family, attendance, learning and fees." active="Students" schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} userName={session.name}>
      <div className="students-workspace">
        <section className="students-hero students-hero-rich">
          <div><div className="eyebrow">People · Student Information System</div><h2>One living record for every learner.</h2><p>Create each learner once and let SukuuNova connect class, family, attendance, academics, assessments, fees and reports throughout the school journey.</p><div className="hero-statline"><span><b>{data.students.length}</b> learners</span><span><b>{data.classes.length}</b> class groups</span><span><b>{assignedCount}</b> placed</span></div></div>
          <div className="hero-actions"><Link href="/school/classes" className="button secondary">Manage classes</Link><a href="#add-student" className="button primary">+ Add student</a></div>
        </section>
        ...
        <section className="student-register-card">
          ...
          {data.students.length === 0 ? <div className="register-empty"><strong>Your student register is empty.</strong><p>Once you create a class and add the first learner, SukuuNova will build the directory automatically.</p><a href="#add-student" className="text-link">Add first student →</a></div> : <div className="student-card-grid">{data.students.map((student) => <Link href={`/school/students/${student.id}`} className="student-card" key={student.id}><div className="student-card-photo">{student.photoUrl ? <img src={student.photoUrl} alt="" /> : <span>{student.name.slice(0, 2).toUpperCase()}</span>}</div><div className="student-card-main"><div className="student-card-top"><span className="student-index">{student.admissionNo}</span><span className={`pill ${student.status === "active" ? "success" : "muted"}`}>{student.status}</span></div><h4>{student.name}</h4><p>{student.class?.level ?? "No grade"} · {student.class?.name ?? "Needs placement"}</p></div><span className="student-card-arrow">→</span></Link>)}</div>}
        </section>

        <section id="add-student" className="student-onboarding-card">
          <div className="onboarding-header"><div><div className="eyebrow">New learner</div><h3>Add a student</h3><p>The system generates the Index Number automatically. Staff do not type or edit it.</p></div><span className="index-badge">AUTO INDEX</span></div>
          <form action={createStudent} className="student-onboarding-form">
            <div className="form-column"><div className="form-section-title"><span>1</span><div><b>Identity & placement</b><small>Establish the learner record and optional class placement.</small></div></div><div className="field-grid"><label>Student full name *<input name="name" required placeholder="e.g. Ama Mensah" /></label><label>Date of birth<input name="dob" type="date" /></label><label>Class group<select name="classId" defaultValue=""><option value="">Leave unassigned</option>{data.classes.map((schoolClass) => <option key={schoolClass.id} value={schoolClass.id}>{schoolClass.level ? `${schoolClass.level} · ` : ""}{schoolClass.name}</option>)}</select></label><div className="generated-index"><span>System-generated index number</span><strong>Created automatically</strong><small>Format: SN-{new Date().getFullYear()}-######</small></div></div></div>
            <div className="form-column"><div className="form-section-title"><span>2</span><div><b>Student photo</b><small>Take it directly on a phone or laptop, or upload an existing portrait.</small></div></div><StudentPhotoCapture /></div>
            <div className="form-column"><div className="form-section-title"><span>3</span><div><b>Primary parent / guardian</b><small>Used later for alerts, receipts, communication and parent access.</small></div></div><div className="field-grid"><label>Guardian name<input name="guardianName" placeholder="e.g. Akosua Mensah" /></label><label>Guardian phone / WhatsApp<input name="guardianPhone" inputMode="tel" placeholder="024 000 0000" /></label><label>Relationship<select name="guardianRelationship" defaultValue="Parent"><option>Parent</option><option>Mother</option><option>Father</option><option>Guardian</option><option>Other</option></select></label></div></div>
            <div className="form-submit"><div><b>Ready to create</b><small>The record becomes part of the learner's permanent school record.</small></div><button className="button primary" type="submit">Create student & generate index →</button></div>
          </form>
        </section>
      </div>
    </AppShell>
