  ...
  return (
    <AppShell universe="school" title="Students" subtitle="Manage the learner register, placement, family links and connected school records." active="Students" schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} userName={session.name}>
      <div className="students-workspace">
        <section className="students-hero students-hero-rich"><div><div className="eyebrow">People · Student register</div><h2>Every learner, clearly organised.</h2><p>Search and manage the school register, resolve placement issues and open the full learner record when you need the detail.</p><div className="hero-statline"><span><b>{data.students.length}</b> learners</span><span><b>{activeCount}</b> active</span><span><b>{data.classes.length}</b> class groups</span></div></div><div className="hero-actions"><Link href="/school/id-cards" className="button secondary"><IdCard size={15} aria-hidden="true" /> School ID cards</Link><Link href="/school/classes" className="button secondary"><GraduationCap size={15} aria-hidden="true" /> Manage classes</Link><AddStudentDialog classes={data.classes} action={createStudent} /></div></section>
