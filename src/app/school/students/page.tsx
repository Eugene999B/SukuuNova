...
async function createStudent(formData: FormData) {
  ...
  await withTenant(session.schoolId, async (tx) => {
    ...
    await tx.auditLogSchool.create({ create: { schoolId: session.schoolId, actorId: session.userId, action: "student.created", entityType: "Student", entityId: student.id, after: { name, indexNumber, classId: classId || null, guardianLinked: Boolean(guardianName && guardianPhone), photoCaptured: Boolean(photoData) } } } as never).catch(() => undefined);
  });
    ...
    <AppShell universe="school" title="Students" subtitle="Manage the learner register, placement, family links and connected school records." active="Students" schoolName={data.school?.name ?? "School Workspace"} schoolCode={data.school?.uniqueCode ?? ""} userName={session.name}>
      <div className="students-workspace">
        <section className="students-hero students-hero-rich">
          <div>
            <div className="eyebrow">People · Student register</div>
            <h2>Every learner, clearly organised.</h2>
            <p>Search and manage the school register, resolve placement issues and open the full learner record when you need the detail.</p>
            <div className="hero-statline"><span><b>{data.students.length}</b> learners</span><span><b>{activeCount}</b> active</span><span><b>{data.classes.length}</b> class groups</span></div>
          </div>
        ...
        <section className="student-metrics">
          ...
          <article className={unassigned.length ? "attention" : "ok"}><span>Needs class placement</span><strong>{unassigned.length}</strong><small>{unassigned.length ? "Open the filtered register to resolve" : "Every learner is placed"}</small></article>
        </section>

        <section className="class-structure-card">
          <div className="section-head"><div><div className="eyebrow">Academic structure</div><h3>Grades, classes & streams</h3><p>See learner distribution by academic level and jump straight into a class roster.</p></div><Link href="/school/classes?action=create" className="button primary">+ Create class</Link></div>
          {data.classes.length === 0 ? <div className="structure-empty"><div className="empty-icon">◎</div><strong>No classes created yet</strong><p>Create your grades and streams before enrolling learners so class-based workflows stay organised.</p><Link href="/school/classes?action=create" className="text-link">Create the first class →</Link></div> : <div className="grade-grid">{Object.entries(grouped).map(([level, sections]) => <article className="grade-card" key={level}><div className="grade-title"><div><span className="grade-kicker">Academic level</span><h4>{level}</h4></div><span className="grade-total">{sections.reduce((sum, item) => sum + item._count.students, 0)} learners</span></div><div className="section-list">{sections.map((schoolClass) => <Link href={`/school/classes?class=${schoolClass.id}`} className="section-row" key={schoolClass.id}><span className="section-badge">{schoolClass.name.slice(0, 2).toUpperCase()}</span><span className="section-main"><b>{schoolClass.name}</b><small>{schoolClass._count.students} learners in this class</small></span><span className="section-count">{schoolClass._count.students}</span><span className="chevron">›</span></Link>)}</div></article>)}</div>}
        </section>
        ...
        <section className="student-register-card">
          ...
          {data.students.length === 0 ? <div className="register-empty"><div className="empty-icon">◌</div><strong>Start the student register</strong><p>No invented students are shown. Create a learner and SukuuNova will generate the permanent Index Number and connect the learner to the chosen class and guardian.</p><AddStudentDialog classes={data.classes} action={createStudent} triggerLabel="Create first student →" /></div> : <div className="student-card-grid">{data.students.map((student) => <Link href={`/school/students/${student.id}`} className="student-card" key={student.id}><div className="student-card-photo">{student.photoUrl ? <img src={student.photoUrl} alt="" /> : <span>{student.name.slice(0, 2).toUpperCase()}</span>}</div><div className="student-card-main"><div className="student-card-top"><span className="student-index">{student.admissionNo}</span><span className={`pill ${student.status === "active" ? "success" : "muted"}`}>{student.status}</span></div><h4>{student.name}</h4><p>{student.class?.level ?? "No grade"} · {student.class?.name ?? "Needs placement"}</p><small>{student._count.reportCards} reports · {student._count.attendanceEvents} attendance events · {student._count.invoices} invoices</small></div><span className="student-card-arrow">→</span></Link>)}</div>}
        </section>

        <section className="student-actions-strip">
          <div><div className="eyebrow">Next actions</div><h3>Keep the register moving.</h3></div>
          <div className="student-actions-grid"><Link href="/school/students?view=unassigned"><strong>{unassigned.length} learners</strong><span>Need class placement →</span></Link><Link href="/school/classes"><strong>{data.classes.length} class groups</strong><span>Manage class structure →</span></Link><Link href="/school/guardians"><strong>Family records</strong><span>Review guardian links →</span></Link><Link href="/school/attendance"><strong>Attendance</strong><span>Open student register →</span></Link></div>
        </section>
      </div>
    </AppShell>
  );
