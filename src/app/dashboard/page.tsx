...
const overviewMetrics = [
  ["♟", "Students", "students"], ["⌂", "Classes", "classes"], ["▤", "Subjects", "subjects"], ["♧", "Guardians", "guardians"], ["₵", "Invoices", "invoices"], ["↙", "Payments", "payments"]
] as const;
  ...
  return <AppShell universe="school" title={`Good morning, ${name.split(" ")[0]} 👋`} subtitle={`${school} · ${code} · ${role}`} active="Overview" schoolName={school} schoolCode={code} userName={name} role={role}>
    <div className="overview-workspace">
      <section className="overview-welcome">
        <div>
          <span className="overview-kicker">School command centre</span>
          <h2>Everything important, at a glance.</h2>
          <p>Your live school snapshot brings people, academics, attendance and finance into one view.</p>
        </div>
        <div className="overview-live"><i/>Live school data</div>
      </section>

      <section className="overview-kpi-grid" aria-label="School totals">
        {overviewMetrics.slice(0,4).map(([icon,label,key])=><Kpi key={key} icon={icon} label={label} value={String(stats[key])} meta={stats[key] ? "Current total" : "Ready for records"}/>)}
      </section>

      <section className="overview-stat-layout">
        <section className="overview-stat-card">
          <div className="overview-section-head"><div><span className="overview-kicker">School statistics</span><h3>What is happening across the school?</h3><p>Live totals from the records already in this school.</p></div><div className="overview-total"><strong>{totalTracked}</strong><span>tracked records</span></div></div>
          <div className="overview-chart" role="img" aria-label="Current school record counts">
          ...
          <div className="overview-chart-foot"><span><i className="legend-dot active"/>Real-time records</span><span><i className="legend-dot empty"/>A quiet area becomes active as records are added</span></div>
        </section>

        <section className="overview-glance-card">
          <div className="overview-section-head compact"><div><span className="overview-kicker">At a glance</span><h3>Operating pulse</h3></div><span className="overview-pulse">Live</span></div>
          <div className="overview-glance-list">
            {[['Attendance records',stats.attendance,'Today'],['Calendar events',stats.events,'Scheduled'],['Fee items',stats.feeItems,'Configured'],['Invoices',stats.invoices,'Issued'],['Payments',stats.payments,'Recorded']].map(([label,value,meta])=><div className="overview-glance-row" key={label as string}><span className="overview-row-icon">{String(label).charAt(0)}</span><div><b>{label}</b><small>{meta}</small></div><strong>{value}</strong></div>)}
          </div>
          <div className="overview-pulse-note"><span>✦</span><div><strong>One school picture</strong><p>These totals stay connected to the underlying records, so the dashboard grows with the school.</p></div></div>
        </section>
      </section>

      <section className="overview-lower-grid">
        <section className="app-card app-panel overview-setup"><div className="app-card-head"><div><span className="overview-kicker">Setup progress</span><h2>Get the school ready</h2><p>{completed} of {setup.length} essentials configured</p></div><div className="overview-progress"><span>{setupPercent}%</span><i style={{"--progress":`${setupPercent}%`} as React.CSSProperties}/></div></div><div className="app-list">{setup.map(([label,done])=><div className="app-list-row" key={label}><span className="app-list-icon">{done?"✓":"○"}</span><div><b>{label}</b><span>{done?"Configured":"Not configured"}</span></div></div>)}</div></section>
        <section className="app-card app-panel"><div className="app-card-head"><div><span className="overview-kicker">Recent communication</span><h2>What needs your attention?</h2><p>Latest messages addressed to this account.</p></div></div>{stats.announcements.length?<div className="app-list">{stats.announcements.map((item)=><div className="app-list-row" key={item.id}><span className="app-list-icon">◈</span><div><b>{item.body.split("\n")[0]}</b><span>{item.body.split("\n").slice(2).join(" ").slice(0,110)}</span></div></div>)}</div>:<div className="overview-empty-message"><div className="empty-message-icon">✉</div><div><strong>Your inbox is quiet.</strong><p>New school messages will appear here as they arrive.</p></div></div>}</section>
        <section className="app-card app-panel overview-actions"><div className="app-card-head"><div><span className="overview-kicker">Quick actions</span><h2>Do something now</h2><p>Jump straight into the work that changes school records.</p></div></div><div className="app-actions"><Link className="app-action" href="/school/students"><strong>＋ Add student</strong>New learner record</Link><Link className="app-action" href="/school/classes"><strong>＋ Create class</strong>New class group</Link><Link className="app-action" href="/school/attendance"><strong>✓ Attendance</strong>Mark a register</Link><Link className="app-action" href="/school/communications/announcements"><strong>◈ Announcement</strong>Send an update</Link></div></section>
      </section>
    </div>
  </AppShell>;
}
