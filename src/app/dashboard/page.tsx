...
function SchoolDashboard({ name, school, code, role, stats }: { name:string; school:string; code:string; role:string; stats:SchoolStats }) {
  ...
  return <AppShell universe="school" title={`Good morning, ${name.split(" ")[0]} 👋`} subtitle={`${school} · ${code} · ${role}`} active="Overview" schoolName={school} schoolCode={code} userName={name} role={role}>
    <div className="overview-workspace">
      <section className="overview-welcome"><div><span className="overview-kicker">School command centre</span><h2>Everything important, at a glance.</h2><p>Your live school snapshot brings people, academics, attendance and finance into one view.</p></div><div className="overview-live"><i/>Live school data</div></section>
