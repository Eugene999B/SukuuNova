...
export default async function DashboardPage() {
  ...
  if (schoolSession) {
    ...
    const todayKey = localDateInTimeZone(new Date(), overview.timezone);
    ...
    const schoolOverview = await withTenant(schoolSession.schoolId, async (tx) => {
      ...
      return {
        ...
        pendingFeeAdjustments: pendingFeeAdjustments[0]?.count ?? 0, pendingStaff, activatedNonOwnerStaff, hasBranding: Boolean(schoolBranding?.logoUrl || schoolBranding?.brandColors),
        classesWithoutTeacher, teachingAssignments, timetableSlots,
      };
    });
    ...
    if (isFinanceRole(roleKeys)) {
      return <AppShell universe="school" title="Finance intelligence" subtitle={`${overview.account.school.name} · ${overview.account.school.uniqueCode}`} active="Overview" schoolName={overview.account.school.name} schoolCode={overview.account.school.uniqueCode} userName={overview.account.name} role={role}>
        <FinanceDashboard name={overview.account.name} school={overview.account.school.name} code={overview.account.school.uniqueCode} role={role} stats={{ invoices: schoolOverview.invoices, payments: schoolOverview.payments, pendingFeeAdjustments: schoolOverview.pendingFeeAdjustments, feeItems: schoolOverview.feeItems, students: schoolOverview.students }} />
      </AppShell>;
    }

    return <SchoolRoleDashboard name={overview.account.name} school={overview.account.school.name} code={overview.account.school.uniqueCode} role={role} profile={dashboardProfile(roleKeys)} stats={schoolOverview} />;
  }
...
type SchoolStats = {
  students: number; guardians: number; staff: number; classes: number; subjects: number; feeItems: number; invoices: number; payments: number; todayAttendance: number; events: number;
  announcements: Array<{ id: string; body: string; createdAt: Date }>; academicYears: number; terms: number; reportTemplates: number; pendingStaff: number; pendingReportCards: number;
  pendingFeeAdjustments: number; activatedNonOwnerStaff: number; hasBranding: boolean; classesWithoutTeacher: number; teachingAssignments: number; timetableSlots: number;
};
...
function SchoolRoleDashboard({ name, school, code, role, profile, stats }: { name: string; school: string; code: string; role: string; profile: DashboardProfile; stats: SchoolStats }) {
  ...
  if (stats.classes > 0 && stats.teachingAssignments === 0) commonInsights.push({ title: "Teaching assignments are not connected", detail: "Classes exist, but no class-subject-teacher connections are configured yet.", href: "/school/academic-setup", severity: "critical", actionLabel: "Configure" });
  ...
  const profileCopy: Record<DashboardProfile, { eyebrow: string; title: string; description: string; primary: { label: string; href: string }; secondary: { label: string; href: string } }> = {
    ...
    academics: { eyebrow: "Academic intelligence", title: `Good morning, ${firstName}. Keep teaching, assessment and reporting connected.`, description: "This home prioritises academic setup, teacher-class-subject connections, timetable activity and report workflow readiness.", primary: { label: "Academic setup", href: "/school/academic-setup" }, secondary: { label: "Gradebook", href: "/school/gradebook" } },
    people: { eyebrow: "People intelligence", title: `Good morning, ${firstName}. Keep the staff structure healthy and ready.`, description: "This home centres on staff activation, workforce visibility and the people connections that affect daily school operations.", primary: { label: "People & access", href: "/school/settings/access" }, secondary: { label: "Staff directory", href: "/school/staff" } },
    ...
    frontdesk: { eyebrow: "Front desk intelligence", title: `Good morning, ${firstName}. See today’s people flow immediately.`, description: "Attendance activity, people records and the fastest operational actions are placed first for reception and gate work.", primary: { label: "Attendance control", href: "/school/devices" }, secondary: { label: "Manual register", href: "/school/attendance/register" } },
    ...
    operations: { eyebrow: "School operations intelligence", title: `Good morning, ${firstName}. Start from what needs action.`, description: "Your home now surfaces the school records, exceptions and workflows most useful to an operational staff account.", primary: { label: "School overview", href: "/school/reports" }, secondary: { label: "Settings", href: "/school/settings" } },
  };
  const copy = profileCopy[profile];

  let insights = commonInsights;
  if (profile === "people") insights = [
    ...(stats.pendingStaff > 0 ? commonInsights.filter((item) => item.title.includes("Staff accounts")) : []),
    { title: "Active workforce visibility", detail: `${stats.staff} active school account${stats.staff === 1 ? "" : "s"} are currently visible in this tenant.`, href: "/school/staff", severity: "positive", actionLabel: "Open staff" },
    ...commonInsights.filter((item) => item.title.includes("class teacher")),
  ];
  ...
  if (profile === "frontdesk") insights = [
    { title: "Today’s attendance channel is active", detail: `${stats.todayAttendance} attendance event${stats.todayAttendance === 1 ? "" : "s"} have been recorded for today so far.`, href: "/school/attendance", severity: stats.todayAttendance > 0 ? "positive" : "info", actionLabel: "Monitor" },
    ...(stats.pendingStaff > 0 ? commonInsights.filter((item) => item.title.includes("Staff accounts")) : []),
  ];

  const actionsByProfile: Record<DashboardProfile, IntelligenceAction[]> = {
    leadership: [
      { label: "Add learner", detail: "Create a new student record.", href: "/school/students" }, { label: "Manage staff", detail: "Open people and access controls.", href: "/school/settings/access" }, { label: "Review reports", detail: "Approve and print report cards.", href: "/school/report-cards" }, { label: "School settings", detail: "Configure school-wide policies.", href: "/school/settings" },
    ],
    academics: [
      { label: "Academic setup", detail: "Classes, subjects, teachers and terms.", href: "/school/academic-setup" }, { label: "Timetable", detail: "Generate or review the official schedule.", href: "/school/timetable" }, { label: "Gradebook", detail: "Monitor assessment and marks.", href: "/school/gradebook" }, { label: "Report cards", detail: "Review reporting workflow.", href: "/school/report-cards" },
    ],
    people: [
      { label: "Add staff", detail: "Create a staff account.", href: "/school/staff" }, { label: "People & access", detail: "Activate roles and permissions.", href: "/school/settings/access" }, { label: "Staff attendance", detail: "Review workforce attendance.", href: "/school/attendance/staff" }, { label: "Roles", detail: "Open role responsibilities.", href: "/school/settings/roles" },
    ],
    admissions: [
      { label: "Add learner", detail: "Create a learner profile.", href: "/school/students" }, { label: "Guardians", detail: "Manage family records.", href: "/school/guardians" }, { label: "Classes", detail: "Review placement options.", href: "/school/classes" }, { label: "Student records", detail: "Open the learner directory.", href: "/school/students" },
    ],
    frontdesk: [
      { label: "Attendance control", detail: "Monitor entry and exit channels.", href: "/school/devices" }, { label: "Manual register", detail: "Record a fallback attendance decision.", href: "/school/attendance/register" }, { label: "Students", detail: "Find a learner quickly.", href: "/school/students" }, { label: "Staff", detail: "Find a staff member quickly.", href: "/school/staff" },
    ],
    transport: [
      { label: "Transport", detail: "Open transport operations.", href: "/school/transport" }, { label: "Attendance", detail: "See school-day arrival signals.", href: "/school/attendance" }, { label: "Students", detail: "Open learner records.", href: "/school/students" }, { label: "Messages", detail: "Check school communication.", href: "/school/communications/messages" },
    ],
    operations: [
      { label: "Students", detail: "Open learner records.", href: "/school/students" }, { label: "Staff", detail: "Open staff records.", href: "/school/staff" }, { label: "Attendance", detail: "Review attendance activity.", href: "/school/attendance" }, { label: "Reports", detail: "Open school reports.", href: "/school/reports" },
    ],
  };
  ...
  return <AppShell universe="school" title="Home intelligence" subtitle={`${school} · ${code}`} active="Overview" schoolName={school} schoolCode={code} userName={name} role={role}>
    <RoleIntelligenceHome
      ...
      focus={[
        { label: "Academic structure", detail: `${stats.subjects} subjects · ${stats.teachingAssignments} teacher-subject connections.`, value: `${stats.timetableSlots} timetable slots`, href: "/school/academic-setup" },
