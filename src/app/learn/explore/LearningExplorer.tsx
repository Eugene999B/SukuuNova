...
const modes: Array<{ id: PracticeMode; label: string; description: string }> = [
  { id: "topic", label: "Topic focus", description: "Stay as close as possible to the selected topic." },
  { id: "adaptive", label: "Adaptive", description: "Start accessible and increase challenge as the session develops." },
  { id: "random", label: "Smart random", description: "Mix question formats and concepts while avoiding recent exposure." },
  { id: "timed", label: "Timed", description: "A faster practice rhythm for exam preparation." },
  { id: "weakness", label: "Weak-area repair", description: "Use saved performance signals to revisit weaker concepts." },
];
...
export function LearningExplorer() {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) setProgress({ ...EMPTY_PROGRESS, ...JSON.parse(stored) });
    } catch {
    ...
    <main className={styles.page}>
      ...
      <section className={styles.hero}>
        <div>
          ...
          <h1>Choose the exact lane. <em>SukuuNova builds the practice.</em></h1>
          <p>Curriculum, level, subject, topic, practice mode and question format now feed a real session engine. Your recent exposure and mastery signals are remembered on this device.</p>
        </div>
      ...
      <section className={styles.workspace}>
        <aside className={styles.laneRail}>
          ...
          {lanes.map((item) => {
            ...
            return (
              <button key={item.id} className={lane === item.id ? styles.laneActive : styles.laneButton} onClick={() => resetSelectionForLane(item.id)}>
        ...
        <div className={styles.builder}>
          ...
          <div className={styles.selectorBlock}>
            <label>Program / exam / discipline</label>
          ...
          <div className={styles.twoColumns}>
            <div className={styles.selectorBlock}>
              <label>Level / pathway</label>
              <select value={levelId} onChange={(event) => selectLevel(event.target.value)}>{program.levels.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select>
            </div>
            <div className={styles.selectorBlock}>
      ...
      <section className={styles.masterySection}>
        <div className={styles.sectionTitle}><div><span className={styles.stepLabel}>04 · MASTERY</span><h2>Your learning map starts immediately.</h2></div><p>This local profile is the bridge to a future signed-in Learner Brain without requiring database changes while Gradebook work is active.</p></div>
  ...
  return (
    <div className={styles.playerCard}>
      ...
      {question.kind === "single" && <div className={styles.optionGrid}>{question.options?.map((option) => {
        ...
        return <button key={option.id} disabled={submitted} className={`${styles.optionButton} ${selected ? styles.optionSelected : ""} ${correctOption ? styles.optionCorrect : ""} ${wrong ? styles.optionWrong : ""}`} onClick={() => setResponse(option.id)}><span>{option.label}</span>{correctOption ? <Check size={17} /> : wrong ? <X size={17} /> : null}</button>;
      })}</div>}
      ...
      {question.kind === "boolean" && <div className={styles.booleanRow}>{[true, false].map((value) => <button key={String(value)} disabled={submitted} className={response === value ? styles.booleanActive : styles.booleanButton} onClick={() => setResponse(value)}>{value ? "True" : "False"}</button>)}</div>}

      {["fill", "short"].includes(question.kind) && <div className={styles.textAnswer}><input disabled={submitted} value={typeof response === "string" ? response : ""} onChange={(event) => setResponse(event.target.value)} placeholder={question.kind === "short" ? "Type your short answer" : "Fill in the answer"} onKeyDown={(event) => { if (event.key === "Enter") submit(); }} /></div>}

      {question.kind === "numeric" && <div className={styles.textAnswer}><input disabled={submitted} inputMode="decimal" value={typeof response === "number" || typeof response === "string" ? response : ""} onChange={(event) => setResponse(event.target.value)} placeholder="Enter your numerical answer" onKeyDown={(event) => { if (event.key === "Enter") submit(); }} /></div>}

      {!submitted ? <div className={styles.answerFooter}><span><Brain size={15} /> Skill: {question.skill}</span><button onClick={submit}>Check answer <ArrowRight size={16} /></button></div> : <div className={correct ? styles.correctFeedback : styles.wrongFeedback}><div className={styles.feedbackSymbol}>{correct ? <CheckCircle2 size={22} /> : <XCircle size={22} />}</div><div><strong>{correct ? "Correct." : "Not quite."}</strong><p>{question.explanation}</p>{question.hint && !correct && <span>Hint for the next variant: {question.hint}</span>}</div><button onClick={next}>{index + 1 === total ? "View results" : "Next question"} <ArrowRight size={16} /></button></div>}
    </div>
  );
}
