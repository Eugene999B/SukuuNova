...
describe("SukuuNova Question Foundry review queue", () => {
  it("normalizes standard and rich starter content into one queue", () => {
    expect(FOUNDRY_REVIEW_SUMMARY.standard).toBe(10);
    expect(FOUNDRY_REVIEW_SUMMARY.rich).toBe(4);
    expect(FOUNDRY_REVIEW_SUMMARY.total).toBe(14);
    expect(new Set(FOUNDRY_REVIEW_ROWS.map((row) => row.id)).size).toBe(14);
  });

  it("shows the current reviewed starter library as publishable and error free", () => {
    expect(FOUNDRY_REVIEW_SUMMARY.publishable).toBe(14);
  ...
  it("filters review rows by family, status, subject and search text", () => {
    ...
    expect(filterFoundryRows({ family: "standard" })).toHaveLength(10);
