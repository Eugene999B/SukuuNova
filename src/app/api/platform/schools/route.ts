...
const schema = z.object({
  ...
  billingMode: z.enum(["flat", "per_student"]).default("flat"),
  studentRate: z.coerce.number().min(0).default(0),
  flatRate: z.coerce.number().min(0).default(0),
  ...
  try {
    ...
    await withTenant(result.school.id, async (tx) => {
      ...
      await tx.$executeRawUnsafe(
        `INSERT INTO "PlatformSchoolBillingConfig" ("schoolId","billingMode","currency","studentRate","flatRate","billingDay","graceDays","trialDays","minimumCharge","maximumCharge","active","updatedAt") VALUES ($1,$2,$3,$4,$5,$6,$7,$8,0,NULL,true,CURRENT_TIMESTAMP) ON CONFLICT ("schoolId") DO UPDATE SET "billingMode"=EXCLUDED."billingMode","currency"=EXCLUDED."currency","studentRate"=EXCLUDED."studentRate","flatRate"=EXCLUDED."flatRate","billingDay"=EXCLUDED."billingDay","graceDays"=EXCLUDED."graceDays","trialDays"=EXCLUDED."trialDays","updatedAt"=CURRENT_TIMESTAMP`,
        result.school.id, input.billingMode, input.currency.toUpperCase(), input.studentRate, input.flatRate, input.billingDay, input.graceDays, input.trialDays,
      );
    });
    return NextResponse.json({ ok: true, result: { ...result, billing: { billingMode: input.billingMode, currency: input.currency, studentRate: input.studentRate, flatRate: input.flatRate, graceDays: input.graceDays, trialDays: input.trialDays } } }, { status: 201 });
  } catch (error) { return routeError(error); }
}
