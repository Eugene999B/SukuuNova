...
const schema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("lifecycle"), schoolId: z.string().min(1), lifecycle: z.enum(["lock", "suspend", "reactivate", "archive", "delete"]), confirmationPhrase: z.string().max(160).optional() }),
]);

type SchoolSelectorRow = { id: unknown; name: unknown; uniqueCode: unknown; status: unknown; studentCount?: unknown; subscriptionPlan?: unknown };
function normalizeSchoolRow(row: SchoolSelectorRow) {
  ...
  return { id: String(candidate.id), name: String(candidate.name), uniqueCode: String(candidate.uniqueCode), status: String(candidate.status), studentCount: Number(candidate.studentCount ?? 0), subscriptionPlan: plan ? { name: String(plan.name ?? ""), price: Number(plan.price ?? 0) } : null };
}
...
export async function POST(request: Request) {
  try {
    const session = await requirePlatformSession(); const input = schema.parse(await request.json());
    ...
    return NextResponse.json({ ok: true, result: await performSchoolLifecycle(session, input.schoolId, input.lifecycle, input.confirmationPhrase) });
  } catch (error) { return routeError(error); }
}
