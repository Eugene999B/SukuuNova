...
export async function POST(request: Request) {
  try {
    ...
    if (input.action === "generate") {
      const result = await generateBillingRulesInvoice(session, input.schoolId, input.period);
      return NextResponse.json({ ok: true, invoice: result, idempotent: result.existing }, { status: result.existing ? 200 : 201 });
    }
    ...
    const result = await withTenant(input.schoolId, async (tx) => {
      ...
      await appendPlatformAudit({ actorId: session.adminId, action: "platform.billing.payment_recorded", targetSchoolId: input.schoolId, targetEntity: `PlatformInvoice:${input.invoiceId}`, meta: { paymentId, invoiceId: input.invoiceId, due, paid, outstanding: Math.max(0, due - paid), overpaid: Math.max(0, paid - due), status, method: input.method, reference: input.reference ?? null } }, tx);
