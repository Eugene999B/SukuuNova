...
import { createPlatformSchool, listBilling, updateSupportTicket } from "@/lib/phase4-service";
...
async function hardenedRecordPayment(schoolId:string,invoiceId:string,paymentAmount:number,method:string,reference:string|undefined,adminId:string){
  const result=await withTenant(schoolId,async(tx)=>{
    ...
    if(reference){
      const duplicate=await tx.$queryRawUnsafe<Array<{id:string;platformInvoiceId:string;amount:string;method:string}>>(`SELECT \"id\",\"platformInvoiceId\",\"amount\"::text,\"method\" FROM \"PlatformPayment\" WHERE \"schoolId\"=$1 AND \"reference\"=$2 LIMIT 1`,schoolId,reference);
      if(duplicate.length){
        const existing=duplicate[0];
        if(existing.platformInvoiceId!==invoiceId) return {duplicateReference:true} as const;
  ...
  if("idempotent" in result) return result;
