...
import { requireActorOwnsSchool, requireBranchesAreOwnedByActor } from "@/lib/group-security-service";
import { listOwnSupportTickets } from "@/lib/support-access-service";
...
export async function GET(){try{const session=await requireSchoolSession();const data=await withTenant(session.schoolId,async(tx)=>{const canRisk=await hasPermission(tx,session.userId,"risk_flags:view"),canAi=await hasPermission(tx,session.userId,"ai_drafts:accept"),canSupportManage=await hasPermission(tx,session.userId,"support:manage"),canSupportOwn=await hasPermission(tx,session.userId,"support:view_own"),isOwner=Boolean(await tx.userRole.findFirst({where:{userId:session.userId,role:{key:"owner"}},select:{userId:true}}));const support=canSupportManage?listSupportTickets(session.schoolId,session.userId):canSupportOwn?listOwnSupportTickets(tx,session.schoolId,session.userId):Promise.resolve([]);const[supportResult,notice,risk,drafts,groupReport]=await Promise.all([support,schoolImpersonationNotice(session.schoolId,session.userId),canRisk?riskFlags(session.schoolId,session.userId):Promise.resolve([]),canAi?listAiDrafts(session.schoolId,session.userId):Promise.resolve([]),isOwner?ownerGroupReport({schoolId:session.schoolId,actorId:session.userId}):Promise.resolve([])]);return{support:supportResult,notice,risk,drafts,groupReport};});return NextResponse.json(data);}catch(error){return routeError(error);}}

export async function POST(request:Request){try{const session=await requireSchoolSession();const input=await parseJson(request,schema);switch(input.action){case"createTicket":await withTenant(session.schoolId,tx=>requirePermission(tx,session.userId,"support:create"));return NextResponse.json({ok:true,result:await createSupportTicket({schoolId:session.schoolId,actorId:session.userId,subject:input.subject,body:input.body})},{status:201});case"aiGenerate":return NextResponse.json({ok:true,result:await createStructuredAiDraft({schoolId:session.schoolId,actorId:session.userId,type:input.type,context:input.context})},{status:201});case"aiAccept":return NextResponse.json({ok:true,result:await acceptStructuredAiDraft({schoolId:session.schoolId,actorId:session.userId,draftId:input.draftId,editedText:input.editedText})});case"aiDiscard":return NextResponse.json({ok:true,result:await discardStructuredAiDraft({schoolId:session.schoolId,actorId:session.userId,draftId:input.draftId})});case"emergencyPrepare":return NextResponse.json({ok:true,result:await withTenant(session.schoolId,(tx)=>prepareEmergencySnapshot(tx,{schoolId:session.schoolId,actorId:session.userId,message:input.message}))});case"emergencyConfirm":return NextResponse.json({ok:true,result:await withTenant(session.schoolId,(tx)=>confirmEmergencySnapshot(tx,{schoolId:session.schoolId,actorId:session.userId,confirmationToken:input.confirmationToken,message:input.message}))});case"groupCreate":await requireActorOwnsSchool(session.schoolId,session.userId);await requireBranchesAreOwnedByActor(input.memberSchoolIds,session.userId);return NextResponse.json({ok:true,result:await createSchoolGroup({schoolId:session.schoolId,actorId:session.userId,name:input.name,memberSchoolIds:input.memberSchoolIds})},{status:201});case"groupAddBranch":await requireActorOwnsSchool(session.schoolId,session.userId);await requireActorOwnsSchool(input.branchSchoolId,session.userId);return NextResponse.json({ok:true,result:await addGroupBranch({schoolId:session.schoolId,actorId:session.userId,groupId:input.groupId,branchSchoolId:input.branchSchoolId})});}}catch(error){return routeError(error);}}