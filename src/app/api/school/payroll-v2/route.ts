...
import { markPayrollPaidV2, payrollV2Snapshot, runPayrollV2, saveSalaryStructureV2 } from "@/lib/payroll-v2-service";
...
export async function POST(request:Request){try{const session=await requireSchoolSession();const input=await parseJson(request,schema);const result=await withTenant(session.schoolId,tx=>{switch(input.action){case"salary.save":return saveSalaryStructureV2(tx,{schoolId:session.schoolId,actorId:session.userId,...input});case"run.process":return runPayrollV2(tx,{schoolId:session.schoolId,actorId:session.userId,period:input.period,staffIds:input.staffIds});case"run.mark_paid":return markPayrollPaidV2(tx,{schoolId:session.schoolId,actorId:session.userId,runId:input.runId,staffIds:input.staffIds,paymentReference:input.paymentReference});}});return NextResponse.json({ok:true,result},{headers:{"cache-control":"private, no-store"}});}catch(error){return routeError(error);}}
