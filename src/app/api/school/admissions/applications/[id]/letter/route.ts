...
async function buildPdf(input: {
  ...
  application: Awaited<ReturnType<typeof getAdmissionApplication>> & {};
}) {
  ...
  for (const [label, value] of details) {
    page.drawText(`${label}:`, { x: margin + 16, y: detailY, size: 9.5, font: bold, color: primary });
...
function buildWordHtml(input: { school: { name: string; uniqueCode: string; logoUrl: string | null }; application: NonNullable<Awaited<ReturnType<typeof getAdmissionApplication>>> }) {
  const { school, application } = input;
  const className = `${application.classLevel ? `${application.classLevel} · ` : ""}${application.className ?? application.intendedClassName ?? "Approved class"}`;
  const logo = school.logoUrl ? `<img src="${escapeHtml(school.logoUrl)}" style="width:72px;height:72px;object-fit:contain" alt="School logo"/>` : `<div style="width:68px;height:68px;border:2px solid #173B57;border-radius:50%;text-align:center;line-height:68px;font:bold 15px Arial;color:#173B57">${escapeHtml(initials(school.name))}</div>`;
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!data.school || !data.application || !["offered", "accepted", "enrolled"].includes(data.application.status)) return new Response("Admission letter not available.", { status: 404 });
  const base = safeFilename(`${data.application.reference}-${data.application.studentName}`);
  if (format === "word") {
    const html = buildWordHtml({ school: data.school, application: data.application });
    return new Response(html, { headers: { "content-type": "application/msword; charset=utf-8", "content-disposition": `attachment; filename="${base}.doc"`, "cache-control": "private, no-store" } });
  }
  const bytes = await buildPdf({ school: data.school, application: data.application });
  return new Response(Buffer.from(bytes), { headers: { "content-type": "application/pdf", "content-disposition": `attachment; filename="${base}.pdf"`, "cache-control": "private, no-store" } });
}
