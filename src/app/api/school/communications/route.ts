...
type ExternalChannel = "sms" | "whatsapp";

const sendSchema = z.object({
  action: z.literal("send"),
  title: z.string().trim().min(2).max(160),
  body: z.string().trim().min(2).max(5000),
  audience: z.enum(["guardians", "teachers", "staff", "individual"]),
...
const broadcastSchema = z.object({
  ...
  title: z.string().trim().min(2).max(160),
  body: z.string().trim().min(2).max(5000),
  audience: z.enum(["guardians", "teachers", "staff", "all"]),
...
const eventSchema = z.object({
  ...
  affectsAttendance: eventFlag,
  affectsTransport: eventFlag,
  notifyGuardians: eventFlag,
  notifyStaff: eventFlag,
});
...
function flag(value: boolean | string | undefined): boolean | undefined {
  if (value === undefined) return undefined;
  if (typeof value === "boolean") return value;
  return ["true", "1", "yes", "on"].includes(value.trim().toLowerCase());
}
function externalBody(title: string, body: string) {
  return `${title}\n\n${body}`;
}
function configuredChannelList(value: unknown) {
  const candidate = Array.isArray(value) ? value : asRecord(value).channels;
  if (!Array.isArray(candidate)) return ["in_app"];
  const channels = candidate.filter((item): item is string => item === "in_app" || item === "sms" || item === "whatsapp");
  return [...new Set(["in_app", ...channels])];
}
function configuredAutomation(value: unknown) {
  return asRecord(asRecord(value).automation);
}
async function canCommunicate(schoolId: string, userId: string) {
  return withTenant(schoolId, async (tx) => {
    const access = await getSchoolAuthorization(tx, userId);
    return access.isOwner || await hasPermission(tx, userId, "communications:manage");
  });
}
async function canManageCalendar(schoolId: string, userId: string) {
  return withTenant(schoolId, (tx) => hasPermission(tx, userId, "calendar:manage"));
}
...
async function wallet(tx: Prisma.TransactionClient, schoolId: string) {
  const rows = await tx.$queryRawUnsafe<Array<{ smsBalance: number; whatsappBalance: number; smsSellRate: string; whatsappSellRate: string; lowBalanceThreshold: number; status: string }>>(
    `SELECT "smsBalance","whatsappBalance","smsSellRate"::text,"whatsappSellRate"::text,"lowBalanceThreshold","status" FROM "PlatformMessagingWallet" WHERE "schoolId"=$1`,
    schoolId,
  );
  return rows[0] ?? { smsBalance: 0, whatsappBalance: 0, smsSellRate: "0", whatsappSellRate: "0", lowBalanceThreshold: 50, status: "unfunded" };
}
async function preflightCredits(tx: Prisma.TransactionClient, schoolId: string, channel: ExternalChannel, body: string, recipientCount: number) {
  const current = await wallet(tx, schoolId);
  if (current.status !== "active") throw new AppError("This school's messaging wallet is not active. Ask SukuuNova administration to fund or reactivate it.", 409, "MESSAGING_WALLET_INACTIVE");
...
async function assertProvider(channel: ExternalChannel) {
...
export async function GET() {
  try {
    ...
    const data = await withTenant(session.schoolId, async (tx) => {
      ...
      return {
        messages,
        recipients: recipients.map((recipient) => ({ id: recipient.id, name: recipient.name, phone: recipient.phone, roles: recipient.userRoles.map((item) => item.role.name) })),
        settings: {
          smsSenderId: settings?.smsSenderId || null,
          channels: configuredChannelList(settings?.notificationChannels),
          automation: configuredAutomation(settings?.notificationChannels),
          whatsapp: settings?.whatsappTemplateConfig || {},
          smsCredits: currentWallet.smsBalance,
          whatsappCredits: currentWallet.whatsappBalance,
          smsSellRate: Number(currentWallet.smsSellRate),
          whatsappSellRate: Number(currentWallet.whatsappSellRate),
          lowBalanceThreshold: currentWallet.lowBalanceThreshold,
  ...
  try {
    ...
    if (input?.action === "send") {
      ...
      return withTenant(session.schoolId, async (tx) => {
        ...
        revalidatePath("/school/communications/announcements");
        const sentCount = value.channel === "in_app" ? recipients.length : deliverable.length;
        return NextResponse.json({ ok: true, credits, message: `Message queued for ${sentCount} recipient${sentCount === 1 ? "" : "s"}.${credits ? ` ${credits.required} credit${credits.required === 1 ? "" : "s"} reserved; ${credits.balanceAfter} remain.` : ""}${capped ? ` Limited to ${MAX_BROADCAST_RECIPIENTS} per request — narrow the audience and send again for the rest.` : ""}` });
      });
    }
    ...
    if (input?.action === "broadcast") {
      ...
      return withTenant(session.schoolId, async (tx) => {
        ...
        revalidatePath("/school/communications/messages");
        revalidatePath("/school/communications/broadcasts");
        const timing = scheduledAt ? `Broadcast scheduled for ${scheduledAt.toLocaleString("en-GH")}.` : "Broadcast queued.";
        return NextResponse.json({ ok: true, credits, message: `${timing} ${queued} recipient${queued === 1 ? "" : "s"}; ${credits.required} credit${credits.required === 1 ? "" : "s"} reserved and ${credits.balanceAfter} remain.${capped ? ` Limited to ${MAX_BROADCAST_RECIPIENTS} per request${scheduledAt ? "." : " — narrow the audience and send again for the rest."}` : ""}` });
      });
    }
    ...
    if (input?.action === "save_settings") {
      ...
      if (flag(input.sms_enabled)) channels.push("sms");
      if (flag(input.whatsapp_enabled)) channels.push("whatsapp");
      ...
      if (reportCardMediaBase) {
        try { new URL(reportCardMediaBase); } catch { return NextResponse.json({ error: "INVALID_INPUT", message: "Report-card media base must be a valid URL." }, { status: 400 }); }
      }
      return withTenant(session.schoolId, async (tx) => {
        ...
        if (whatsappFrom) Object.assign(nextConfig, { from: whatsappFrom });
        if (reportCardMediaBase) Object.assign(nextConfig, { reportCardMediaBase });
        const notificationConfig = {
          channels,
          automation: {
            payment_received: Boolean(flag(input.payment_received)),
            report_card_ready: Boolean(flag(input.report_card_ready)),
            student_absence: Boolean(flag(input.student_absence)),
            staff_late: Boolean(flag(input.staff_late)),
            transport_boarding: Boolean(flag(input.transport_boarding)),
            emergency_broadcast: Boolean(flag(input.emergency_broadcast)),
          },
        };
