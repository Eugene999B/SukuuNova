...
const timetable=z.object({days:z.array(day).min(1).max(7),periodMinutes:z.number().int().min(20).max(480),breaks:z.array(z.object({name:z.string().min(1).max(40),start:z.string().regex(/^\d{2}:\d{2}$/),end:z.string().regex(/^\d{2}:\d{2}$/)})).max(8),periodsPerDay:z.number().int().min(1).max(16),published:z.boolean(),weeklyPeriods:z.record(z.string(),z.number().int().min(1).max(10)).optional(),periods:z.array(period).max(16).optional()});
...
export async function POST(request:Request){try{const session=await requireSchoolSession();const input=schema.parse(await request.json());return await withTenant(session.schoolId,async tx=>{if(input.action==="save"){const result=await saveAcademicEngineConfig(tx,{schoolId:session.schoolId,actorId:session.userId,timetable:input.timetable,assessment:input.assessment,reportCard:input.reportCard});return NextResponse.json(result);}const result=await generateBalancedTimetable(tx,{schoolId:session.schoolId,actorId:session.userId,replaceExisting:input.replaceExisting,classIds:input.classIds});return NextResponse.json(result);});}catch(e){return routeError(e);}}
