...
export async function GET() {
  try {
    ...
    const result = await withTenant(session.schoolId, async (tx) => {
      ...
      const cards = await listIdentityCards(tx, session.schoolId, school.uniqueCode);
      return { school, cards };
    });
...
export async function POST(request: Request) {
  try {
    ...
    if (input.action === "download" && input.scope === "selected" && !(input.ids?.length)) {
      throw new AppError("Select at least one card to download.", 400, "NO_SELECTION");
    }
    const result = await withTenant(session.schoolId, async (tx) => {
      await requirePermission(tx, session.userId, "identity_cards:manage");
      const school = await tx.school.findUnique({ where: { id: session.schoolId }, select: { name: true, uniqueCode: true, logoUrl: true, brandColors: true } });
      ...
      await ensureIdentityCardsForSchool(tx, session.schoolId, school.uniqueCode);
      ...
      const pdf = await buildIdentityCardPdf(cards, school, origin);
    ...
    return new NextResponse(result.pdf, {
      ...
      headers: {
        ...
        "content-disposition": `attachment; filename="${session.schoolId}-identity-cards.pdf"`,
