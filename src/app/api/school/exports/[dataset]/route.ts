...
const EXPORT_PERMISSIONS: Record<string,string> = { students: "exports:students", staff: "exports:staff", attendance: "exports:attendance", fees: "exports:finance", gradebook: "exports:gradebook" };
...
function parseDate(value: string | null, endOfDay = false) { if (!value) return undefined; const date = new Date(`${value}${endOfDay ? "T23:59:59.999Z" : "T00:00:00.000Z"}`); if (Number.isNaN(date.getTime())) throw new AppError("Export date is invalid.", 400, "INVALID_EXPORT_DATE"); return date; }
  ...
  try {
    ...
    const result = await withTenant(session.schoolId, async (tx) => {
      ...
      if (dataset === "attendance") {
        const from = parseDate(url.searchParams.get("from")); const to = parseDate(url.searchParams.get("to"), true); if (from && to && from > to) throw new AppError("Export start date must not be after the end date.", 400, "INVALID_EXPORT_RANGE");
      ...
      if (dataset === "gradebook") {
        const assessments = await tx.assessment.findMany({ orderBy: [{ class: { name: "asc" } }, { subject: { name: "asc" } }, { name: "asc" }], select: { id: true, name: true, type: true, maxScore: true, scores: { select: { studentId: true, value: true, student: { select: { admissionNo: true, name: true } } } }, class: { select: { name: true } }, subject: { select: { name: true } } } });
        const rows: unknown[][] = [];
        for (const assessment of assessments) for (const score of assessment.scores) rows.push([assessment.class.name, assessment.subject.name, assessment.name, assessment.type, Number(assessment.maxScore).toFixed(2), score.student?.admissionNo ?? "", score.student?.name ?? "", Number(score.value).toFixed(2)]);
        return { filename: `${school.uniqueCode}-gradebook.csv`, body: csv(["Class", "Subject", "Assessment", "Category", "Max Score", "Admission No", "Student Name", "Score"], rows) };
      }
