...
export async function GET(request: Request) {
  try {
    ...
    return await withTenant(session.schoolId, async (tx) => {
      ...
      const submission = await tx.$queryRawUnsafe<Array<Record<string, unknown>>>(`SELECT s."id",s."studentId",st."name" AS "studentName",s."status",s."totalAwarded",s."submittedAt",s."reviewNotes" FROM "TeacherAcademicSubmission" s INNER JOIN "Student" st ON st."id"=s."studentId" AND st."schoolId"=s."schoolId" WHERE s."id"=$1 AND s."workId"=$2 AND s."schoolId"=$3 LIMIT 1`, submissionId, workId, session.schoolId);
