...
import { createExpenseV2, createFinanceCategory, createFeeStructureV2, createScholarshipProgramV2, decideExpenseV2, financeV2Snapshot } from "@/lib/finance-v2-service";
import { publishFeeStructureV2Safe } from "@/lib/finance-v2-publish-service";
...
import { recordLegacyInvoicePayment } from "@/lib/finance-legacy-payment";

const line=z.object({categoryId:z.string().min(1),amount:z.coerce.number().positive(),dueDate:z.string().nullable().optional(),optional:z.boolean().optional()});
...
const schema=z.discriminatedUnion("action",[
  ...
  z.object({action:z.literal("payment.record"),studentId:z.string().min(1),invoiceId:z.string().min(1),method:z.enum(["cash","momo","bank","card","cheque"]),reference:z.string().trim().min(1).max(120),allocations:z.array(allocation).min(1).max(30)}),
  z.object({action:z.literal("payment.legacy"),studentId:z.string().min(1),invoiceId:z.string().min(1),amount:z.coerce.number().positive(),method:z.enum(["cash","momo","bank","card","cheque"]),reference:z.string().trim().min(1).max(120)}),
