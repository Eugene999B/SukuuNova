...
import { createExpenseV2, createFinanceCategory, createFeeStructureV2, createScholarshipProgramV2, decideExpenseV2, financeV2Snapshot, publishFeeStructureV2, recordAllocatedPaymentV2, awardScholarshipV2 } from "@/lib/finance-v2-service";
...
const schema=z.discriminatedUnion("action",[
  ...
  z.object({action:z.literal("payment.record"),studentId:z.string().min(1),invoiceId:z.string().min(1),method:z.string().min(1).max(30),reference:z.string().max(120).nullable().optional(),allocations:z.array(allocation).min(1).max(30)}),
...
export async function POST(request:Request){try{const session=await requireSchoolSession();const input=await parseJson(request,schema);const result=await withTenant(session.schoolId,async tx=>{switch(input.action){case"category.create":return createFinanceCategory(tx,{schoolId:session.schoolId,actorId:session.userId,...input});case"structure.create":return createFeeStructureV2(tx,{schoolId:session.schoolId,actorId:session.userId,...input});case"structure.publish":return publishFeeStructureV2(tx,{schoolId:session.schoolId,actorId:session.userId,structureId:input.structureId});case"payment.record":return recordAllocatedPaymentV2(tx,{schoolId:session.schoolId,actorId:session.userId,...input});case"scholarship.program.create":return createScholarshipProgramV2(tx,{schoolId:session.schoolId,actorId:session.userId,...input});case"scholarship.award":return awardScholarshipV2(tx,{schoolId:session.schoolId,actorId:session.userId,...input});case"expense.create":return createExpenseV2(tx,{schoolId:session.schoolId,actorId:session.userId,...input});case"expense.decide":return decideExpenseV2(tx,{schoolId:session.schoolId,actorId:session.userId,...input});}});return NextResponse.json({ok:true,result},{headers:{"cache-control":"private, no-store"}});}catch(error){return routeError(error);}}
