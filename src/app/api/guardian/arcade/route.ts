...
import { assertArcadeV5ProgressionAvailable, guardianArcadeV5Progress } from "@/lib/arcade-v5-progress";
...
const schema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("start"), studentId: z.string().min(1).max(100), game: z.string().min(1).max(100), ageBand: ageBand.optional(), easier: z.boolean().optional(), roundLength: z.number().int().min(1).max(50).optional(), challengeMode: z.boolean().optional(), node: z.number().int().min(1).max(20).optional(), level: z.number().int().min(1).max(20).optional() }),
...
export async function GET(request: Request) {
  try {
    ...
    return json(await withTenant(current.schoolId, async (tx) => {
      const overview = await arcadeOverview(tx, current, url.searchParams.get("studentId") || undefined);
      const gameProgress = overview.selected ? await guardianArcadeV5Progress(tx, current, overview.selected.id) : [];
      return { ...overview, gameProgress };
    }));
  ...
  try {
    ...
    return json(await withTenant(current.schoolId, async (tx) => {
      if (input.action === "start") {
        ...
        const requestedNode = input.node ?? input.level;
        await assertArcadeV5ProgressionAvailable(tx, current, input.studentId, input.game, requestedNode);
        const round = await startVariedArcadeRound(tx, current, { ...input, node: requestedNode });
        if (input.game === "force-motion-lab") {
          ...
          await recordNovaCoreDecisionBestEffort(tx, {
            ...
            inputFingerprint: fingerprintNovaCoreInput({
              ...
              engine: round.engine,
              progressionMode: round.progressionMode,
              progressionNode: round.progressionNode,
            }),
            confidence: 1,
