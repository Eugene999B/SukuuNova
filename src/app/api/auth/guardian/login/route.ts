...
import { GUARDIAN_COOKIE, createGuardianSessionToken, sessionCookieOptions } from "@/lib/guardian-auth";
...
export async function POST(request: Request) {
  try {
    ...
    response.cookies.set(GUARDIAN_COOKIE, await createGuardianSessionToken({ kind: "guardian", userId: guardian.user.id, guardianId: guardian.id, schoolId: school.id, name: guardian.name, schoolName: school.name, needsPasswordChange: await compare("12345", guardian.user.passwordHash) }), sessionCookieOptions());
