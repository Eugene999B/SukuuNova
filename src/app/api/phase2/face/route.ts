  ...
  try {
    ...
    const data = await withTenant(session.schoolId, async (tx) => {
      ...
      const [enrollments, reviews] = await Promise.all([
        tx.faceEnrollment.findMany({ select: { id: true, studentId: true, staffId: true, enrolledAt: true, consentByGuardianId: true, student: { select: { name: true } }, staff: { select: { name: true } } }, orderBy: { enrolledAt: "desc" } }),
        tx.faceMatchReview.findMany({ include: { candidateStudent: { select: { name: true } }, candidateStaff: { select: { name: true } } }, orderBy: { createdAt: "desc" }, take: 50 })
      ]);
...
export async function POST(request: Request) {
  try {
    ...
    const result = await withTenant<unknown>(session.schoolId, async (tx) => {
      ...
      if (input.action === "enrollStudent") return enrollFace(tx, { ...common, target: { studentId: input.studentId, consentByGuardianId: input.consentByGuardianId }, image: input.image });
      if (input.action === "enrollStaff") return enrollFace(tx, { ...common, target: { staffId: input.staffId }, image: input.image });
      if (input.action === "match") {
        if (input.periodId) await tx.$executeRaw`SELECT set_config('sukuunova.attendance_period', ${input.periodId}, true)`;
