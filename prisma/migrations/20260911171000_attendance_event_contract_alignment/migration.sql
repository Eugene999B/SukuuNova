-- Align the persisted AttendanceEvent contract with the canonical register and
-- the period-aware AttendanceRecord resolver. Earlier migrations taught the
-- resolver about absent/late events and additional device methods without
-- widening the original Phase 1 CHECK constraints, so valid register writes
-- could fail at the database boundary.

ALTER TABLE "AttendanceEvent"
  DROP CONSTRAINT IF EXISTS "AttendanceEvent_type_check";
ALTER TABLE "AttendanceEvent"
  ADD CONSTRAINT "AttendanceEvent_type_check"
  CHECK ("type" IN ('in','out','absent','late','excused'));

ALTER TABLE "AttendanceEvent"
  DROP CONSTRAINT IF EXISTS "AttendanceEvent_method_check";
ALTER TABLE "AttendanceEvent"
  ADD CONSTRAINT "AttendanceEvent_method_check"
  CHECK ("method" IN ('manual','school_register','qr','face','fingerprint','card'));
...
BEGIN
  ...
  IF NEW."method" IN ('face','fingerprint','card','qr') THEN
    v_source := 'DEVICE';
  ELSIF NEW."method" IN ('manual','school_register') THEN
    v_source := 'MANUAL';
  ELSE
    v_source := 'IMPORT';
  END IF;
  ...
  ELSE
    ...
    ELSIF v_existing.status = 'PRESENT' AND v_status = 'LATE' THEN
      ...
      SET "status" = 'LATE',
          ...
          "updatedAt" = CURRENT_TIMESTAMP
      WHERE "id" = v_existing."id";
    ELSIF v_existing.status = 'LATE' AND v_status = 'PRESENT' THEN
      UPDATE "AttendanceRecord"
      SET "eventIds" = COALESCE("eventIds", '[]'::jsonb) || jsonb_build_array(NEW."id"),
          "updatedAt" = CURRENT_TIMESTAMP
      WHERE "id" = v_existing."id";
